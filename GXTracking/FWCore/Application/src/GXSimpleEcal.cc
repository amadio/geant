...
GXSimpleEcal::GXSimpleEcal( int nphi, int nz, double density)
  : GXUserGeometry( 1024 * nphi * nz )
{
  fNphi = nphi;
  fNz = nz;
  fDensity = density;
...
void GXSimpleEcal::create_impl()
{
  ...
  GPMaterial_Constructor_ByElement(ecal_mat,ecal_density);
  ...
  for ( int j = 0; j < crystal_nz ; ++j ) {
    for ( int i = 0; i < crystal_nphi ; ++i ) {
      ...
      GPThreeVector ecal_trans = 
	GPThreeVector_create(0,0,ecal_zmin+(2.0*j+1.0)*ecal_dz);
            
      GPLogicalVolume_Constructor(ecal_log+iptr, (GPVSolid*)ecal, ecal_mat);
      GPVPhysicalVolume_Constructor(ecal_phy+iptr, idRot, ecal_trans, 
				    ecal_log+iptr);
  ...
  for ( int j = 0; j < crystal_nz ; ++j ) {
    for ( int i = 0; i < crystal_nphi ; ++i ) {
      ...
      (ecal_phy+iptr)->fIndex = iptr; //just one physical volume per logical 
                                      //all are in the world logical volume.
    }
  }
}
