#include "GXSimpleCMS.h"

#include "GPBox.h"
#include "GPTubs.h"
#include "GPCons.h"

void GXSimpleCMS::create_impl()
{
...
template <class T> static T GXSimpleCMS::get( std::istream& in )
{
  T obj;
  in >> obj;
...
static G4double GXSimpleCMS::degToRad( G4double d )
{
  return d / 360.0 * twopi;
}

GPThreeVector GXSimpleCMS::readVec( std::istream& in )
{
...
GPRotationMatrix GXSimpleCMS::readRot( std::istream& in )
{
  G4double elem[3][3];
  for (int i=0; i<3; ++i)
    for (int j=0; j<3; ++j)
...
void GXSimpleCMS::readMaterials( std::istream& in )
{
...
void GXSimpleCMS::readSolids( std::istream& in )
{
...
void GXSimpleCMS::readStructure( std::istream &in )
{
