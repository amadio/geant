...
int main (int argc, char* argv[]) 
{
  ...
  int kernelType  = 0; //0 photon+electron, 1 photon kernel, 2 electron kernel
  int runType  = 0; //0 normal 1 test
  int numStep  = 1; //number of steps
  int ndivarg = -1; //number of concurrent 

  if(argc >= 2) kernelType = atoi(argv[1]);
  if(argc >= 3) runType = atoi(argv[2]);
  if(argc >= 4) numStep = atoi(argv[3]);
  if(argc >= 5) ndivarg = atoi(argv[4]);

  if(kernelType < 0 || kernelType > 5) {
    std::cout << "Usage: trackingEM kernelType=[0-2]: ... " << std::endl;
    return 0;
  }
  ...
  for (int i = 0 ; i < nevent ; i++) {
    ...
    if(kernelType == 0 || kernelType == 3)
      printf("= (%d,%d)\n", nElectrons,nPhotons);
    else if(kernelType == 1) printf("= (%d,%d)\n", 0,nPhotons);
    else if(kernelType == 2) printf("= (%d,%d)\n", nElectrons,0);
	...
	photon_gpu(devStates, photon_d0, count1, 
	...
	int count2 = min( NchunkG, nPhotons - (i+count1) );
	if (count2) {
	  cudaMemcpyAsync(photon_d1, photon_h+i+count1,
			  count2*sizeof(GXTrack), 
			  cudaMemcpyHostToDevice, stream1);
	  
	  photon_gpu(devStates, photon_d1, count2, 
		     (GPGeomManager*)geom_d, bmap_d, physicsTable_d,
		     secTracks_d, stackSize_d, offset_d, numStep, runType,
		     theNBlocks, theNThreads, stream1);
	  
	  cudaMemcpyAsync(photon_h+i+NchunkG, photon_d1, 
			  count2*sizeof(GXTrack), 
	...
	electron_gpu(devStates, electron_d0, count1, 
	...
	if (count2) {
	  cudaMemcpyAsync(electron_d1, electron_h+i+count1,
			  count2*sizeof(GXTrack), 
			  cudaMemcpyHostToDevice, stream1);
	  
	  electron_gpu(devStates, electron_d1, count2, 
		       (GPGeomManager*)geom_d,bmap_d,physicsTable_d,sbData_d,
		       secTracks_d, stackSize_d, offset_d, numStep, runType,
		       theNBlocks, theNThreads,stream1);
	  
	  cudaMemcpyAsync(electron_h+i+count1, electron_d1,
			  count2*sizeof(GXTrack), 
			  cudaMemcpyDeviceToHost, stream1);
	}
      }
