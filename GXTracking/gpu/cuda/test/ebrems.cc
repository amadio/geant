...
int main (int argc, char* argv[]) 
{
  ...
  //---------------------------------------------------------------------------
  BuildInversePDF_SeltzerBerger(Z, sbData, 1.0, minP,maxP,
			       NROW,NCOL, pdfx,pdfy); 
  //---------------------------------------------------------------------------
  cudaEventRecord (stop,0);
  cudaEventSynchronize (stop);
  cudaEventElapsedTime (&elapsedTimeInit_PDF,start,stop);
  printf("Time to build SeltzerBerger PDF Tables = %f\n",elapsedTimeInit_PDF);

  cudaEventRecord (start,0);
  //---------------------------------------------------------------------------
  BuildAliasSamplingTable_SeltzerBerger(Z, sbData, 1.0, minP,maxP,
				       NROW, NCOL, pdfx, pdfa, pdfq); 
  //---------------------------------------------------------------------------
  cudaEventRecord (stop,0);
  cudaEventSynchronize (stop);
  cudaEventElapsedTime (&elapsedTimeInit_Alias,start,stop);
  printf("Time to build SeltzerBerger Alias Tables = %f\n",
  ...
  theHisto->BookHistograms("ebrems.root");
