  ...
  for (int i = 0 ; i < nevent ; i++) {
    ...
    if(kernelType ==1) {
      for(int i = 0, round = 0 ; i < nTracks ; i += 2*Nchunk, ++round) { 
        ...
        tracking_gpu(devStates,(GPGeomManager*)geom_d,bmap_d,
                     track_d0,
      ...
      for(int i = 0 ; i < nPhotons ; i += 2*NchunkG) { 
         ...
         tracking_photon_gpu(devStates,(GPGeomManager*)geom_d,bmap_d,photon_d0,
                             eBrem_table_d, eIoni_table_d, msc_table_d,
                             count1, 
                             theNBlocks,theNThreads, stream0);
         ...
         tracking_photon_gpu(devStates,(GPGeomManager*)geom_d,bmap_d,photon_d1,
                             eBrem_table_d, eIoni_table_d, msc_table_d,
      ...
      for(int i = 0 ; i < nElectrons ; i += 2*NchunkE) { 
        ...
        tracking_electron_gpu(devStates1,(GPGeomManager*)geom_d,bmap_d,electron_d0,
                              eBrem_table_d, eIoni_table_d, msc_table_d,
                              count1, 
                              theNBlocks,theNThreads, stream0);
        ...
        tracking_electron_gpu(devStates1,(GPGeomManager*)geom_d,bmap_d,electron_d1,
                              eBrem_table_d, eIoni_table_d, msc_table_d,
                              count2, 
                              theNBlocks,theNThreads, stream1);
    ...
    printf("Time Elapsed on GPU CPU : %6.3f %6.3f ms\n",elapsedTimeGPU,elapsedTime2);
