...
void setupMagField(int, int, float*, float*);
...
int main (int argc, char* argv[]) 
{
  ...
  int nbinZ2 = (nbinZ + 1) / 2;
  int nbinR2 = nbinR * 2;
  float** fieldMapZ = (float**) malloc( nbinR * sizeof(float*) );
  float** fieldMapR = (float**) malloc( nbinZ2 * sizeof(float*) );
  for (int i = 0; i < nbinR; i++) {
    fieldMapZ[i] = (float*) malloc( nbinZ * sizeof(float) );
  }
  for (int i = 0; i < nbinZ2; i++) {
    fieldMapR[i] = (float*) malloc( nbinR2 * sizeof(float) );
  }
  ...
  if (ifile.is_open()) {
    ...
    for(int i = 0 ; i < ngrid ; i++) {
      ...
      if(abs(fd.iz) > noffZ || fd.ir > nbinR) {
        ...
        fieldMapZ[fd.ir][noffZ+fd.iz] = fd.Bz;
        if (fd.iz <= 0) {
          fieldMapR[fd.iz+noffZ][fd.ir] = fd.Br;
        } else {
          fieldMapR[fd.iz][fd.ir+nbinR] = fd.Br;
        }
      }
    }
    ifile.close();
  }
  printf("After closing we have at 1600 334 : %f\n",fieldMapR[1600][334]);
  printf("Compared to 1600 334 : %lf\n",fieldMap[1600][334].Br);
  ...
  setupMagField(nbinZ, nbinR, &fieldMapZ[0][0], &fieldMapR[0][0]);
  ...
  for (int i = 0 ; i < nevent ; i++) {
    ...
    if(kernelType == 1 || kernelType == 2) {
      ...
      //counter for the last array position of physics processes
      count_by_process_block_gpu(
        nElectrons, electron_d, nbrem_d, nioni_d, theNBlocks, theNThreads, stream0);
