...
SteppingActionBrem::SteppingActionBrem()                                         
{
...
void SteppingActionBrem::UserSteppingAction(const G4Step* aStep)
{
  ...
  // G4cout <<"SteppingActionBrem: trkID="<< track->GetTrackID()
...
#ifdef GPUDEBUG
  G4cout<<"\nBefore brem: Ene="<< eleEne/MeV <<" Ekin="<< (eleEne-mass)/MeV <<' '<< preStepPoint->GetKineticEnergy()/MeV
...
#ifdef GPUPLOTS
  hmgr.getHisto("hbremEnergy").Fill(pcEtot-mass);
  hmgr.getHisto("hbremAngle").Fill(pcAngle);

  G4dataBuffer& buffers = G4dataBuffer::getInstance();
#ifdef GPUDEBUG
  G4cout<<"SteppingActionBrem: preStepLambda buffer: "<< buffers.getBuffer("hbremPreStepLambda").size() << G4endl;
#endif

  hmgr.getHisto("hbremStepLengthPost").Fill(stepl/mm);
  hmgr.getHisto("hbremEnergyLoss").Fill( (preStepPoint->GetTotalEnergy()-postStepPoint->GetTotalEnergy())/MeV );

  hmgr.fillFromVector("hbremPreStepLambda", buffers.getBuffer("hbremPreStepLambda") );
  hmgr.fillFromVector("hbremPreStepScaledEnergy", buffers.getBuffer("hbremPreStepScaledEnergy") );
  hmgr.fillFromVector("hbremNbOfIntLengthLeft", buffers.getBuffer("hbremNbOfIntLengthLeft") );

#endif // GPUPLOTS

#ifdef GPUDEBUG
  G4cout<<"After brem: Ene="<< eleEne/MeV <<" Ekin="<< (eleEne-mass)/MeV <<' '<< postStepPoint->GetKineticEnergy()/MeV
...
#ifdef GPUPLOTS
    hmgr.getHisto("hbremSecAngle").Fill(angle);
...
void SteppingActionBrem::WritePhysicsTables( const G4Step * theStep ) {
