...
SteppingActionMsc::SteppingActionMsc()
{
...
void SteppingActionMsc::UserSteppingAction(const G4Step* aStep)
{
  ...
  // G4cout <<"SteppingActionMsc: trkID="<< track->GetTrackID()
...
#ifdef GPUDEBUG
  G4cout<<"\nBefore msc: Ene="<< eleEne/MeV <<" Ekin="<< (eleEne-mass)/MeV <<' '<< preStepPoint->GetKineticEnergy()/MeV
...
#ifdef GPUPLOTS
  hmgr.getHisto("hmscEnergy").Fill(pcEtot-mass);
  hmgr.getHisto("hmscAngle").Fill(pcAngle);

  G4dataBuffer& buffers = G4dataBuffer::getInstance();
#ifdef GPUDEBUG
  G4cout<<"SteppingActionMsc: preStepLambda buffer: "<< buffers.getBuffer("hmscPreStepLambda").size() << G4endl;
#endif

  hmgr.getHisto("hmscStepLengthPost").Fill(stepl/mm);
  hmgr.getHisto("hmscEnergyLoss").Fill( (preStepPoint->GetTotalEnergy()-postStepPoint->GetTotalEnergy())/MeV );

  hmgr.fillFromVector("hmscPreStepLambda", buffers.getBuffer("hmscPreStepLambda") );
  hmgr.fillFromVector("hmscPreStepScaledEnergy", buffers.getBuffer("hmscPreStepScaledEnergy") );
  hmgr.fillFromVector("hmscNbOfIntLengthLeft", buffers.getBuffer("hmscNbOfIntLengthLeft") );

#endif // GPUPLOTS

#ifdef GPUDEBUG
  G4cout<<"After msc: Ene="<< eleEne/MeV <<" Ekin="<< (eleEne-mass)/MeV <<' '<< postStepPoint->GetKineticEnergy()/MeV
...
#ifdef GPUPLOTS
    hmgr.getHisto("hmscSecAngle").Fill(angle);
    hmgr.getHisto("hmscSecEnergy").Fill(secKinE/MeV);
#endif
  }
  //=============================================
...
void SteppingActionMsc::WritePhysicsTables( const G4Step * theStep ) {
