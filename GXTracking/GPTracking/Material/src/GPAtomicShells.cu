...
{
  assert (Z>0 && Z<101);
  return GPAtomicShells::fNumberOfShells[Z];
}

FQUALIFIER
G4double GPAtomicShells_GetBindingEnergy(G4int Z, G4int ShellNb)
{
  assert (Z>0 && Z<101 && ShellNb < GPAtomicShells::fNumberOfShells[Z]);
  return GPAtomicShells::fBindingEnergies[GPAtomicShells::fIndexOfShells[Z] 
					  + ShellNb]*eV;
}

FQUALIFIER
G4int GPAtomicShells_GetNumberOfElectrons(G4int Z, G4int ShellNb)
{
  assert (Z>0 && Z<101 && ShellNb<GPAtomicShells::fNumberOfShells[Z]);
  return GPAtomicShells::fNumberOfElectrons[GPAtomicShells::fIndexOfShells[Z] 
					    + ShellNb];
}
...
G4double GPAtomicShells_GetTotalBindingEnergy (G4int Z)
{
  ...
  G4int idx = GPAtomicShells::fIndexOfShells[Z];
  G4int idxmax = idx + GPAtomicShells::fNumberOfShells[Z];
  G4double energy = 0.0;
  for (G4int i=idx; i<idxmax; ++i) {
    energy += GPAtomicShells::fBindingEnergies[i];
  }
  return energy*eV;
}
...
namespace GPAtomicShells 
{
   ...
   VARTYPE G4int fNumberOfShells[101] = 
      {
...
// 1 + G4AtomicShells::TotalNumberOfShells(100) = 1 + 1539 = 1540 
   ...
   VARTYPE G4int fIndexOfShells[101] =
  {    0,
   ...
   VARTYPE G4double fBindingEnergies[1540] = 
   {
 ...
 143090.0 , 27700.0 , 26810.0 , 20900.0 , 7205.0 , 6793.0 , 5397.0 , 4766.0 ,
   ...
   VARTYPE G4int fNumberOfElectrons[1540] =
      {
  0,   // Nonexisting zero element
  //  H  ---------------------------------------------------------
  1,
