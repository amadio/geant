...
void *GPUserGeometry::reserveThingUnaligned( int thingsz )
{
   assert(thingsz > 0);
...
void *GPUserGeometry::reserveThingAligned( int thingsz, int alignment )
{
...
void *GPUserGeometry::reserveThing( int thingsz )
{
   return reserveThingAligned( thingsz, default_alignment );
}

void *GPUserGeometry::addThingAligned( const void *thing,
...
void *GPUserGeometry::addThingUnaligned( const void *thing, int thingsz )
{
...
void *GPUserGeometry::addThing( const void *thing, int thingsz )
{
...
void GPUserGeometry::addLogicalVolumePointers( GPLogicalVolume *vol )
{
...
void GPUserGeometry::addPhysicalVolumePointers( GPVPhysicalVolume *vol )
{
 ...
 */
void GPUserGeometry::addLogicalVolumeDaughter( GPLogicalVolume *vol,
                                              GPVPhysicalVolume *d )
{
...
GPSmartVoxelNode *GPUserGeometry::storeVoxelNode( const GPSmartVoxelNode &original )
{
   nvoxelnodes++;
...
GPSmartVoxelHeader *GPUserGeometry::storeVoxelHeader( const GPSmartVoxelHeader &original )
{
...
void GPUserGeometry::createVoxels( GPLogicalVolume *mother, G4double smartless = kInfinity )
{
...
G4double GPUserGeometry::uniformRand()
{
#ifdef GPUNONRANDOM
   return 0.123456
#else
...
GPRotationMatrix GPUserGeometry::randomRot()
{
 ...
 */
GPUserGeometry::GPUserGeometry( void *buffer, int maxsz,
...
GPUserGeometry::GPUserGeometry( int maxsz )
:
userbuf(false),
create_called(false),
bufsz(0),
...
GPUserGeometry::~GPUserGeometry()
{
   if (!userbuf) std::free( buf );
}

void GPUserGeometry::relocate( void *newbegin )
{
...
template void GPUserGeometry::addPointer( GPLogicalVolume **& ptr );
