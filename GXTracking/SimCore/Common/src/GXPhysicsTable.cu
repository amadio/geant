...
FQUALIFIER 
GXPhysicsVector::GXPhysicsVector() {
  type = 2;
  edgeMin = 0;
  edgeMax = 0;
  numberOfNodes = 0;
...
FQUALIFIER 
GXPhysicsVector::GXPhysicsVector(GXPhysicsVector& right) {
...
FQUALIFIER
bool GXPhysicsVector::SplinePossible() {
...
FQUALIFIER
void GXPhysicsVector::ComputeSecDerivatives() {
...
FQUALIFIER
void GXPhysicsVector::ComputeSecondDerivatives(G4double firstPointDerivative,
...
FQUALIFIER
void GXPhysicsVector::FillSecondDerivatives() {
  ...
  //cout << "GXPhysicsVector_FillSecondDerivatives() n= " << n 
  //     << "   " << this << endl;
  // cout << *this << endl;
...
FQUALIFIER
int GXPhysicsVector::FindBinLocation(G4double energy) {
  return int(log10(energy) / dBin - baseBin);
}

FQUALIFIER
G4double GXPhysicsVector::SplineInterpolation(double energy, int lastBin) {
...
FQUALIFIER
G4double GXPhysicsVector::LinearInterpolation(double energy, int lastBin) {
...
FQUALIFIER
G4double GXPhysicsVector::Interpolation(double energy, int lastBin) {
  G4double value = 0;
  if (useSpline) {
...
FQUALIFIER
void GXPhysicsVector::SetSpline(bool val) {
  useSpline = val;
  if (!isSecondDerivativeFilled) {
    FillSecondDerivatives();
    isSecondDerivativeFilled = true;
...
FQUALIFIER
G4double GXPhysicsVector::Value(double energy) {
...
FQUALIFIER GXPhysicsTable::GXPhysicsTable() {
  nPhysicsVector = 0;
}

FQUALIFIER
G4double GXPhysicsVector::Energy(size_t binNumber)
{
  return binVector[binNumber];
}

FQUALIFIER
G4double GXPhysicsVector::InvRangeValue(double energy) 
{
  G4double value = 0.0;
...
FQUALIFIER
int GXPhysicsVector::InvRangeFindBinLocation(G4double energy) 
{
  G4int n1 = 0;
...
FQUALIFIER
void GXPhysicsTable::Print() {
  /*
  printf("%f\n",nPhysicsVector);
  
  for(int idx=0; idx<nPhysicsVector; idx++){
    GXPhysicsVector* pv = &(physicsVectors[idx]);
    printf("%d\n",pv->type);
