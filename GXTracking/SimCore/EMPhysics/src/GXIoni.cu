...
FQUALIFIER
void GXIoni::SetCurandState(curandState* v) {
	localState = v;
}

FQUALIFIER
void GXIoni::UseIntegral(bool v) {
	integral = v;
}

FQUALIFIER
void GXIoni::InitialiseStep(G4double kineticEnergy) {
...
FQUALIFIER
void GXIoni::SetLambdaTable(GPPhysicsTable* val) {
	theLambdaTable = val;
}

FQUALIFIER
void GXIoni::UseLambdaTable(bool v) {
	useLambdaTable = v;
}

FQUALIFIER
void GXIoni::ResetNumberOfInteractionLengthLeft() {
	theNumberOfInteractionLengthLeft = -log(rand_wrapper(localState));
}

FQUALIFIER
void GXIoni::SubtractNumberOfInteractionLengthLeft(
...
FQUALIFIER G4double GXIoni::ComputeCrossSectionPerElectron(double kineticEnergy, double cutEnergy, double maxEnergy) {
...
FQUALIFIER
G4double GXIoni::ComputeCrossSectionPerAtom(double kineticEnergy, double Z,
		G4double cut) {
  return Z*ComputeCrossSectionPerElectron(kineticEnergy,cut, 100.0*TeV);
}

FQUALIFIER
G4double GXIoni::CrossSectionPerVolume(double kineticEnergy,
...
FQUALIFIER
G4double GXIoni::GetCurrentLambda(double e) {

  G4double x = 0.0;
  if (useLambdaTable && theLambdaTable) {
    //    x = GXIoni::GetLambdaFromTable(e);
...
FQUALIFIER
void GXIoni::ComputeIntegralLambda(G4double e) {
...
FQUALIFIER
G4double GXIoni::PostStepGetPhysicalInteractionLength(double kineticEnergy,
...
FQUALIFIER
G4double GXIoni::SampleSecondaries(GXTrack* dp, double tmin,
				 G4double maxEnergy) {

  G4double deltaKinEnergy = 0.0; // to be returned

  //  G4double kineticEnergy = dp->E;@@@calculate this
  G4double kineticEnergy = 100.0;
  ...
  if (isElectron) {
    ...
    } while(grej * rand_wrapper(localState) > z);
  ...
  // G4double phi = twopi * rand_wrapper(localState);
...
G4double GXIoni::PostStepDoIt(GXTrack* dp) {
