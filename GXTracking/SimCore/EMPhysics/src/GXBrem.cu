...
FQUALIFIER
void GXBrem::SetCurandState(curandState* v) {
	localState = v;
}

FQUALIFIER
void GXBrem::UseIntegral(bool v) {
	integral = v;
}

FQUALIFIER
void GXBrem::InitialiseStep(G4double kineticEnergy) {
...
FQUALIFIER
void GXBrem::SetLambdaTable(GPPhysicsTable* val) {
	theLambdaTable = val;
}

FQUALIFIER
void GXBrem::UseLambdaTable(bool v) {
	useLambdaTable = v;
}

FQUALIFIER
void GXBrem::ResetNumberOfInteractionLengthLeft() {
	theNumberOfInteractionLengthLeft = -log(rand_wrapper(localState));
}

FQUALIFIER
void GXBrem::SubtractNumberOfInteractionLengthLeft(
...
FQUALIFIER
G4double GXBrem::SupressionFunction(double kineticEnergy, double gammaEnergy,
		bool LPMFlag) {
...
FQUALIFIER
G4double GXBrem::PositronCorrFactorSigma(double Z, double kineticEnergy,
		G4double cut) {
...
FQUALIFIER
G4double GXBrem::ComputeCrossSectionPerAtom(double kineticEnergy, double Z,
		G4double cut) {
	G4double cross = 0.0;
...
FQUALIFIER
G4double GXBrem::CrossSectionPerVolume(double kineticEnergy,
...
FQUALIFIER
G4double GXBrem::GetCurrentLambda(double e) {

	G4double x = 0.0;
	if (useLambdaTable && theLambdaTable) {
		//    x = GXBrem::GetLambdaFromTable(e);
...
FQUALIFIER
void GXBrem::ComputeIntegralLambda(G4double e) {
...
FQUALIFIER
G4double GXBrem::PostStepGetPhysicalInteractionLength(double kineticEnergy, G4double previousStepSize,GPForceCondition* condition) {
...
G4double GXBrem::ScreenFunction1(double ScreenVariable) {
...
G4double GXBrem::ScreenFunction2(double ScreenVariable) {
...
G4double GXBrem::PolarAngle(double initial_energy, double final_energy,
		int Z) {
	...
	do {
		u = -log(
				rand_wrapper(localState)
						* rand_wrapper(localState));

		if (border > rand_wrapper(localState))
			u /= a1;
		else
			u /= a2;
...
FQUALIFIER
G4double GXBrem::GPVEmModel_CrossSectionPerVolume(double ekin, double emin,
		G4double emax) {
	G4double cross = 0.0;
	const G4double theAtomNumDensity[nElements] = { 1.0958e+19, 1.0958e+19,
			4.3832e+19 };
...
int GXBrem::SelectRandomAtom(G4double kinEnergy, double tcut, double tmax) {
...
FQUALIFIER
G4double GXBrem::SampleSecondaries(GXTrack* dp, double tmin,
		G4double maxEnergy)
...
//    (Nuc Phys 20(1960),15).
	...
	//	G4double kineticEnergy = dp->E;@@@@calcuate this
	G4double kineticEnergy = 100.;
		...
		if (kineticEnergy > tlow) {
			do {
				...
				greject = migdal * (F1 - epsil * (ah * F1 - bh * epsil * F2))
						/ (42.392 - FZ);
			} while (greject < rand_wrapper(localState) * grejmax);
...
FQUALIFIER
//G4double GPVEnergyLossProcess_PostStepDoIt(GXTrack* dp)
G4double GXBrem::PostStepDoIt(GXTrack* dp) {
	...
	//	G4double finalT = dp->E;@@@calcuate this
	G4double finalT = 100.0;
...
//  }
	...
	if (integral) {
		...
		if (lx <= 0.0) {
			return 0;
		} else if (preStepLambda * rand_wrapper(localState) > lx) {
			return 0;
		}
	}
