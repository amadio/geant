...
#include "GPRandom.h"
#else
FQUALIFIER G4double rand_wrapper(curandState* devState);
FQUALIFIER G4double rand_normal_wrapper(curandState* devState, int id, double mean, double sigma);
#endif
...
void GXMsc::Print() {
	...
	printf(
			"useLambdaTable(%d), integral(%d), lambdaFactor(%lf), theLambdaTable(%ld), \
...
theNumberOfInteractionLengthLeft(%lf), currentInteractionLength(%lf), isElectron(%d), LPMFlag(%d)\n",
			int(useLambdaTable), int(integral), lambdaFactor,
...
FQUALIFIER
void GXMsc::SetCurandState(curandState* v) {
	localState = v;
}

FQUALIFIER
void GXMsc::UseIntegral(bool v) {
	integral = v;
}

FQUALIFIER
void GXMsc::InitialiseStep(G4double kineticEnergy) {
...
FQUALIFIER
void GXMsc::SetLambdaTable(GPPhysicsTable* val) {
	theLambdaTable = val;
}

FQUALIFIER
void GXMsc::UseLambdaTable(bool v) {
	useLambdaTable = v;
}

FQUALIFIER
void GXMsc::ResetNumberOfInteractionLengthLeft() {
	theNumberOfInteractionLengthLeft = -log(rand_wrapper(localState));
}

FQUALIFIER
void GXMsc::SubtractNumberOfInteractionLengthLeft(
...
FQUALIFIER
G4double GXMsc::SupressionFunction(double kineticEnergy, double gammaEnergy,
		bool LPMFlag) {
...
FQUALIFIER
G4double GXMsc::PositronCorrFactorSigma(double Z, double kineticEnergy,
		G4double cut) {
...
FQUALIFIER
G4double GXMsc::ComputeCrossSectionPerAtom(double kineticEnergy, double Z,
		G4double cut) {
	G4double cross = 0.0;
...
FQUALIFIER
G4double GXMsc::CrossSectionPerVolume(double kineticEnergy,
...
FQUALIFIER
G4double GXMsc::GetCurrentLambda(double e) {

	G4double x = 0.0;
	if (useLambdaTable && theLambdaTable) {
		//    x = GXMsc::GetLambdaFromTable(e);
...
FQUALIFIER
void GXMsc::ComputeIntegralLambda(G4double e) {
...
FQUALIFIER
G4double GXMsc::AlongStepGetPhysicalInteractionLength(GXTrack* dp, double currentMinimalStep, GPGPILSelection* selection) {
  ...
  *selection = NotCandidateForSelection;
  ...
  //  G4double ekin = dp->E;@@@calcuate this
  G4double ekin = 100.;
...
FQUALIFIER G4double GXMsc::ComputeTruePathLengthLimit(GXTrack* dp, double currentMinimalStep) {
  ...
  //  currentKinEnergy = dp->E;@@@calculate this
    ...
    tlimit = geomlimit-0.999*skindepth;
      ...
      if((stepStatus == fUndefined) ||
        ...
        { 
          ...
          {
            do {
	      temptlimit = rand_normal_wrapper(localState,0,tlimit,0.3*tlimit);        
...
FQUALIFIER
G4double GXMsc::GetRange(double kinEnergy) {
  // from G4VMscModel.hh
...
FQUALIFIER
G4double GXMsc::ComputeGeomPathLength() {
  ...
  if(samplez)
  {
    ...
    if (tPathLength > stepmin && zt < ztmax)              
    {
      ...
      if(zt >= third)
      {
        ...
        G4double grej ;
        do {
           u = exp(log(rand_wrapper(localState))/cz1) ;
            grej = exp(cz*log(u/u0))*(1.-u)/(1.-u0) ;
           } while (grej < rand_wrapper(localState)) ;
      }
      else
      {
        u = 2.*zt*rand_wrapper(localState);
      }
...
FQUALIFIER
G4double GXMsc::PostStepGetPhysicalInteractionLength(double kineticEnergy,
...
FQUALIFIER
//G4double G4VMultipleScattering_AlongStepDoIt(GXTrack* dp)
G4double GXMsc::AlongStepDoIt(GXTrack* dp) {
  // from G4VMultipleScattering.cc
...
FQUALIFIER
//G4double G4VMultipleScattering_PostStepDoIt(GXTrack* dp)
G4double GXMsc::PostStepDoIt(GXTrack* dp) {
  // from G4VMultipleScattering.cc
...
FQUALIFIER G4double GXMsc::SampleScattering(GXTrack* dp)
{
  return 0.0;
