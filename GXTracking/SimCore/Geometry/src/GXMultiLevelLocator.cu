...
/*
GXMultiLevelLocator_~GXMultiLevelLocator()
{
  for ( G4int idepth=0; idepth<max_depth+1; idepth++)
  {
                ...
                GPThreeVector&      previousSftOrigin)             // In/Out
{
  ...
  GXFieldTrack  CurrentA_PointVelocity = CurveStartPointVelocity; 
  GXFieldTrack  CurrentB_PointVelocity = CurveEndPointVelocity;

  GPThreeVector CurrentE_Point = TrialPoint;
  G4bool        validNormalAtE = false;
  GPThreeVector NormalAtEntry;

  //GXFieldTrack ApproxIntersecPointV(CurveEndPointVelocity);//FT-Def-Construct
  GXFieldTrack &ApproxIntersecPointV = CurveEndPointVelocity;
  ...
  //
  GXFieldTrack SubStart_PointVelocity = CurveStartPointVelocity;
  ...
  do
  {
    ...
    do // REPEAT param
    {
      GPThreeVector Point_A = GXFieldTrack_GetPosition(&CurrentA_PointVelocity);  
      GPThreeVector Point_B = GXFieldTrack_GetPosition(&CurrentB_PointVelocity);
       
      // F = a point on true AB path close to point E 
      // (the closest if possible)
      //
      ApproxIntersecPointV = GXChordFinder_ApproxCurvePointV(
      ...
      GPThreeVector CurrentF_Point= GXFieldTrack_GetPosition(&ApproxIntersecPointV);
      ...
      GPThreeVector  NewMomentum= GXFieldTrack_GetMomentum(&ApproxIntersecPointV); 
      GPThreeVector NewMomentumDir = GPThreeVector_unit(NewMomentum);
      ...
      { 
        ...
        GXFieldTrack_SetPosition(&IntersectedOrRecalculatedFT,CurrentE_Point);
        ...
        if ( GPVIntersectionLocator_GetAdjustementOfFoundIntersection(This) )
        {
          ...
          //  
          GPThreeVector IP;
          GPThreeVector Momentum=GXFieldTrack_GetMomentum(&ApproxIntersecPointV);
          GPThreeVector MomentumDir=GPThreeVector_unit(Momentum);
          ...
          {
            IntersectedOrRecalculatedFT = ApproxIntersecPointV;
            GXFieldTrack_SetPosition(&IntersectedOrRecalculatedFT,IP);
          }
        }
	...
	last_AF_intersection = Intersects_AF;
	  ...
	  NormalAtEntry  = GPVIntersectionLocator_GetSurfaceNormal(This, 
        ...
        linDistSq = GPThreeVector_mag2(GPThreeVector_sub( 
		    GXFieldTrack_GetPosition(&CurrentB_PointVelocity), 
                    GXFieldTrack_GetPosition(&CurrentA_PointVelocity))); 

        curveDist = GXFieldTrack_GetCurveLength(&CurrentB_PointVelocity)
                  - GXFieldTrack_GetCurveLength(&CurrentA_PointVelocity);
	  ...
	  //G4Exception("GXMultiLevelLocator_EstimateIntersectionPoint()",
    ...
    {
      G4double did_len = fabs(GXFieldTrack_GetCurveLength(&CurrentA_PointVelocity)
			    - GXFieldTrack_GetCurveLength(&SubStart_PointVelocity));

      G4double all_len = fabs(GXFieldTrack_GetCurveLength(&CurrentB_PointVelocity)
			    - GXFieldTrack_GetCurveLength(&SubStart_PointVelocity));
         ...
         && (depth<max_depth) && (!sub_final_section) )
      {
        ...
        GXFieldTrack start = CurrentA_PointVelocity;
        GXMagInt_Driver* integrDriver
	  = GXChordFinder_GetIntegrationDriver(
                          GPVIntersectionLocator_GetChordFinderFor(This));

	GXMagInt_Driver_AccurateAdvance(integrDriver,
                                        start, Sub_len, 
        ...
        //
        GPThreeVector Point_A = GXFieldTrack_GetPosition(&CurrentA_PointVelocity);
        GPThreeVector SubE_point = GXFieldTrack_GetPosition(&CurrentB_PointVelocity);   
...
GPVIntersectionLocator_ReEstimateEndpoint(GXMultiLevelLocator *This, 
					  ...
					  G4double      curveDist )
{  
  GXFieldTrack& newEndPoint = CurrentStateA;
  GXMagInt_Driver* integrDriver= GXChordFinder_GetIntegrationDriver(
                     GPVIntersectionLocator_GetChordFinderFor(This)); 

  GXFieldTrack& retEndPoint = CurrentStateA;
  G4bool goodAdvance;
  G4int  itrial=0;
  const G4int no_trials= 20;

  G4double endCurveLen= GXFieldTrack_GetCurveLength(&EstimatedEndStateB);
  do
  {
     G4double currentCurveLen= GXFieldTrack_GetCurveLength(&newEndPoint);
     ...
     goodAdvance= 
       GXMagInt_Driver_AccurateAdvance(integrDriver,newEndPoint, advanceLength,
				       GPVIntersectionLocator_GetEpsilonStepFor(This),0);
     }
  }
  ...
  if( goodAdvance )
  {
    sumCorrectionsSq += GPThreeVector_mag2(GPThreeVector_sub(
			GXFieldTrack_GetPosition(&EstimatedEndStateB),
			GXFieldTrack_GetPosition(&newEndPoint)));
  }
...
FQUALIFIER
GPThreeVector 
GPVIntersectionLocator_GetSurfaceNormal(GXMultiLevelLocator *This, 
