...
__global__
void rebuildMap(vecgeom::map<double,double>* devMap, double* key, double* value, int N) {
...
void launchRebuildMap(vecgeom::cxx::DevicePtr<vecgeom::cuda::map<double,double> > &devMap, 
                      vecgeom::cxx::DevicePtr<double> key, vecgeom::cxx::DevicePtr<double> value, int N, int nBlocks, int nThreads) {
   int threadsPerBlock = nThreads;
   int blocksPerGrid   = nBlocks;
