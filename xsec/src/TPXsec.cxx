...
//_________________________________________________________________________
void TPXsec::Interp(double egrid[], float value[], int nbins, 
			   double eildelta, int stride, double en, float result[]) {
...
bool TPXsec::Resample() {
   ...
   for(int ien=0; ien<fNEbins; ++ien) {
      ...
      for(int ixs=0; ixs<fNXsec; ++ixs) xnorm+=lXSecs[ien*fNXsec+ixs];
      xnorm = 1/xnorm;
      for(int ixs=0; ixs<fNXsec; ++ixs) lXSecs[ien*fNXsec+ixs]*=xnorm;
...
bool TPXsec::SetPartXS(const float xsec[], const int dict[]) {
   ...
   for(int i=0; i<TPartIndex::I()->NProc(); ++i) fRdict[i]=fRmap[i]=-1;
   ...
   for(int i=0; i<fNEbins; ++i) {
      fTotXs[i]=0;
      for(int j=0; j<fNXsec; ++j) fTotXs[i]+=fXSecs[i*fNXsec+j];
      if(fTotXs[i]) 
	 for(int j=0; j<fNXsec; ++j) fXSecs[i*fNXsec+j]/=fTotXs[i];
   }
   return kTRUE;
}
...
//___________________________________________________________________
bool TPXsec::SetPartMS(const float angle[], const float ansig[],
			 const float length[], const float lensig[]) {
   fNCbins = fNEbins;
...
//_________________________________________________________________________
bool TPXsec::MS(double en, float &ang, float &asig, 
		  float &len, float &lsig) const {
   if(!fNCbins) {
      ang = asig = len = lsig = 0;
      return kFALSE;
   }
