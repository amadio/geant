...
// Load elements from geometry
  ...
  while ((mat = (TGeoMaterial *)next())) {
    ...
    for (Int_t iel = 0; iel < nelem; ++iel) {
      ...
      if (zd < 1 || zd > NELEM) {
        Fatal("TTabPhysMgr", "In material %s found element with z=%d > NELEM=%d", mat->GetName(), (Int_t)zd, NELEM);
      }
...
//______________________________________________________________________________
void TTabPhysMgr::TransformLF(Int_t /*indref*/, GeantTrack_v & /*tracks*/, Int_t /*nproducts*/, Int_t /*indprod*/,
                              GeantTrack_v & /*output*/) {
...
//______________________________________________________________________________
Int_t TTabPhysMgr::SampleFinalStates(Int_t imat, Int_t ntracks, GeantTrack_v &tracks, GeantTaskData *td) {
  ...
  for (Int_t t = 0; t < ntracks; ++t) {
    ...
    if (nSecPart) {
      ...
      Double_t corFactor =
          Math::Sqrt(curPrimEkin * (curPrimEkin + 2.0 * primMass) / (orgPrimEkin * (orgPrimEkin + 2.0 * primMass)));
...
/*
//______________________________________________________________________________
Int_t TTabPhysMgr::SampleInt(Int_t imat, Int_t ntracks, GeantTrack_v &tracks, GeantTaskData *td)
{
...
GEANT_CUDA_DEVICE_CODE
void TTabPhysMgr::GetRestFinStates(Int_t partindex, TMXsec *mxs, Double_t energyLimit, GeantTrack_v &tracks,
                                   Int_t iintrack, Int_t &nTotSecPart, GeantTaskData *td) {
...
//______________________________________________________________________________
void TTabPhysMgr::SampleDecayInFlight(Int_t partindex, TMXsec *mxs, Double_t energyLimit, GeantTrack_v &tracks,
                                      Int_t iintrack, Int_t &nTotSecPart, GeantTaskData *td) {
  Int_t nSecPart = 0;     // number of secondary particles per reaction
  const Int_t *pid = 0;   // GeantV particle codes [nSecPart]
  const Float_t *mom = 0; // momentum vectors the secondaries [3*nSecPart]
...
void TTabPhysMgr::RotateNewTrack(Double_t oldXdir, Double_t oldYdir, Double_t oldZdir, GeantTrack &track) {
  ...
  Double_t delta = one5 - half * (track.fXdir * track.fXdir + track.fYdir * track.fYdir + track.fZdir * track.fZdir);
  track.fXdir *= delta;
  track.fYdir *= delta;
  track.fZdir *= delta;
}
...
// already updated in GeantTrack track; here we rotate it to lab. frame
GEANT_CUDA_DEVICE_CODE
void TTabPhysMgr::RotateNewTrack(Double_t oldXdir, Double_t oldYdir, Double_t oldZdir, GeantTrack_v &tracks,
                                 Int_t itrack) {
  const Double_t one = 1.0;
  const Double_t zero = 0.0;
  const Double_t amin = 1.0e-10;
...
// phi are the scattering angles measured form the particle local Z
GEANT_CUDA_DEVICE_CODE
void TTabPhysMgr::RotateTrack(GeantTrack_v &tracks, Int_t itrack, Double_t theta, Double_t phi) {
  const Double_t one = 1.0;
  const Double_t zero = 0.0;
  const Double_t amin = 1.0e-10;
  const Double_t one5 = 1.5;
  ...
  Double_t sinTheta0 =
      Math::Sqrt(tracks.fXdirV[itrack] * tracks.fXdirV[itrack] + tracks.fYdirV[itrack] * tracks.fYdirV[itrack]);
  Double_t cosPhi0;
  Double_t sinPhi0;
  Double_t cosTheta = Math::Cos(theta);
  Double_t sinTheta = Math::Sin(theta);
