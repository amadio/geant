...
bool CheckAlign() {
  bool isaligned=true;
  if(((unsigned long) fEGrid) % sizeof(fEGrid[0]) != 0) {std::cout << "TEFstate::fEGrid misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fAtcm3) % sizeof(fAtcm3) != 0) {std::cout << "TEFstate::fAtcm3 misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fEmin) % sizeof(fEmin) != 0) {std::cout << "TEFstate::fEmin misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fEmax) % sizeof(fEmax) != 0) {std::cout << "TEFstate::fEmax misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fEilDelta) % sizeof(fEilDelta) != 0) {std::cout << "TEFstate::fEilDelta misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fDens) % sizeof(fDens) != 0) {std::cout << "TEFstate::fDens misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fEle) % sizeof(fEle) != 0) {std::cout << "TEFstate::fEle misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fNEbins) % sizeof(fNEbins) != 0) {std::cout << "TEFstate::fNEbins misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fNEFstat) % sizeof(fNEFstat) != 0) {std::cout << "TEFstate::fNEFstat misaligned" << std::endl;isaligned=false;}
  if(((unsigned long) &fNRpart) % sizeof(fNRpart) != 0) {std::cout << "TEFstate::fNRpart misaligned" << std::endl;isaligned=false;}
  for(auto i=0; i< fNRpart; ++i)
    if(((unsigned long) fPFstateP[i]) % sizeof(double) != 0) {std::cout << "TEFstate::fPFstateP[" << i << "] misaligned" << std::endl;isaligned=false;}
#ifdef MAGIC_DEBUG
  if(((unsigned long) &fMagic) % sizeof(fMagic) != 0) {std::cout << "TEFstate::fMagic misaligned" << std::endl;isaligned=false;}
#endif
  if(((unsigned long) &fStore) % sizeof(double) != 0) {std::cout << "TEFstate::fStore misaligned" << std::endl;isaligned=false;}
  return isaligned;
}
