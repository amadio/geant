...
public:
  ...
  Int_t ProcIndex(const Char_t *reac) const {
    Int_t nr = fgNProc;
    while (nr--)
  ...
  const Char_t *PartName(Int_t i) const { return TDatabasePDG::Instance()->GetParticle(fPDG[i])->GetName(); }

  // GV particle index <- PDG code
  Int_t PartIndex(Int_t pdg) const;

  // GV particle index <- particle name
  Int_t PartIndex(const Char_t *partname) const { return PartIndex(PDG(partname)); }
  // Number of particles
