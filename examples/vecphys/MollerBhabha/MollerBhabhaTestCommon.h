...
{
  MollerBhabhaIonizationModel *model = new MollerBhabhaIonizationModel(iselecton);
...
VecMollerBhabhaIonizationModel *PrepareVecMBModel(bool useAlias, bool iselectron)
{
  VecMollerBhabhaIonizationModel *model = new VecMollerBhabhaIonizationModel(iselectron);
  model->SetLowEnergyUsageLimit(minEn);
  model->SetHighEnergyUsageLimit(maxEn);
  model->SetUseSamplingTables(useAlias);
  model->GetListActiveRegions().resize(1);
