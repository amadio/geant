...
bool CMSFieldConstruction::CreateFieldAndSolver(bool useRungeKutta, VVectorField **fieldPP)
{
  ...
  int stepperTypeId = 6; // AG: this seems to be DormandPrince, contrary to the description below
  ...
  // AG: This description is very inconsistent with StepperFactory::CreateStepper
  ...
  if (fUseUniformField) {
    ...
    rtv      = geant::UserFieldConstruction::CreateSolverForField<UniformMagField>(gvUniformField);
    ...
    rtv         = geant::UserFieldConstruction::CreateSolverForField<CMSmagField>(fCMSfield, stepperTypeId);
    fieldPtr    = fCMSfield;
  }
