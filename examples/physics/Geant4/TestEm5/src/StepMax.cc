...
StepMax::StepMax(const G4String &processName) : G4VDiscreteProcess(processName), fMaxChargedStep(DBL_MAX), fMess(0)
{
...
G4double StepMax::PostStepGetPhysicalInteractionLength(const G4Track &aTrack, G4double, G4ForceCondition *condition)
{
  ...
  *condition = NotForced;
  ...
  if ((fMaxChargedStep > 0.) && (aTrack.GetVolume() != 0) && (aTrack.GetVolume()->GetName() == "Absorber"))
