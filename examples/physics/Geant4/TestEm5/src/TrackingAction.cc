...
void TrackingAction::PostUserTrackingAction(const G4Track *aTrack)
{
  ...
  G4bool transmit =
      (((fDirX >= 0.0 && position.x() >= fXendAbs) || (fDirX < 0.0 && position.x() <= fXstartAbs)) && energy > 0.0);
  G4bool reflect =
      (((fDirX >= 0.0 && position.x() <= fXstartAbs) || (fDirX < 0.0 && position.x() <= fXendAbs)) && energy > 0.0);
  ...
  if (notabsor) {
    ...
    if ((aTrack->GetDefinition() == G4Positron::Positron()) && (trackID > 1)) eleak += 2 * electron_mass_c2;
