...
Run::Run(DetectorConstruction *det)
    : G4Run(), fDetector(det), fParticle(0), fEkin(0.), fChargedStep(0), fNeutralStep(0), fN_gamma(0), fN_elec(0),
      fN_pos(0), fApplyLimit(false)
{
...
void Run::EndOfRun()
{
  ...
  for (G4int k = 1; k <= fDetector->GetNbOfAbsor(); k++) {
    ...
    // print
    //
    G4cout << std::setw(14) << fDetector->GetAbsorMaterial(k)->GetName() << ": " << std::setprecision(6) << std::setw(6)
           << G4BestUnit(MeanEAbs, "Energy") << " :  " << std::setprecision(6) << std::setw(5)
           << G4BestUnit(rmsEAbs, "Energy") << std::setw(10) << resolution << " +- " << std::setw(5) << rmsres << " %"
           << std::setprecision(6) << std::setw(10) << G4BestUnit(MeanLAbs, "Length") << " +- " << std::setw(4)
           << G4BestUnit(rmsLAbs, "Length") << G4endl;
  }
  G4cout << "\n------------------------------------------------------------\n";

  G4cout << std::setprecision(14);
  G4cout << " Beam particle " << fParticle->GetParticleName() << "  E = " << G4BestUnit(beamEnergy, "Energy") << G4endl;
  ...
  for (G4int j = 1; j <= fDetector->GetNbOfAbsor(); j++) {
    if (fLimittrue[j] < DBL_MAX) {
      ...
      acc.EmAcceptanceGauss("Erms" + mat, nEvt, rmsEAbs, fRmstrue[j], fRmstrue[j], 2.0 * fLimittrue[j]);
    }
  }
