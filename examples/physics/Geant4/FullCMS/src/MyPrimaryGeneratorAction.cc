...
const G4String MyPrimaryGeneratorAction::gNameParticlesVector[MyPrimaryGeneratorAction::gNumberCandidateParticles] = {
    "e-", "e+", "gamma"};
const std::map<G4String, G4int> MyPrimaryGeneratorAction::gPrimaryNameToIndexMap = {{"e-", 0}, {"e+", 1}, {"gamma", 2}};
  ...
  for (G4int i = 0; i < fNumPrimaryPerEvt; ++i) {
    ...
    if (!fIsUserPrimaryType) {
      G4int caseBeamParticle =
          static_cast<G4int>(G4UniformRand() * MyPrimaryGeneratorAction::gNumberCandidateParticles);
    ...
    if (!pDef) {
      G4cerr << " *** ERROR in MyPrimaryGeneratorAction::GeneratePrimaries() " << G4endl
             << "       UNKNOWN PRIMARY PARTICLE WITH NAME = " << fPrimaryParticleName << G4endl << G4endl;
      exit(-1);
    }
...
void MyPrimaryGeneratorAction::Print()
{
  ...
  if (gPrimaryDir.mag() == 0.) {
    ...
    G4String sdir = "[";
    sdir += std::to_string(gPrimaryDir.x()) + ", " + std::to_string(gPrimaryDir.y()) + ", " +
