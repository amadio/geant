...
void MyRun::FillPerEvent(const MyEventDataPerPrimary &data, G4int primtypeindx)
{
  ...
  runData.fEdep += data.fEdep;
  runData.fEdep2 += data.fEdep * data.fEdep;
  runData.fTrackLCh += data.fTrackLCh;
  runData.fTrackLCh2 += data.fTrackLCh * data.fTrackLCh;
  runData.fTrackLNe += data.fTrackLNe;
  runData.fTrackLNe2 += data.fTrackLNe * data.fTrackLNe;
  runData.fChargedStep += data.fChargedStep;
  runData.fChargedStep2 += data.fChargedStep * data.fChargedStep;
  runData.fNeutralStep += data.fNeutralStep;
  runData.fNeutralStep2 += data.fNeutralStep * data.fNeutralStep;
  runData.fNGamma += data.fNGamma;
  runData.fNGamma2 += data.fNGamma * data.fNGamma;
  runData.fNElec += data.fNElec;
  runData.fNElec2 += data.fNElec * data.fNElec;
  runData.fNPosit += data.fNPosit;
  runData.fNPosit2 += data.fNPosit * data.fNPosit;
}
...
void MyRun::EndOfRun()
{
  ...
  for (G4int ipt = 0; ipt < fNumPrimaryTypes; ++ipt) {
    ...
    G4cout << "  Total energy deposit per primary = " << meanEdep / GeV << " +- " << rmsEdep / GeV << " [GeV]"
           << G4endl;
    G4cout << G4endl;
    G4cout << "  Total track length (charged) per primary = " << meanLCh / cm << " +- " << rmsLCh / cm << " [cm]"
           << G4endl;
    G4cout << "  Total track length (neutral) per primary = " << meanLNe / cm << " +- " << rmsLNe / cm << " [cm]"
           << G4endl;
    G4cout << G4endl;
    ...
    G4cout << G4endl;
    G4cout << "  Number of secondaries per primary : " << G4endl << "     Gammas    =  " << meanNGam << " +- "
           << rmsNGam << G4endl << "     Electrons =  " << meanNElec << " +- " << rmsNElec << G4endl
