 ...
 *  which counts instructions only within a certain callstack - subtree using the Intel PIN API
 *
 *  author: Sandro Wenzel, adapted from original opcodemix.cpp example
 */
...
KNOB<string> KnobEntryFunction(KNOB_MODE_WRITEONCE,      "pintool",
    "f", "0", "specify a function(symbol) name");
...
{
  std::string routinename = KnobEntryFunction.Value();

  std::string name = RTN_Name(rtn);
  std::string undecodedname = PIN_UndecorateSymbolName ( name, UNDECORATION_NAME_ONLY );

  if (undecodedname.compare( routinename ) == 0) 
    {
      std::cerr << " opcode mix constraint to " << PIN_UndecorateSymbolName ( name, UNDECORATION_COMPLETE ) << std::endl;
      RTN_Open(rtn);
      // Insert a call at the entry point of a routine to increment the call count
      RTN_InsertCall(rtn, IPOINT_BEFORE, (AFUNPTR)enableCounts, IARG_END);
      RTN_InsertCall(rtn, IPOINT_AFTER, (AFUNPTR)disableCounts, IARG_END);
      RTN_Close(rtn);
    }
}
