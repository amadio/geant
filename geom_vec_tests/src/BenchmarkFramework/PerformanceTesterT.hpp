...
void ShapeBenchmarker<T>::timeContains(double & Tacc, unsigned int vecsize)
{
  ...
  if(vecsize==1)
    {
      timer.Start();
      isin=testshape->T::Contains( &points_C[3*startindex] );
      timer.Stop();
    }
  else
    {
	...
	{
	  isin=testshape->T::Contains( &points_C[3*index] );
	}
      timer.Stop();
    }
...
void ShapeBenchmarker<T>::timeDistanceFromInside(double & Tacc, unsigned int vecsize)
{
  ...
  if(vecsize==1)
    {
      timer.Start();
      distance=testshape->T::DistFromInside( &points_dI[3*startindex], &dirs_dI[3*startindex], 3, TGeoShape::Big(), 0);
      timer.Stop();
    }
  else
    {
      ...
      for(unsigned int index=startindex; index < startindex+vecsize; ++index)
	{
	  distance=testshape->T::DistFromInside( &points_dI[index], &dirs_dI[3*index], 3, TGeoShape::Big(), 0);
	}
      timer.Stop();
    }
  Tacc+=timer.getDeltaSecs();
...
void ShapeBenchmarker<T>::timeDistanceFromOutside(double & Tacc, unsigned int vecsize)
{
  ...
  if(vecsize==1)
    {
      timer.Start();
      distance=testshape->T::DistFromOutside( &points_dO[3*startindex], &dirs_dO[3*startindex], 3, TGeoShape::Big(), 0);
      timer.Stop();
    }
  else
    {
      ...
      for(unsigned int index=startindex; index < startindex+vecsize; ++index)
	{
	  distance=testshape->T::DistFromOutside( &points_dO[3*index], &dirs_dO[3*index], 3, TGeoShape::Big(), 0);
	}
      timer.Stop();
    }
  Tacc+=timer.getDeltaSecs();
