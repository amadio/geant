...
#pragma novector
        for (int i = 0; i < npoints; ++i) {
          distance_v[i] =
              TGeoBBox_v::DistFromInsideS(&points[3 * i], &dir[3 * i], dx, dy, dz, origin, TGeoShape::Big());
        ...
        if (checksum_v != checksum || checksum_box != checksum)
          ;
        {
          //  std::cerr << "#" << checksum_box << " " << checksum_v <<  " " << checksum << " " <<  checksum_l <<
          //  std::endl;
        }
      }
...
#pragma novector
    ...
    std::cerr << "#P " << npoints << " " << DeltaT / NREP << " " << DeltaT_l / NREP << " " << DeltaT_v / NREP << " "
              << DeltaT / DeltaT_l << " " << DeltaT / DeltaT_v << " " << DeltaT_b / DeltaT_v << std::endl;
