...
if [ $# -ge 5 ]; then
  ...
  echo "$0: expecting 5 arguments [LABEL]  [COMPILER] [BUILDTYPE] [EXTERNALS] [WORKSPACE]"
  return
fi
...
then
  ...
  export CTEST_BUILD_OPTIONS="-DUSE_VECGEOM_NAVIGATOR=ON '-DCMAKE_CXX_FLAGS=-O2 -std=c++11' -DUSE_ROOT=ON -DCTEST=ON "
