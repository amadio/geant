...
#define ENABLE_IF(preamble, condition, return_type) \
  ...
  preamble typename std::enable_if<sizeof(Bogus) && condition, return_type>::type
...
public:
  ...
  virtual const char *what() const noexcept { return "Expansion beyond maximum hashpower"; }
...
template <class F>
static void parallel_exec(size_t start, size_t end, size_t num_threads, F func)
{
