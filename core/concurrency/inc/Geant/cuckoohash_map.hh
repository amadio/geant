...
template <class Key, class T, class Hash = DefaultHasher<Key>, class Pred = std::equal_to<Key>,
          class Alloc = std::allocator<std::pair<const Key, T>>, size_t SLOT_PER_BUCKET = DEFAULT_SLOT_PER_BUCKET>
...
public:
  ...
  private:
    ...
    reference(cuckoohash_map<Key, T, Hash, Pred, Alloc, slot_per_bucket> &owner, const key_type &key)
...
private:
  ...
  {
    static size_t cores =
        std::thread::hardware_concurrency() == 0 ? sysconf(_SC_NPROCESSORS_ONLN) : std::thread::hardware_concurrency();
    return cores;
  }
  ...
  public:
    ...
    {
      return *static_cast<const value_type *>(static_cast<const void *>(&kvpairs_[ind]));
    }

    value_type &kvpair(size_t ind) { return *static_cast<value_type *>(static_cast<void *>(&kvpairs_[ind])); }

    storage_value_type &storage_kvpair(size_t ind)
    {
      return *static_cast<storage_value_type *>(static_cast<void *>(&kvpairs_[ind]));
    }
    ...
    void setKV(size_t ind, K &&key, Args &&... args)
    {
      ...
      occupied_[ind] = true;
      pair_allocator.construct(&storage_kvpair(ind), std::piecewise_construct,
    ...
    static void move_to_bucket(Bucket &b1, size_t slot1, Bucket &b2, size_t slot2)
    {
  ...
  typedef std::vector<Bucket, typename allocator_type::template rebind<Bucket>::other> buckets_t;
  ...
  static_assert(LOCK_ARRAY_GRANULARITY >= 0 && LOCK_ARRAY_GRANULARITY <= 16,
                ...
                " inclusive");
  typedef lazy_array<16 - LOCK_ARRAY_GRANULARITY, LOCK_ARRAY_GRANULARITY, spinlock,
                     typename allocator_type::template rebind<spinlock>::other>
      locks_t;
  ...
  // barriers
  size_t get_hashpower() const { return hashpower_.load(std::memory_order_acquire); }

  void set_hashpower(size_t val) { hashpower_.store(val, std::memory_order_release); }
...
public:
   ...
   */
  cuckoohash_map(size_t n = DEFAULT_SIZE, double mlf = DEFAULT_MINIMUM_LOAD_FACTOR, size_t mhp = NO_MAXIMUM_HASHPOWER,
      ...
      : hash_fn(hf), eq_fn(eql)
  {
    ...
    if (mhp != NO_MAXIMUM_HASHPOWER && hp > mhp) {
      throw std::invalid_argument("hashpower for initial size " + std::to_string(hp) +
  ...
  double load_factor() const noexcept { return cuckoo_loadfactor(get_hashpower()); }
  ...
  {
    if (mlf < 0.0) {
      throw std::invalid_argument("load factor " + std::to_string(mlf) + " cannot be "
                                                                         " less than 0");
    } else if (mlf > 1.0) {
      throw std::invalid_argument("load factor " + std::to_string(mlf) + " cannot be "
                                                                         " greater than 1");
   ...
   */
  double minimum_load_factor() noexcept { return minimum_load_factor_.load(std::memory_order_acquire); }
   ...
   */
  void maximum_hashpower(size_t mhp) noexcept { maximum_hashpower_.store(mhp, std::memory_order_release); }

  /**
   * @return the maximum hashpower of the table
   */
  size_t maximum_hashpower() noexcept { return maximum_hashpower_.load(std::memory_order_acquire); }
  ...
  {
    return cuckoo_insert_loop(hashed_key(key), std::forward<K>(key), std::forward<Args>(val)...);
  }
  ...
  bool update(const key_type &key, V &&val)
  {
    size_t hv              = hashed_key(key);
    auto b                 = snapshot_and_lock_two(hv);
    const cuckoo_status st = cuckoo_update(hv, b.i[0], b.i[1], key, std::forward<V>(val));
    return (st == ok);
  }
  ...
  typename std::enable_if<std::is_convertible<Updater, updater_type>::value, bool>::type update_fn(const key_type &key,
                                                                                                   Updater fn)
  {
    size_t hv              = hashed_key(key);
    auto b                 = snapshot_and_lock_two(hv);
  ...
  typename std::enable_if<std::is_convertible<Updater, updater_type>::value, void>::type upsert(K &&key, Updater fn,
                                                                                                Args &&... val)
  {
    size_t hv = hashed_key(key);
    cuckoo_status st;
    do {
      ...
      st = cuckoo_insert(hv, std::move(b), std::forward<K>(key), std::forward<Args>(val)...);
      if (st == failure_table_full) {
        cuckoo_fast_double(hp);
        // Retry until the insert doesn't fail due to expansion.
        if (cuckoo_insert_loop(hv, std::forward<K>(key), std::forward<Args>(val)...)) {
          break;
        }
  ...
  hasher hash_function() const noexcept { return hash_fn; }

  //! key_eq returns the equality predicate object used by the table.
  key_equal key_eq() const noexcept { return eq_fn; }
...
private:
  ...
  struct BucketContainer {
    ...
    BucketContainer(const cuckoohash_map *_map, Args &&... inds) : map(_map), i{{inds...}}
    {
    }

    BucketContainer(const cuckoohash_map *_map, std::array<size_t, N> _i) : map(_map), i(_i) {}
    ...
    BucketContainer(BucketContainer &&bp) { *this = std::move(bp); }
    ...
    bool is_active() const { return map != nullptr; }
  ...
  private:
    ...
    void unlock(std::array<size_t, 1> inds) const { map->locks_[lock_ind(inds[0])].unlock(); }
  ...
  //
  // throws hashpower_changed if it changed after taking the lock.
  TwoBuckets lock_two(const size_t hp, const size_t i1, const size_t i2) const
  {
  ...
  //
  // throws hashpower_changed if it changed after taking the lock.
  std::pair<TwoBuckets, OneBucket> lock_three(const size_t hp, const size_t i1, const size_t i2, const size_t i3) const
  {
    std::array<size_t, 3> l{{lock_ind(i1), lock_ind(i2), lock_ind(i3)}};
    ...
    return std::make_pair(
        ...
        OneBucket{(lock_ind(i3) == lock_ind(i1) || lock_ind(i3) == lock_ind(i2)) ? nullptr : this, i3});
  }
  ...
  // hashpower.
  TwoBuckets snapshot_and_lock_two(const size_t hv) const noexcept
  {
    while (true) {
  ...
  static inline size_t lock_ind(const size_t bucket_ind) { return bucket_ind & (kNumLocks - 1); }

  // hashsize returns the number of buckets corresponding to a given
  // hashpower.
  static inline size_t hashsize(const size_t hp) { return 1UL << hp; }
  ...
  inline size_t hashed_key(const key_type &key) const { return hash_function()(key); }

  // index_hash returns the first possible bucket that the given hashed key
  // could be.
  static inline size_t index_hash(const size_t hp, const size_t hv) { return hv & hashmask(hp); }
  ...
  static inline size_t alt_index(const size_t hp, const partial_t partial, const size_t index)
  {
  ...
  static constexpr size_t const_pow(size_t a, size_t b) { return (b == 0) ? 1 : a * const_pow(a, b - 1); }
  ...
  struct b_slot {
    ...
    b_slot() {}
    b_slot(const size_t b, const size_t p, const decltype(depth) d) : bucket(b), pathcode(p), depth(d)
    {
  ...
  //
  // throws hashpower_changed if it changed during the search
  int cuckoopath_search(const size_t hp, CuckooRecords &cuckoo_path, const size_t i1, const size_t i2)
  {
    ...
    for (int i = 1; i <= x.depth; ++i) {
      ...
      assert(prev.bucket == index_hash(hp, prev.hv) ||
             prev.bucket == alt_index(hp, partial_key(prev.hv), index_hash(hp, prev.hv)));
  ...
  bool cuckoopath_move(const size_t hp, CuckooRecords &cuckoo_path, size_t depth, TwoBuckets &b)
  {
    assert(!b.is_active());
    if (depth == 0) {
    ...
    while (depth > 0) {
      ...
      if (depth == 1) {
        ...
        std::tie(twob, extrab) = lock_three(hp, b.i[0], b.i[1], to.bucket);
      } else {
      ...
      if (hashed_key(fb.key(fs)) != from.hv || tb.occupied(ts) || !fb.occupied(fs)) {
        return false;
      }
  ...
  cuckoo_status run_cuckoo(TwoBuckets &b, size_t &insert_bucket, size_t &insert_slot)
  {
  ...
  bool try_read_from_bucket(const partial_t partial, const key_type &key, mapped_type &val, const Bucket &b) const
  {
    for (size_t i = 0; i < slot_per_bucket; ++i) {
      if (!b.occupied(i)) {
        continue;
  ...
  bool check_in_bucket(const partial_t partial, const key_type &key, const Bucket &b) const
  {
    for (size_t i = 0; i < slot_per_bucket; ++i) {
      if (!b.occupied(i)) {
        continue;
  ...
  // for use afterwards.
  template <typename K, typename... Args>
  void add_to_bucket(const partial_t partial, Bucket &b, const size_t slot, K &&key, Args &&... val)
  {
  ...
  bool try_find_insert_bucket(const partial_t partial, const key_type &key, const Bucket &b, int &slot) const
  {
    slot             = -1;
  ...
  bool try_del_from_bucket(const partial_t partial, const key_type &key, Bucket &b)
  {
    for (size_t i = 0; i < slot_per_bucket; ++i) {
      if (!b.occupied(i)) {
        continue;
      ...
      if (key_eq()(b.key(i), key)) {
        b.eraseKV(i);
        num_deletes_[get_counterid()].num.fetch_add(1, std::memory_order_relaxed);
        return true;
      }
    }
    return false;
  ...
  template <typename V>
  bool try_update_bucket(const partial_t partial, Bucket &b, const key_type &key, V &&val)
  {
    for (size_t i = 0; i < slot_per_bucket; ++i) {
      if (!b.occupied(i)) {
        continue;
  ...
  template <typename Updater>
  bool try_update_bucket_fn(const partial_t partial, const key_type &key, Updater fn, Bucket &b)
  {
    for (size_t i = 0; i < slot_per_bucket; ++i) {
      if (!b.occupied(i)) {
        continue;
  ...
  cuckoo_status cuckoo_find(const key_type &key, mapped_type &val, const size_t hv, const size_t i1,
  ...
  // and released outside the function.
  bool cuckoo_contains(const key_type &key, const size_t hv, const size_t i1, const size_t i2) const
  {
    const partial_t partial = partial_key(hv);
  ...
  template <typename K, typename... Args>
  cuckoo_status cuckoo_insert(const size_t hv, TwoBuckets b, K &&key, Args &&... val)
  {
    int res1, res2;
    const partial_t partial = partial_key(hv);
    ...
    if (res1 != -1) {
      add_to_bucket(partial, b0, res1, std::forward<K>(key), std::forward<Args>(val)...);
      return ok;
    }
    if (res2 != -1) {
      add_to_bucket(partial, b1, res2, std::forward<K>(key), std::forward<Args>(val)...);
      return ok;
    }
    ...
    if (st == failure_under_expansion) {
      ...
      assert(insert_bucket == index_hash(get_hashpower(), hv) ||
             insert_bucket == alt_index(get_hashpower(), partial, index_hash(get_hashpower(), hv)));
      ...
      add_to_bucket(partial, buckets_[insert_bucket], insert_slot, std::forward<K>(key), std::forward<Args>(val)...);
      return ok;
    }
    assert(st == failure);
  ...
  cuckoo_status cuckoo_delete(const key_type &key, const size_t hv, const size_t i1, const size_t i2)
  {
    const partial_t partial = partial_key(hv);
  ...
  // function.
  template <typename V>
  cuckoo_status cuckoo_update(const size_t hv, const size_t i1, const size_t i2, const key_type &key, V &&val)
  {
    const partial_t partial = partial_key(hv);
    if (try_update_bucket(partial, buckets_[i1], key, std::forward<V>(val))) {
      return ok;
    }
    if (try_update_bucket(partial, buckets_[i2], key, std::forward<V>(val))) {
      return ok;
    }
    return failure_key_not_found;
  }
  ...
  template <typename Updater>
  cuckoo_status cuckoo_update_fn(const key_type &key, Updater fn, const size_t hv, const size_t i1, const size_t i2)
  {
    const partial_t partial = partial_key(hv);
  ...
  {
    return (static_cast<double>(cuckoo_size()) / slot_per_bucket / hashsize(hp));
  }

  void move_buckets(size_t current_hp, size_t new_hp, size_t start_lock_ind, size_t end_lock_ind)
  {
    for (; start_lock_ind < end_lock_ind; ++start_lock_ind) {
      for (size_t bucket_i = start_lock_ind; bucket_i < hashsize(current_hp); bucket_i += locks_t::size()) {
        ...
        for (size_t slot = 0; slot < slot_per_bucket; ++slot) {
          ...
          const size_t old_ahash = alt_index(current_hp, old_bucket.partial(slot), old_ihash);
          const size_t new_ihash = index_hash(new_hp, hv);
          const size_t new_ahash = alt_index(new_hp, old_bucket.partial(slot), new_ihash);
          if ((bucket_i == old_ihash && new_ihash == new_bucket_i) ||
            ...
            // one
            Bucket::move_to_bucket(old_bucket, slot, new_bucket, new_bucket_slot++);
          } else {
  ...
  cuckoo_status cuckoo_fast_double(size_t current_hp)
  {
    ...
    // before we want them.
    const size_t locks_to_move = std::min(locks_t::size(), hashsize(current_hp));
    parallel_exec(0, locks_to_move, kNumCores(), [this, current_hp, new_hp](size_t start, size_t end) {
      move_buckets(current_hp, new_hp, start, end);
    });
    parallel_exec(locks_to_move, locks_.allocated_size(), kNumCores(), [this](size_t i, size_t end) {
      for (; i < end; ++i) {
        locks_[i].unlock();
      }
    });
  ...
  // new one.
  static void insert_into_table(cuckoohash_map<Key, T, Hash, Pred, Alloc, slot_per_bucket> &new_map, buckets_t &buckets,
                                size_t i, size_t end)
  {
    for (; i < end; ++i) {
  ...
  cuckoo_status cuckoo_expand_simple(size_t new_hp, bool is_expansion)
  {
    ...
    cuckoohash_map<Key, T, Hash, Pred, Alloc, slot_per_bucket> new_map(hashsize(new_hp) * slot_per_bucket);
    const size_t threadnum          = kNumCores();
    const size_t buckets_per_thread = ((hashsize(hp) + threadnum - 1) / threadnum);
    std::vector<std::thread> insertion_threads(threadnum);
    for (size_t i = 0; i < threadnum; ++i) {
      insertion_threads[i] = std::thread(insert_into_table, std::ref(new_map), std::ref(buckets_),
                                         i * buckets_per_thread, std::min((i + 1) * buckets_per_thread, hashsize(hp)));
    }
    for (size_t i = 0; i < threadnum; ++i) {
...
public:
  ...
  class locked_table {
    ...
    locked_table(cuckoohash_map<Key, T, Hash, Pred, Alloc, SLOT_PER_BUCKET> &hm)
        : unlocker_(std::move(hm.snapshot_and_lock_all())), buckets_(hm.buckets_), has_table_lock_(new bool(true))
    {
    }

  public:
    locked_table(locked_table &&lt)
        : unlocker_(std::move(lt.unlocker_)), buckets_(std::move(lt.buckets_)),
    ...
    bool has_table_lock() const noexcept { return has_table_lock_ && *has_table_lock_; }
  ...
  private:
    ...
    class templated_iterator : public std::iterator<std::bidirectional_iterator_tag, value_type> {

      // The buckets locked and owned by the locked table being iterated
      // over.
      std::reference_wrapper<typename std::conditional<IS_CONST, const buckets_t, buckets_t>::type> buckets_;
    ...
    public:
      ...
      bool operator==(const templated_iterator<OTHER_CONST> &it) const noexcept
      {
        return (*has_table_lock_ && *it.has_table_lock_ && &buckets_.get() == &it.buckets_.get() &&
    ...
    private:
      static const std::pair<intmax_t, intmax_t> end_pos(const buckets_t &buckets)
      {
      ...
      templated_iterator(typename decltype(buckets_)::type &buckets, std::shared_ptr<bool> has_table_lock, size_t index,
                         size_t slot)
          : buckets_(buckets), has_table_lock_(has_table_lock), index_(index), slot_(slot)
      {
        if (std::make_pair(index_, slot_) != end_pos(buckets) && !buckets[index_].occupied(slot_)) {
          operator++();
        }
      }
  ...
  public:
    ...
    iterator end()
    {
      ...
      return iterator(buckets_.get(), has_table_lock_, end_pos.first, end_pos.second);
    }
    ...
    const_iterator end() const
    {
      check_table();
      const auto end_pos = const_iterator::end_pos(buckets_.get());
      return const_iterator(buckets_.get(), has_table_lock_, end_pos.first, end_pos.second);
    }
...
private:
  ...
  std::vector<cacheint, typename allocator_type::template rebind<cacheint>::other> num_inserts_, num_deletes_;
