...
template <uint8_t OFFSET_BITS, uint8_t SEGMENT_BITS, class T, class Alloc = std::allocator<T>>
class lazy_array {
  static_assert(SEGMENT_BITS + OFFSET_BITS <= sizeof(size_t) * 8, "The number of segment and offset bits cannot exceed "
...
private:
  ...
  {
    clear();
    std::copy(arr.segments_.begin(), arr.segments_.end(), segments_.begin());
    std::fill(arr.segments_.begin(), arr.segments_.end(), nullptr);
  }

  inline size_t get_segment(size_t i) const { return i >> OFFSET_BITS; }

  static const size_t OFFSET_MASK = ((1UL << OFFSET_BITS) - 1);
  inline size_t get_offset(size_t i) const { return i & OFFSET_MASK; }
...
public:
  ...
  // Moving is allowed
  lazy_array(lazy_array &&arr) : segments_{{nullptr}} { move_other_array(std::move(arr)); }
  ...
  size_t allocated_size() const
  {
    ...
    for (; (num_allocated_segments < NUM_SEGMENTS && segments_[num_allocated_segments] != nullptr);
         ++num_allocated_segments) {
    }
    return num_allocated_segments * SEGMENT_SIZE;
  }

  static constexpr size_t size() { return 1UL << (OFFSET_BITS + SEGMENT_BITS); }
};
