...
NumaUtilsStruct::NumaUtilsStruct()
{
  ...
  hwloc_bitmap_foreach_begin(i, set)
  {
    ...
    std::cout << "  node #" << obj->logical_index << " (OS index " << i << ") with " << obj->memory.local_memory
              << " bytes of memory\n";
  }
  ...
  while ((obj = hwloc_get_next_obj_by_type(fTopology, HWLOC_OBJ_NUMANODE, obj)) != nullptr) {
    ...
    buffer =
        (char *)hwloc_alloc_membind_nodeset(fTopology, 4096, obj->nodeset, HWLOC_MEMBIND_BIND, HWLOC_MEMBIND_STRICT);
    if (!buffer) {
     ...
     */
    std::cout << "***   buffer membind policy is " << policy << " while we requested " << HWLOC_MEMBIND_REPLICATE
              << " or " << HWLOC_MEMBIND_BIND << std::endl;
...
NumaUtilsStruct::~NumaUtilsStruct()
{
  ...
  if (fAvailable) hwloc_topology_destroy(fTopology);
}
...
void *NumaUtilsStruct::NumaAlignedMalloc(size_t bytes, int node, size_t alignment)
{
  ...
  */
  ...
  p1 = hwloc_alloc_membind_nodeset(fTopology, bytes + alignment + twosize, obj->nodeset, HWLOC_MEMBIND_BIND,
