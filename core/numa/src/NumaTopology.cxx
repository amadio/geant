...
{
#ifdef GEANT_USE_NUMA
  os << "NUMA nodes: " << topo.fNodes << "   cores: " << topo.fNcores << "   cpus: " << topo.fNcpus
     << "   hyperthreading: " << topo.fHT << "   page size: " << topo.fPageSize << std::endl;
  for (auto i = 0; i < topo.fNodes; ++i)
    os << *(topo.fListNodes[i]) << std::endl;
#else
