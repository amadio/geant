...
private:
  /** @brief Constructor */
  NumaBlock(size_t block_size, int node, int id)
      : fCurrent(0), fUsed(0), fSize(block_size), fId(id), fNode(node), fAddress(this)
  {
    ...
    for (size_t i = 0; i < block_size; ++i)
      T::MakeInstanceAt((char *)&fArray[0] + i * el_size);
...
public:
  ...
  GEANT_FORCE_INLINE bool ReleaseObject()
  {
    ...
    if ((nused > 0) || (fCurrent.load() < fSize)) return false;
    return true;
  }

  /** @brief Release an object to the container */
  GEANT_FORCE_INLINE void Clear()
  {
    fCurrent.store(0);
    fUsed.store(0);
  }
