                                                      ...
                                                      double *step, double *safe, bool *isonbdr)
{
  ...
  for (int itr = 0; itr < ntracks; ++itr) {
    ...
    nav.FindNextBoundaryAndStep(Vector3D_t(x[itr], y[itr], z[itr]), Vector3D_t(dirx[itr], diry[itr], dirz[itr]),
                                *instate[itr], *outstate[itr], Math::Min<double>(1.E20, pstep[itr]), step[itr]);
...
//#### To add small step detection and correction - see ScalarNavInterfaceTGeo ####//
    ...
    if (Math::Abs(step[itr] - stepcmp) > 1E-6) {
      ...
      geant::Print("", "## PSTEP %lf ONBOUND %d VECGEOMSAFETY %lf ROOTSAFETY %lf BRUTEFORCEROOT %lf", pstep[itr],
                   isonbdr[itr], safe[itr], rootnav->Safety());
...
//#### To add small step detection and correction - see ScalarNavInterfaceTGeo ####//
  ...
  if (Math::Abs(track.GetStep() - stepcmp) > 1E-6) {
    ...
    geant::Print("", "## PSTEP %lf ONBOUND %d VECGEOMSAFETY %lf ROOTSAFETY %lf BRUTEFORCEROOT %lf", track.GetPstep(),
                 track.Boundary(), track.GetSafety(), rootnav->Safety());

    // check nextpath
    tmp = track.NextPath()->ToTGeoBranchArray();
    tmp->InitFromNavigator(rootnav);
    geant::Print("", "## VECGEOMNEXTNODE %p ROOTNEXTNODE %p", track.NextPath()->GetCurrentNode(),
                 tmp->GetCurrentNode());
    geant::Print("", "## VECGEOMBOUNDARY %d ROOTBOUNDARY %d", track.NextPath()->IsOnBoundary(),
