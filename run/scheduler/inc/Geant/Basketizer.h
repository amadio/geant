...
public:
  ...
  {
    if (ibook - Ibook() >= fBsize) return false;
  ...
  void Print()
  {
    std::cout << "== ibook = " << Ibook() << " nbook0 = " << Nbook0() << " nbooktot = " << Nbooktot()
              << " nfilled = " << Nfilled() << std::endl;
  }
...
public:
   ...
   */
  Basketizer() : fBsize(0), fBmask(0), fBuffer(0), fCounters(0), fBufferMask(0), fIbook(0), fNstored(0), fNbaskets(0)
  {
#ifndef VECCORE_CUDA_DEVICE_COMPILATION
    fLock.clear();
#endif
  ...
  bool AddElement(T *const data, vector_t<T *> &basket)
  {
    ...
    if (ibasket == ibook_buf) fNbaskets++;
...
// Deploy basket (copy data)
  ...
  void CheckBaskets()
  {
    ...
    std::cout << "ibook = " << fIbook.load() << "  sumbooked = " << nbooktot << "  sumstored = " << nstored
              << "  stillbooked = " << nbooked << std::endl;
    Unlock();
  }
...
// Now fill the objects from the pending basket
  ...
  static VECCORE_ATT_HOST_DEVICE GEANT_FORCE_INLINE size_t SizeofInstance(size_t buffer_size)
  {
  ...
  static VECCORE_ATT_HOST_DEVICE Basketizer<T> *MakeInstanceAt(void *addr, size_t buffer_size, size_t basket_size)
  {
