...
#include <fstream>
//#include <math.h>
// g++ test_mpmc.cc -o test_mpmc -O3 -pthread -std=c++11 -I$HOME/boost/include -I`root-config --incdir` `root-config
// --libs`
...
void performance()
{
  ...
  if (sumwt == sumrt)
    std::cout << "checksum matching: " << sumwt << std::endl;
  else
    std::cout << "checksum NOT matching: " << std::endl;
...
int main(int argc, char *argv[])
{
  ...
  if (lock_free)
    std::cout << "std::atomic is lock free" << std::endl;
  else
    std::cout << "std::atomic is NOT lock free" << std::endl;
  ...
  for (auto icall = 0; icall < NREP; ++icall)
    performance();
  ofs1.close();
  ofs2.close();
  ofs3.close();
