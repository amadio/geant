...
const Int_t NG       = 33;
const Int_t NS       = 15;
const char *exps[NG] = {"aleph", "barres",  "felix",  "phenix", "chambers", "p326", "bes",    "dubna",    "ganil",
                        "e907",  "phobos2", "hermes", "na35",   "na47",     "na49", "wa91",   "sdc",      "integral",
                        "ams",   "brahms",  "gem",    "tesla",  "btev",     "cdf",  "hades2", "lhcbfull", "star",
                        "sld",   "cms",     "alice2", "babar2", "belle",    "atlas"};
const char *shp[NS] = {"box",  "tube", "tubeseg", "cone", "coneseg", "trd1",      "trd2", "para",
                       "pcon", "pgon", "trap",    "gtra", "sphere",  "composite", "xtru"};
void perfshapes();
void countshapes();
...
Double_t performance(TGeoVolume *vol)
{
  ...
  timer.Start(true);
  for (Int_t i = 0; i < ntr; i++)
    shape->DistFromOutside(&vec[6 * i], &vec[6 * i + 3], 3);
  timer.Stop();
  //   timer.Print();
