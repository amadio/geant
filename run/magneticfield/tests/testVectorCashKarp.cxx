...
int main(int argc, char *args[])
{
  ...
  // Field
  auto gvUniformField =
      new UniformMagField(fieldUnits::tesla * ThreeVector_f(x_field, y_field, z_field)); // New classes
  ...
  Double_v yInVec[] = {Double_v(x_pos * mmGVf), Double_v(y_pos * mmGVf), Double_v(z_pos * mmGVf),
                       Double_v(x_mom * ppGVf), Double_v(y_mom * ppGVf), Double_v(z_mom * ppGVf)};
...
// Creating the baseline stepper
  ...
  Double_v dydxVecRef[8] = {0., 0., 0., 0., 0., 0., 0., 0.}, yOutVecX[8] = {0., 0., 0., 0., 0., 0., 0., 0.},
  ...
  for (int j = 0; j < no_of_steps; j++) {
    ...
    {
      myStepper->StepWithErrorEstimate(yInVec, dydxVec, chargeVec, step_len, yOutVec,
                                       yErrVec); // Call the 'trial' stepper
//         *********************
#ifdef BASELINESTEPPER
