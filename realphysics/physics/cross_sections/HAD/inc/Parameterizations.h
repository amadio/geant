...
namespace geantphysics {
  ...
  /// Mandelstam 
  static double CalcMandelstamS( const double mp , 
				 const double mt , 
				 const double Plab )
  {
  ...
  /// Nucleus Radius
  static double GetNucleusRadius(int At)
  {
  ...
  /// Coulomb barrier
  static double GetCoulombBarrier(int particlePDG, double proj_mass, double energyKin,
  ...
  static double GetHadronNucleonXscPDG(int particlePDG, double mass, double energyKin, int targetPDG)
  {
	...
	if ( proton )
	  {
	    NucleonTotalXsc = Zt*( 35.80 + B*std::pow(std::log(sMand/s0),2.) 
			    + 40.15*std::pow(sMand,-eta1) - 30.*std::pow(sMand,-eta2));// on p
	  }
	if ( neutron )
	  {
	    NucleonTotalXsc  = Nt*( 35.45 + B*std::pow(std::log(sMand/s0),2.) 
	...
	if ( proton )
	  {      
	    NucleonTotalXsc  = Zt*( 35.45 + B*std::pow(std::log(sMand/s0),2.) 
			     + 42.53*std::pow(sMand,-eta1) - 33.34*std::pow(sMand,-eta2));
	  }
	if ( neutron )
	  {
	    NucleonTotalXsc = Nt*( 35.80 + B*std::pow(std::log(sMand/s0),2.) 
	...
	if ( neutron )
      ...
      {
	NucleonTotalXsc  = At*( 20.86 + B*std::pow(std::log(sMand/s0),2.) 
	...
	if ( proton )
	  {      
	    NucleonTotalXsc  = Zt*( 17.91 + B*std::pow(std::log(sMand/s0),2.) 
			     + 7.14*std::pow(sMand,-eta1) - 13.45*std::pow(sMand,-eta2));
	  }
	if ( neutron )
	  {
	    NucleonTotalXsc = Nt*( 17.87 + B*std::pow(std::log(sMand/s0),2.) 
	...
	if ( neutron )
      ...
      {
	NucleonTotalXsc  = At*( 35.20 + B*std::pow(std::log(sMand/s0),2.) 
			 - 199.*std::pow(sMand,-eta1) + 264.*std::pow(sMand,-eta2) );
      } 
    else if(particlePDG == 22 && pORn ) // modify later on
      {
	NucleonTotalXsc  = At*( 0.0 + B*std::pow(std::log(sMand/s0),2.) 
  ...
  static double GetHadronNucleonTotalXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
  {
    double NucleonTotalXsc(0); 
  ...
  static double GetKaonNucleonTotalXscGG(int particlePDG, double mass, double energyKin, int targetPDG)
  {
  ...
  static double GetHadronNucleonInelasticXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
  {
  ...
  static double GetKaonNucleonInelasticXscGG(int particlePDG, double mass, double energyKin, int targetPDG)
  {
