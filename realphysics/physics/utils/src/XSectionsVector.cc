...
size_t XSectionsVector::FindCSBinLocation(double energy, size_t idx) const
{
  ...
  if (idx >= fNumberOfNodes || energy < fBinVector[idx] || energy > fBinVector[idx + 1]) {
...
// Binary search algorithm to find the binIndex corresponding to 'energy'and retreiving the corresponding linear
// interpolated value.
...
// Given an energy, first retrieve the binIndex corresponding to that energy and then calculate the interpolated value
// (Linear Interpolation) corresponding to the data stored at that bin index
