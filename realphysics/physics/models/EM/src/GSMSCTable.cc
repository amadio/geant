...
const double GSMSCTable::gG1Values1[] = {
...
const double GSMSCTable::gScrAValues1[] = {
...
const double GSMSCTable::gScrBValues1[] = {
...
const double GSMSCTable::gG1Values2[] = {
...
const double GSMSCTable::gScrAValues2[] = {
...
const double GSMSCTable::gScrBValues2[] = {
...
void GSMSCTable::Initialize() {
  ...
  if (!gIsInitialised) {
    ...
    fLogG1FuncMin1      = std::log(gG1Values1[0]);
    fInvLogDeltaG1Func1 = 1./((std::log(gG1Values1[gNUMSCR1-1]/gG1Values1[0]))/(gNUMSCR1-1.));
    fLogG1FuncMin2      = std::log(gG1Values2[0]);
    fInvLogDeltaG1Func2 = 1./((std::log(gG1Values2[gNUMSCR2-1]/gG1Values2[0]))/(gNUMSCR2-1.));
    LoadMSCData();
  }
...
void GSMSCTable::Sampling(double lambdaval, double qval, double scra, double &cost, double &sint, Geant::GeantTaskData *td){
  ...
  double dum0 = SampleCosTheta(lambdaval, qval, scra, rndArray[0], rndArray[1], rndArray[2]);
  // add protections
...
double GSMSCTable::SampleCosTheta(double lambdaval, double qval, double scra, double rndm1, double rndm2, double rndm3) {
...
// for Q Grid 1
double GSMSCTable::SampleCosTheta1(double lambdaval, double qval, double scra, double rndm1, double rndm2, double rndm3) {
  // determine lower lambda index
  // logspacing in lambdaval => L=s/lambda_el
  double logLambda       = std::log(lambdaval);
  double pLambdaIndxPlus = (logLambda-fLogLambda0)*fInvLogDeltaLambda;
  int    lambdaIndx      = (int)(pLambdaIndxPlus);    // lower index of the lambda bin
  ...
  double pQIndxPlus = (qval-gQMIN1)*fInvDeltaQ1;
  int    qIndx      = (int)(pQIndxPlus);        // lower index of the Q bin
  pQIndxPlus        = pQIndxPlus-qIndx;
  if (rndm2<pQIndxPlus) {
    ++qIndx;
  }
  int indx      = lambdaIndx*gQNUM1+qIndx;
  GSMSCAngularDtr *gsDtr = fGSMSCAngularDistributions1[indx];
  // sampling form the selected distribution
  double delta  = 1.0/(gsDtr->fNumData-1.);
  // determine lower cumulative bin inidex
  int indxl     = rndm3/delta;
  double  aval  = rndm3-indxl*delta;
  ...
  double para = dum0*(lambdaval+4.0)*scra;
...
// for Q Grid 2
double GSMSCTable::SampleCosTheta2(double lambdaval, double qval, double scra, double rndm1, double rndm2, double rndm3) {
  // determine lower lambda index
  // logspacing in lambdaval => L=s/lambda_el
  double logLambda       = std::log(lambdaval);
  double pLambdaIndxPlus = (logLambda-fLogLambda0)*fInvLogDeltaLambda;
  ...
  double logQ            = std::log(qval);
  double lambdaLow       = fLogLambda0+lambdaIndx*fLogDeltaLambda;
  if (lambdaLow<logQ) {
    double min  = (logQ-lambdaLow);
  ...
  if (qval<gQMAX2) {
    double pQIndxPlus = (qval-gQMIN2)*fInvDeltaQ2;
    int    qIndx      = (int)(pQIndxPlus);        // lower index of the Q bin
    pQIndxPlus        = pQIndxPlus-qIndx;
    int indx          = lambdaIndx*gQNUM2+qIndx;
...
double GSMSCTable::GetScreeningParam(double G1){
   if (G1<gG1Values1[0]) {
    return gSCRMIN1;
  } else if (G1<gG1Values2[0]) {
    double logG1 = std::log(G1);
    int k = (int)((logG1-fLogG1FuncMin1)*fInvLogDeltaG1Func1);
    return std::exp(gScrAValues1[k]*logG1)*gScrBValues1[k]; // log-log linear
  } else if (G1<gG1Values2[gNUMSCR2-1]) {
    double logG1 = std::log(G1);
    int k = (int)((logG1-fLogG1FuncMin2)*fInvLogDeltaG1Func2);
    return std::exp(gScrAValues2[k]*logG1)*gScrBValues2[k]; // log-log linear
  } else {
    return gSCRMAX2;
  }
}
...
//
// Load pre-computed transformed angular distributions from file
void GSMSCTable::LoadMSCData() {
  ...
  for (int il=0; il<gLAMBNUM; ++il) {
    double lambda = std::exp(fLogLambda0+fLogDeltaLambda*il);
    char fname[512];
    ...
    for (int iq=0; iq<gQNUM2; ++iq) {
      double qDelta = fDeltaQ2; // indicates that = fDeltaQ2
      int numData;
      infile >> numData;
      if (numData>0) {
        double qval   = gQMIN2+iq*fDeltaQ2;
        if (iq<gQNUM2-1) {
          double qvalPlus = qval+fDeltaQ2;
          double g1   = qvalPlus/lambda; // it's G1 that should be <1.0
          if (g1>1.0) {
            qvalPlus = 1.0*lambda;
...
void GSMSCTable::InitMoliereMSCParams() {
   const double const1   = 7821.6;      // [cm2/g]
   const double const2   = 0.1569;      // [cm2 MeV2 / g]
   const double finstrc2 = 5.325135453E-5; // fine-structure const. square
   ...
   fMoliereBc.resize(numMaterials);
   fMoliereXc2.resize(numMaterials);

   const double xims = 0.0; // use xi_MS = 0.0 value now. We will see later on.
   for (unsigned int imat = 0; imat<numMaterials; ++imat) {
     ...
     const double* theFractionVect = theMaterial->GetMassFractionVector();
     double zs = 0.0;
     double zx = 0.0;
     double ze = 0.0;
     for(int ielem = 0; ielem < numelems; ielem++) {
       double izet = theElemVect[ielem]->GetZ();
       double iwa  = theElemVect[ielem]->GetA()/(geant::g/geant::mole);
//       int ipz     = theAtomsVect[ielem];
       double ipz  = theFractionVect[ielem];

       double dum  = ipz/iwa*izet*(izet+xims);
       zs += dum;
       ze += dum*(-2.0/3.0)*std::log(izet);
       zx += dum*std::log(1.0+3.34*finstrc2*izet*izet);
//       wa += ipz*iwa;
     }
     double density = theMaterial->GetDensity()*geant::cm3/geant::g; // [g/cm3]

     fMoliereBc[theMaterial->GetIndex()]  = const1*density*zs*std::exp(ze/zs)/std::exp(zx/zs);  //[1/cm]
     fMoliereXc2[theMaterial->GetIndex()] = const2*density*zs;  // [MeV2/cm]

     // change to Geant4 internal units of 1/length and energ2/length
     fMoliereBc[theMaterial->GetIndex()]  *= 1.0/geant::cm;
     fMoliereXc2[theMaterial->GetIndex()] *= geant::MeV*geant::MeV/geant::cm;
   }
}
