...
Real_v PowerSameIf(const Real_v &inpValue, const double &exponent, const vecCore::Mask_v<Real_v> &condition = true);
...
// ===========---------------------------------------------------
template <class Real_v>
inline Real_v PowerSameIf(const Real_v &inpValue, const double &exponent, const vecCore::Mask_v<Real_v> &condition)
{
  using vecCore::Get;
  using vecCore::Set;
...
template <class Real_v>
inline Real_v PowerDiffIf(const Real_v &inpValue, const Real_v &exponent,
...
inline int countMaskTrue(vecCore::Mask_v<Real_v> &flagLane)
{
  ...
  count = 0;
  for (unsigned int i = 0; i < VecSize; i++) {
    if (vecCore::Get(flagLane, i)) count++;
  }
#endif
  return count;
}
