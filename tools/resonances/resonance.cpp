...
public:
  ...
  int LRX;
  double eLo, eHi, SPI, AP,
      rad_a; // Resonance range Low, High, Target Spin (I), Scattering Radius (AP) Channel radius (a)
...
{
  double k;
  k = (!isPrime) ? factor_k * sqrt(std::abs(x)) : factor_k * 0.5 * (1.0 / sqrt(std::abs(x)));
  return k;
}
...
double resonance::calcPhiP(double x, int l)
{
  ...
  case 3:
    return rhoCPrime *
           ((x6(x) - 60.0 * x5(x) + 1292.0 * x4(x) - 13500.0 * x3(x) + 49500.0 * x2(x) + 900.0 * x - 3150.0) / Fac3(x));
  }
}
...
double resonance::calcShiftP(double x, int l)
{
  ...
  case 3:
    return rhoP * ((6.0 * x * (3375.0 + 1800.0 * x2(x) + 765.0 * x4(x) + 60.0 * x6(x) + 2.0 * x6(x) * x2(x))) /
                   x2(225.0 + 45.0 * x2(x) + 6.0 * x4(x) + x6(x)));
  }
}
...
double resonance::calcPeneP(double x, int l)
{
  ...
  case 3:
    return rhop *
           ((x6(x) * (x6(x) + 18.0 * x4(x) + 225.0 * x2(x) + 1575.0)) / x2(x6(x) + 6.0 * x4(x) + 45.0 * x2(x) + 225.0));
  }
}
...
double resonance::GetERP(double x, int r, int lVal, int isPrime)
{
  ...
  case 0:
    result = (!lVal) ? x : x +
                               (calcShift(std::abs(er), lVal) - calcShift(x, lVal)) /
                                   (2.0 * calcPene(std::abs(er), lVal)) * Gamma_nrE(std::abs(er), r, lVal);
  case 1:
    result =
        (!lVal) ? 0.0 : -calcShift(x, lVal) / (2.0 * calcPene(std::abs(er), lVal)) * Gamma_nrE(std::abs(er), r, lVal);
    return result;
  }
}
  ...
  for (int lcnt = 0; lcnt < NLS; lcnt++) {
    ...
    for (int jVal = JMIN; jVal <= JMAX; jVal++) {
      ...
      for (int r = 0; r < nrsl; r++) {
        ...
        fac2p2d =
            (Gamma_nrEP(x, r, lVal) * (-2.0 * s2fil * (1.0 - ERP) + 2.0 * c2fil * GNR - 2.0 * GXR * s2fil)) / deno;
        fac2p2e = ((GNR * (GNR * c2fil) - 2.0 * GXR * s2fil + 2.0 * (x - ERP) * s2fil) *
                   (2.0 * x2(1.0 - ERP) + 0.5 * GNR * Gamma_nrEP(x, r, lVal))) /
                  x2(deno);
...
double bisection(double &a, double &b, double &c, double sig1, double sig2, double sig3, double sigP1, double sigP2,
                 double sigP3)
{
  double epsa = 1.0e-5;
  double tmp  = 0.0;
...
void resonance::Linearize()
{
  ...
  for (int i = 1; i < (totalPoints - 1); i++) { // first and last point excluded
    ...
    std::cout << i << " ER " << ER << " a " << a << " b " << b << " sgn siga " << sigPa << "  sgn sigb " << sigPb
              << std::endl;
  }
}
