...
void scan()
{
  ...
  while ((current = iter.Next())) {
    ...
    //      print the Normals (one rmal per triangle)

    file << "\"normals\": [\n";
    cout << "\n-->Generated " << Nfaces << " Normals ... ";
    cout << "Writing normals to file ... ";
    int Knormal = 0;
    while (!Fnormals.empty()) {
      Double_t *Vnorm = Fnormals.front();
      file << Vnorm[0] << "," << Vnorm[1] << "," << Vnorm[2];
      //          cout << "0," << point[0]<<","<<point[1]<<","<<point[2];
      Knormal = Knormal + 1;
    ...
    while (!Faces.empty()) {
      ...
      file << "16," << point[0] << "," << point[1] << "," << point[2] << "," << knorm << "  ";
