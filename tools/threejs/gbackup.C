...
void scan()
{
  ...
  while ((current = iter.Next())) {
    ...
    for (int i = 0; i < buffer_vol->NbPnts() * 3; i = i + 3) {
      cout << "[ " << j << " ]  " << buffer_vol->fPnts[i] << "  ||  " << buffer_vol->fPnts[i + 1] << "  ||  "
           << buffer_vol->fPnts[i + 2] << "\n";
    ...
    for (int i = 0; i < buffer_vol->NbSegs() * 3; i = i + 3) {
      cout << "[ " << j << " ]  " << buffer_vol->fSegs[i] << "  ||  " << buffer_vol->fSegs[i + 1] << "  ||  "
           << buffer_vol->fSegs[i + 2] << "\n";
    ...
    for (int i = 0; i < buffer_vol->NbPols(); i++) {
      ...
      for (int k = 1; k < buffer_vol->fPols[j] + 1; k++) {
        ...
        cout << "  ||  " << buffer_vol->fSegs[3 * buffer_vol->fPols[j + k] + 1] << "  |  "
             << buffer_vol->fSegs[3 * buffer_vol->fPols[j + k] + 2] << "  ||  ";
      }
...
void SavePicture(const char *name, TObject *objcanvas, TObject *objvol, Int_t iaxis, Double_t step)
{
  ...
  default:
    if (step == 0)
      sprintf(fname, "t_%sdiv%s.gif", name, vol->GetShape()->GetAxisName(iaxis));
    else
      sprintf(fname, "t_%sdivstep%s.gif", name, vol->GetShape()->GetAxisName(iaxis));
  }
  c->Print(fname);
}
...
void pcon(Int_t iaxis = 0, Int_t ndiv = 8, Double_t start = 0, Double_t step = 0)
{
  ...
  for (Int_t j = 0; j < pcon->GetNz(); j++) {
    char line[128];
    sprintf(line, "fZ[%i]=%5.2f  fRmin[%i]=%5.2f  fRmax[%i]=%5.2f", j, pcon->GetZ()[j], j, pcon->GetRmin()[j], j,
            pcon->GetRmax()[j]);
...
void pgon(Int_t iaxis = 0, Int_t ndiv = 8, Double_t start = 0, Double_t step = 0)
{
  ...
  for (Int_t j = 0; j < pgon->GetNz(); j++) {
    char line[128];
    sprintf(line, "fZ[%i]=%5.2f  fRmin[%i]=%5.2f  fRmax[%i]=%5.2f", j, pgon->GetZ()[j], j, pgon->GetRmin()[j], j,
            pgon->GetRmax()[j]);
    text = pt->AddText(line);
    text->SetTextColor(4);
    text->SetTextAlign(12);
