...
GVComptonProcess::GVComptonProcess() : 
  fProcessId (-1), fEnergyLimit (0.), fVComptonModel(0), fTargetElements(0),
  fParentTrackIndices(0), fPrimaryTracks(0), fSecondaryTracks(0)
{}
...
//------------------------------------------------------------------------------
FQUALIFIER
int GVComptonProcess::ApplyPostStepProcess(GeantTrack_v& gTrackV, int numtracks, 
                                           int tid)
{
...
void GVComptonProcess::Deallocator(){
   if(fTargetElements)     delete fTargetElements; fTargetElements = 0;
   if(fParentTrackIndices) delete fParentTrackIndices; fParentTrackIndices = 0;
   if(fPrimaryTracks)      GUTrackDeallocator(*fPrimaryTracks);
   if(fSecondaryTracks)    GUTrackDeallocator(*fSecondaryTracks);
}

//------------------------------------------------------------------------------
void GVComptonProcess::GUTrackAllocator(GUTrack_v &gutrack_v, int size){
   if(gutrack_v.capacity > 0) GUTrackDeallocator(gutrack_v);
