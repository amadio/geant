...
struct Workload {
  ...
  Workload(size_t nthreads) : 
...
#endif
      nnodes_(1), nthreads_(nthreads), fLock(ATOMIC_FLAG_INIT) {
    ...
    for (size_t n = 0; n < nthreads; ++n) future_[n] = promise_[n].get_future();
    ntracks_ = 1000000;
...
int main(int argc, char *argv[]) {
  ...
  size_t checksum = 0;
  for (size_t n = 0; n < nthreads; ++n) checksum += work.future_[n].get();

  std::cout << "run time: " << rt1-rt0 << "   cpu time: " << cpu1-cpu0 
            << "  checksum: " << checksum << " ref: " << work.checksum_ref_ << std::endl;
}
