...
typedef int (*kernelFunc_t)(vecgeom::cxx::DevicePtr<Geant::cuda::GeantTaskData> &workSpace, size_t workspaceSizeOf,
...
public:
  ...
  public:
    ...
    unsigned int AddTrack(Task *task, GeantBasket &basket, unsigned int hostIdx);
    unsigned int TrackToDevice(Task *task, int tid, GeantBasket &basket, unsigned int startIdx);
