...
void TThreadMergingServer::Listen() {
  ...
  while (!finish || fOutput->size() > 0)
    while (fOutput->try_pop(buffer)) {
      ...
      TMemFile *transient = new TMemFile(filename, buffer->Buffer() + buffer->Length(), length,
                                         "UPDATE"); // UPDATE because we need to remove the TTree after merging them.

      buffer->SetBufferOffset(buffer->Length() + length);

      const Float_t clientThreshold =
          0.75; // control how often the histogram are merged.  Here as soon as half the clients have reported.
