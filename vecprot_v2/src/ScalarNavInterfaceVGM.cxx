...
//______________________________________________________________________________
GEANT_CUDA_BOTH_CODE
void ScalarNavInterfaceVGM::NavFindNextBoundaryAndStep(const double &pstep, 
         const double &x, const double &y, const double &z,
         const double &dirx, const double &diry, const double &dirz, 
         const VolumePath_t *instate, VolumePath_t *outstate, 
         double &step, double &safe, bool &isonbdr) {
...
//         isonbdr - propagated point is on a boundary
  ...
  VNavigator const * newnav = instate->Top()->GetLogicalVolume()->GetNavigator();
  // Check if current safety allows for the proposed step
  if (safe > pstep) {
    step = pstep;
    isonbdr = false;
  ...
  step = newnav->ComputeStepAndSafetyAndPropagatedState(Vector3D_t(x, y, z),
                             Vector3D_t(dirx, diry, dirz),
                             Math::Min<double>(1.E20, pstep),
                             *instate, *outstate, !isonbdr, safe);
...
//______________________________________________________________________________
void ScalarNavInterfaceVGM::NavIsSameLocation(const double &x, const double &y, const double &z,
       const double &/*dirx*/, const double &/*diry*/, const double &/*dirz*/,
       const VolumePath_t *start, VolumePath_t *end, bool &same, VolumePath_t *tmpstate) {
// 
// Checks if the navigation states corresponding to positions (x,y,z) are the
