// This files was autogenerated by geant::ParticleOld::ReadFile

#if defined(__clang__) && !defined(__APPLE__)
#pragma clang optimize off
#endif
#include "ParticleOld.h"
#ifdef VECCORE_CUDA
#include "base/Vector.h"
template <typename T>
using vector = vecgeom::Vector<T>;
#else
using std::vector;
#endif

namespace geant {
inline namespace GEANT_IMPL_NAMESPACE {


//________________________________________________________________________________
VECCORE_ATT_HOST_DEVICE
void CreateParticle0016() {
   vector<int> daughters;
   ParticleOld *part = nullptr;

   // Creating B*_2-
   new ParticleOld("B*_2-", -525, 0, "B-Meson", 100, -1, 5.7469, 0.02, 100, 100, 1, 100, 1);
   part = const_cast<ParticleOld*>(&ParticleOld::Particles().at(-525));
   daughters.clear();
   daughters.push_back(-511);
   daughters.push_back(-211);
   part->AddDecay(ParticleOld::Decay(0, 0.3,  daughters));
   daughters.clear();
   daughters.push_back(-513);
   daughters.push_back(-211);
   part->AddDecay(ParticleOld::Decay(0, 0.16,  daughters));
   daughters.clear();
   daughters.push_back(-521);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.15,  daughters));
   daughters.clear();
   daughters.push_back(-513);
   daughters.push_back(-211);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.13,  daughters));
   daughters.clear();
   daughters.push_back(-523);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.08,  daughters));
   daughters.clear();
   daughters.push_back(-511);
   daughters.push_back(-211);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.08,  daughters));
   daughters.clear();
   daughters.push_back(-523);
   daughters.push_back(-211);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.06,  daughters));
   daughters.clear();
   daughters.push_back(-521);
   daughters.push_back(-211);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.04,  daughters));

   // Creating B*-
   new ParticleOld("B*-", -523, 0, "Meson", 100, -1, 5.3251, 0, 100, 100, 1, 100, 1);
   part = const_cast<ParticleOld*>(&ParticleOld::Particles().at(-523));
   daughters.clear();
   daughters.push_back(-521);
   daughters.push_back(-22);
   part->AddDecay(ParticleOld::Decay(0, 1,  daughters));

   // Creating B-
   new ParticleOld("B-", -521, 0, "B-Meson", 100, -1, 5.27915, 0, 100, 100, 1, 100, 1);
   part = const_cast<ParticleOld*>(&ParticleOld::Particles().at(-521));
   daughters.clear();
   daughters.push_back(-2);
   daughters.push_back(1);
   daughters.push_back(4);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(48, 0.4291,  daughters));
   daughters.clear();
   daughters.push_back(-2);
   daughters.push_back(4);
   daughters.push_back(1);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(13, 0.08,  daughters));
   daughters.clear();
   daughters.push_back(-4);
   daughters.push_back(3);
   daughters.push_back(4);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(13, 0.07,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(423);
   part->AddDecay(ParticleOld::Decay(42, 0.055,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(423);
   part->AddDecay(ParticleOld::Decay(42, 0.055,  daughters));
   daughters.clear();
   daughters.push_back(-16);
   daughters.push_back(15);
   daughters.push_back(423);
   part->AddDecay(ParticleOld::Decay(42, 0.03,  daughters));
   daughters.clear();
   daughters.push_back(423);
   daughters.push_back(-433);
   part->AddDecay(ParticleOld::Decay(0, 0.025,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(421);
   part->AddDecay(ParticleOld::Decay(42, 0.02,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(421);
   part->AddDecay(ParticleOld::Decay(42, 0.02,  daughters));
   daughters.clear();
   daughters.push_back(-4);
   daughters.push_back(4);
   daughters.push_back(3);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(13, 0.02,  daughters));
   daughters.clear();
   daughters.push_back(421);
   daughters.push_back(-433);
   part->AddDecay(ParticleOld::Decay(0, 0.0185,  daughters));
   daughters.clear();
   daughters.push_back(423);
   daughters.push_back(-20213);
   part->AddDecay(ParticleOld::Decay(0, 0.018,  daughters));
   daughters.clear();
   daughters.push_back(421);
   daughters.push_back(-431);
   part->AddDecay(ParticleOld::Decay(0, 0.015,  daughters));
   daughters.clear();
   daughters.push_back(-2);
   daughters.push_back(1);
   daughters.push_back(2);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(42, 0.015,  daughters));
   daughters.clear();
   daughters.push_back(423);
   daughters.push_back(-431);
   part->AddDecay(ParticleOld::Decay(0, 0.0135,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(425);
   part->AddDecay(ParticleOld::Decay(42, 0.012,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(425);
   part->AddDecay(ParticleOld::Decay(42, 0.012,  daughters));
   daughters.clear();
   daughters.push_back(421);
   daughters.push_back(-213);
   part->AddDecay(ParticleOld::Decay(0, 0.011,  daughters));
   daughters.clear();
   daughters.push_back(-16);
   daughters.push_back(15);
   daughters.push_back(421);
   part->AddDecay(ParticleOld::Decay(42, 0.01,  daughters));
   daughters.clear();
   daughters.push_back(423);
   daughters.push_back(-213);
   part->AddDecay(ParticleOld::Decay(0, 0.009,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(20423);
   part->AddDecay(ParticleOld::Decay(42, 0.008,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(20423);
   part->AddDecay(ParticleOld::Decay(42, 0.008,  daughters));
   daughters.clear();
   daughters.push_back(421);
   daughters.push_back(-20213);
   part->AddDecay(ParticleOld::Decay(0, 0.0055,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(10421);
   part->AddDecay(ParticleOld::Decay(42, 0.005,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(10423);
   part->AddDecay(ParticleOld::Decay(42, 0.005,  daughters));
   daughters.clear();
   daughters.push_back(-14);
   daughters.push_back(13);
   daughters.push_back(10421);
   part->AddDecay(ParticleOld::Decay(42, 0.005,  daughters));
   daughters.clear();
   daughters.push_back(-12);
   daughters.push_back(11);
   daughters.push_back(10423);
   part->AddDecay(ParticleOld::Decay(42, 0.005,  daughters));
   daughters.clear();
   daughters.push_back(-4);
   daughters.push_back(3);
   daughters.push_back(2);
   daughters.push_back(-2);
   part->AddDecay(ParticleOld::Decay(42, 0.005,  daughters));
   daughters.clear();
   daughters.push_back(423);
   daughters.push_back(-211);
   part->AddDecay(ParticleOld::Decay(0, 0.0042,  daughters));
   daughters.clear();
   daughters.push_back(421);
   daughters.push_back(-211);
   part->AddDecay(ParticleOld::Decay(0, 0.0035,  daughters));
   daughters.clear();
   daughters.push_back(-20443);
   daughters.push_back(-323);
   part->AddDecay(ParticleOld::Decay(0, 0.0025,  daughters));
   daughters.clear();
   daughters.push_back(-20443);
   daughters.push_back(-321);
   part->AddDecay(ParticleOld::Decay(0, 0.0019,  daughters));
   daughters.clear();
   daughters.push_back(-443);
   daughters.push_back(-323);
   part->AddDecay(ParticleOld::Decay(0, 0.0014,  daughters));
   daughters.clear();
   daughters.push_back(-443);
   daughters.push_back(-321);
   part->AddDecay(ParticleOld::Decay(0, 0.0008,  daughters));
   daughters.clear();
   daughters.push_back(-441);
   daughters.push_back(-323);
   part->AddDecay(ParticleOld::Decay(0, 0.0007,  daughters));
   daughters.clear();
   daughters.push_back(-441);
   daughters.push_back(-321);
   part->AddDecay(ParticleOld::Decay(0, 0.0004,  daughters));

   // Creating B*_20_bar
   new ParticleOld("B*_20_bar", -515, 0, "B-Meson", 100, 0, 5.7469, 0.02, 100, 100, 1, 100, 1);
   part = const_cast<ParticleOld*>(&ParticleOld::Particles().at(-515));
   daughters.clear();
   daughters.push_back(-521);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.3,  daughters));
   daughters.clear();
   daughters.push_back(-523);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.16,  daughters));
   daughters.clear();
   daughters.push_back(-511);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.15,  daughters));
   daughters.clear();
   daughters.push_back(-523);
   daughters.push_back(211);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.13,  daughters));
   daughters.clear();
   daughters.push_back(-513);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.08,  daughters));
   daughters.clear();
   daughters.push_back(-521);
   daughters.push_back(211);
   daughters.push_back(-111);
   part->AddDecay(ParticleOld::Decay(0, 0.08,  daughters));
   daughters.clear();
   daughters.push_back(-513);
   daughters.push_back(-211);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.06,  daughters));
   daughters.clear();
   daughters.push_back(-511);
   daughters.push_back(-211);
   daughters.push_back(211);
   part->AddDecay(ParticleOld::Decay(0, 0.04,  daughters));

   // Creating B*0_bar
   new ParticleOld("B*0_bar", -513, 0, "B-Meson", 100, 0, 5.3251, 0, 100, 100, 1, 100, 1);
   part = const_cast<ParticleOld*>(&ParticleOld::Particles().at(-513));
   daughters.clear();
   daughters.push_back(-511);
   daughters.push_back(-22);
   part->AddDecay(ParticleOld::Decay(0, 1,  daughters));
}

} // End of inline namespace
} // End of geant namespace
#if defined(__clang__) && !defined(__APPLE__)
#pragma clang optimize on
#endif
