...
: fIsElectron(iselectron), fDataFileIndx(datafileindx), fModelName(modelname) {
  fDCSMaxZet                       = 0;
...
/**
  ...
  */
double SeltzerBergerBremsModel::SamplePhotonEnergy(MaterialCuts *matcut, double eekin, double r1, double r2, double r3) {
  ...
  if (fMaxElecEnergy<eekin){
    std::cerr<<" **** Elecetonr energy = "<<eekin/geant::GeV<<" [GeV] > fMaxElecEnergy = "<<fMaxElecEnergy<<std::endl;
    exit(-1);
  }
      ...
      double *logReducedPhotonEnergyGrid = new double[fLoadDCSNumReducedPhotonEnergies];
       ...
       for (int irpener=0; irpener<fLoadDCSNumReducedPhotonEnergies; ++irpener) {
        logReducedPhotonEnergyGrid[irpener] = std::log(1.0-fLoadDCSReducedPhotonEnergyGrid[irpener]+1.0e-12);
         ...
         theDCS[irpener] = 0.0;
         for (int ielem=0; ielem<theElements.size(); ++ielem) {
 ...
 */
double SeltzerBergerBremsModel::ComputeXSectionPerAtom(Element *elem, Material *mat, double gammaprodcutenergy,
                                                       double electronekin){
  double xsec = 0.0;
 ...
 */
double SeltzerBergerBremsModel::ComputeXSectionPerVolume(Material *mat, double gammaprodcutenergy, double electronekin){
  double xsec = 0.0;
  if (electronekin<=gammaprodcutenergy || electronekin<fLoadDCSElectronEnergyGrid[0])
    return xsec;
 ...
 *  \f$\xi\f$.
 */
double SeltzerBergerBremsModel::ComputeDEDXPerVolume(Material *mat, double gammaprodcutenergy, double electronekin){
  double dedx = 0.0;
