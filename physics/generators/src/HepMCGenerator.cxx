...
EventInfo HepMCGenerator::NextEvent(geant::TaskData * /*td*/)
{
  ...
  if (!(input_file->read_event(evt))) geant::Fatal("HepMCGenerator::ImportTracks", "No more particles to read!");
  ...
  std::cout << std::endl << "Number of stable particles ";
  if (fEtaCut || fPhiCut) std::cout << "within selected cuts: ";
  std::cout << ntracks;
  if (ntot > ntracks) std::cout << " out of " << ntot;
  std::cout << std::endl;
...
void HepMCGenerator::GetTrack(int n, geant::Track &gtrack, geant::TaskData * /*td*/)
{
  ...
  for (const HepMC::GenParticlePtr &genpart : search->results()) {
    ...
    if ((bool)genpart->production_vertex()) {
      gtrack.SetPosition(genpart->production_vertex()->position().x(), genpart->production_vertex()->position().y(),
    ...
    gtrack.SetDirection(genpart->momentum().px() / ptrack, genpart->momentum().py() / ptrack,
