...
GunGenerator::GunGenerator()
    : fAverage(0), fPDG(11),           // PDG code of the primary: 11 -> e-
      ...
      fYDir(0.), fZDir(1.), fGVPartIndex(-1), fPartPDG(0), fMass(0), fCharge(0), fPTotal(0), fETotal(0), fRndgen(0)
{
  // Dummy constructor
}
...
GunGenerator::GunGenerator(int aver, int partpdg, double partekin, double xpos, double ypos, double zpos, double xdir,
    ...
    : fAverage(aver), fPDG(partpdg), fPartEkin(partekin), fXPos(xpos), fYPos(ypos), fZPos(zpos), fXDir(xdir),
      fYDir(ydir), fZDir(zdir), fGVPartIndex(-1), fPartPDG(0), fMass(0), fCharge(0), fPTotal(0), fETotal(0), fRndgen(0)
{
  // Constructor
...
EventInfo GunGenerator::NextEvent(geant::TaskData * /*td*/)
{
  //
  int ntracks               = 1;
  if (fAverage > 1) ntracks = fRndgen->Poisson(fAverage);
