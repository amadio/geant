...
protected:
   ...
   */
  double SampleTotalEnergyTransfer(const double egamma, const int izet, const double r1, const double r2,
                                   const double r3);
   ...
   * @param[in]      delta  Screening variable \f$ \delta(\epsilon) = 136 Z^{-1/3} \epsilon_0/(\epsilon(1-\epsilon))
   * \f$.
  ...
  struct RatinAliasDataPerElement {
    RatinAliasDataPerElement(int numeprims) { fRatinAliasData.resize(numeprims, nullptr); }
