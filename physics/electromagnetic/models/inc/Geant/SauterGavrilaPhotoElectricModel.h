...
/**
 * @brief  This class implements a PhotoElectric model for gamma based on SauterGavrila differential cross section for
 * the sampling of the scattering angle of the secondary particle (e-) and on the EPICS2014 cross-sections data.
 *
 * @class   SauterGavrilaPhotoElectricModel
 * @author  M Bandieramonte
 * @date    June 2017
 *
 * The photoelectric effect is the ejection of an electron from a material after a photon has been absorbed by that
 * material. Depending on the input energy of the incident gamma, the model provides cross-sections based either on the
 * interpolation of tabulated cross-sections data or on the parameterization of cross-sections data obtained through two
 * fits in two different energy ranges (low and high). Note that for this process the lambda table is not built, since
 * the cross-section is not a smooth function of the energy, therefore in all calculations the cross section is used
 * directly. Final state sampling: The incident photon is absorbed and an electron is emitted with a direction that is
 * calculated using the Sauter-Gavrila distribution (for K-shell) \cite sautergavrila. The electron kinetic energy is
 * the difference between the incident photon energy and the binding energy of the electron before the interaction. The
...
public:
   ...
   * @param[in] aliasActive   Boolean true if we want to use Alias Sampling to sample the secondary particle direction.
   * By default is set to false and model uses composition-rejection sampling
   */
   ...
   * PhysicsManagerPerParticle object. This method is implemented even if for photoelectric effect the table is not
   *built to be used at run-time, but the cross-sections are calculated on the flight.
   *
   ...
   * Primary track properties are updated and secondary track(s) are generated according to the photoelectric effect
   * interaction. After photoelectric interaction the incident photon is absorbed and an electron is emitted. The
   * electron kinetic energy is the difference between the incident photon energy and the binding energy of the electron
   * before the interaction. The sub-shell, from which the electron is emitted, is randomly
   * selected according to the relative cross-sections of all subshells, determined at the given energy. The interaction
   * leaves the atom in an excited state but the deexcitation of the atom is not simulated yet.
   *
   * @param[in,out] track     Primary track. At input, it stores the pre-interaction primary particle properties and
   *                          some information about the current material-cut couple (if needed). It is updated by the
   * method and it stores the post-interaction primary track properties at output. After the interaction the gamma is
...
protected:
   ...
   * This is a unit test for SampleTargetElementIndex method. The methos is calculating the Macroscopic cross-section
   * for all the materials active in the simulation and storing it in xsec vector. Then SampleTargetElementIndex is
   * called 10^9 times and sampled index used to increment the corresponding entry of xsecSampled vector. The xsec and
   * xsecSampled (normalized) vectors are then stored in an output file.
   *
   * @param[in] matcut    MaterialCuts to retrieve the element composition of the material.
  ...
  /**
   * @brief Private method to calculate Differential cross section based on SauterGavrila distribution for k-shell  but
   * using the transformed variable \f$xsi\f$.
   *
   *
   * @param[in] energy    primary particle (gamma) kinetic energy.
   * @param[in] costheta  cosTheta of the secondary particle (photoelectron).
  ...
  /**
   * @brief Private method to compute the cross-section per atom for the photoelectric effect. The computation is
   * performed in a different way depending on the Z of the element and on the energy of the incident gamma. There are
   * two parameterizations (low and high energy) and two sets of tabulated cross-section data (below k-shell binding
   * energy and above k-shell binding energy).
   *
   * The total photoelectric and single shell cross-sections are tabulated from threshold to a lowParamThreshold that
   * starts from 5keV and increases with the k-shell binding energy of the corresponding element. Above the
   * lowParamThreshold, EPICS2014 cross sections \cite epics2014 are parameterized as following:
   *
   * \f[
   ...
   *   - le-cs data : for energies below k-shell binding energy - Linear interpolation of low-energy cs tabulated data
   * (as done in Geant4).
   *   - cs data    : for energies above k-shell binding energy - Spline interpolation of cs tabulated data (as done in
   * Geant4).
   ...
   *
   * To avoid tracking problems for very low-energy gamma, the photoelectric cross section is not equal to zero below
   * the first ionisation potential but it has a constant value (equal to corresponding value at the ionization
   * potential). This means that all types of media are not transparent for gamma.
   *
  ...
  /**
   * @brief Private method to sample PhotoElectron direction corresponding to a specific gamma kinetic energy, with
   * Alias sampling. The tables are created at initialization phase.
   *
   * @param[in] energy    primary particle (gamma) kinetic energy.
   ...
   */
  double SamplePhotoElectronDirection_Alias(double energy, double r1, double r2, double r3) const;
   ...
   * \f]
   * where  \f$E_e\f$ is the electron energy, \f$m_e\f$ its rest mass and \f$\beta\f$ its velocity in units of the speed
   * of light \f$c\f$. Random sampling of \f$\nu\f$ from this distribution can be performed analytically. For more
   * details have a look at the Penelope manual \cite salvat2006penelope. Though the Sauter distribution, strictly
   * speaking, is adequate only for ionisation of the K-shell by high-energy photons, in many practical simulations it
   ...
   *
   *  Used at initialisation of the model to load parameterization data used to calculate cross-sections. It selects
   *only the materials present in the list of active regions and for each of them calls the internal method ReadData to
   *read and store the corresponding parameterization files.
   *
   **/
  void LoadData();
   ...
   *
   *  The files read are:
   *  - pe-le-cs-zeta.dat:    Low energy (below k-shell binding energy) cross-section data. They are stored in
   *fLECSVector[Z].
   ...
   *  fNumSamplingPrimEnergiesPerDecade variable. For energies higher than 100MeV the secondary is considered to go
   *straight and being ejected from the atom with the same direction as the incident gamma. A sampling table, using
   *Walker's discrete alias method combined with <em>linear approximation of the p.d.f.</em>, is built at each gamma
   *energy grid point using the BuildOneLinAlias() method. These sampling tables are used at run-time to sample the
   *secondary (photoelectron) direction.
   **/
  void InitSamplingTables();
  void InitShellSamplingTables();
   ...
   *  call the CalculateDiffCrossSection internal method to compute the Differential cross section based on
   * SauterGavrila distribution for k-shell \cite sautergavrila.
   *
   ...
   *        This method is preparing data structure for Alias Sampling using an adaptive binning, The number of points
   *used for binning is not fixed but varies from energy to energy to meet the precision requirements that is set to the
   *value of gsingleTableErrorThreshold variable
   *
   *
...
protected:
  ...
  /** @brief Vector of booleans. fCrossSection[Z] is true if there are CrossSections data (for energies above k-shell
   * binding energy) for element Z. */
  static bool *fCrossSection; // true if there are CrossSections data (for energies above k-shell binding energy)

  /** @brief Vector of booleans. fCrossSectionLE[Z] is true if there are Low-Energy CrossSections data (for energies
   * below k-shell binding energy) for element Z. */
  static bool
      *fCrossSectionLE; // true if there are Low-Energy CrossSections data (for energies below k-shell binding energy)
  ...
  /**
   * @name Members to describe the discrete gamma energy grid for sampling tables that will be used to sample the
   * photoelectron (e-) direction
   *
   ...
   * The min/max of the table is set to fMinPrimEnergy and fMaxPrimEnergy limits and the number of discrete gamma energy
   * points will be determined by the value of SauterGavrilaPhotoElectricModel::fNumSamplingPrimEnergiesPerDecade
   * variable. The default value can be changed by the user through the SetNumberOfPhotonEnergiesPerDecade()(to do: add
   * this method) public method (must be set before the initialisation of the model!).
   *
   */
  //@{
  //---------------------------------------------
  ...
  int fShellNumSamplingPrimEnergies;
  int fNumAliasTables;
   ...
   * photoelectron angle related variable in [log(e-12),log(2)]. This is dinamically set at initialization time. */
  int fNumSamplingAngles;
  int fShellNumSamplingAngles;

  //---------------------------------------------
  /** @brief Minimum of the gamma kinetic energy grid (default 1.0 [eV]).*/
  double fMinPrimEnergy;
  double fShellMinPrimEnergy; //= 0.1*geant::units::keV;
   ...
   *        [SauterGavrilaPhotoElectricModel::fMinGammaEnergy, SauterGavrilaPhotoElectricModel::fMaxGammaEnergy]
   * interval.
   */
