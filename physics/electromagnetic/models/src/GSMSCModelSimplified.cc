...
GSMSCModelSimplified::GSMSCModelSimplified(bool iselectron, const std::string &name)
    : MSCModel(name), fIsElectron(iselectron)
{
...
GSMSCModelSimplified::~GSMSCModelSimplified()
{
  if (fGSTable) {
    delete fGSTable;
    fGSTable = nullptr;
...
void GSMSCModelSimplified::ConvertTrueToGeometricLength(geant::Track *gtrack, geant::TaskData * /*td*/)
{
  ...
  if (tau <= fTauSmall) {
    ...
    if (tau < fTauLim) {
      ...
      mscdata.fTheZPathLenght = mscdata.fLambda1 * (1. - geant::Exp(-tau));
    }
...
void GSMSCModelSimplified::ConvertGeometricToTrueLength(geant::Track *gtrack, geant::TaskData * /*td*/)
{
  ...
  if (gtrack->GetStep() < fTLimitMinfix2) {
    ...
    if (gtrack->GetStep() > mscdata.fLambda1 * fTauSmall) {
      if (mscdata.fPar1 < 0.) {
        tlength = -mscdata.fLambda1 * geant::Log(1. - gtrack->GetStep() / mscdata.fLambda1);
      } else {
...
void GSMSCModelSimplified::SampleMSC(geant::Track *gtrack, geant::TaskData *td)
{
  ...
  if (0.5 * Qn1 > 7.0) {
    ...
    double lekin = geant::Log(efEnergy);
    ...
    int mcDeltIdx                      = -1;
    double transfPar                   = 0.;
    bool isMsc = fGSTable->Sampling(0.5 * lambdan, 0.5 * Qn1, scra, cosTheta1, sinTheta1, lekin, beta2, matIndx, &gsDtr,
                                    mcEkinIdx, mcDeltIdx, transfPar, td, true);
    fGSTable->Sampling(0.5 * lambdan, 0.5 * Qn1, scra, cosTheta2, sinTheta2, lekin, beta2, matIndx, &gsDtr, mcEkinIdx,
                       mcDeltIdx, transfPar, td, !isMsc);
  ...
  double loga  = geant::Log(1.0 + 1.0 / scra);
                                             ...
                                             double &scra, double &g1, double &pMCtoQ1, double &pMCtoG2PerG1)
{
  ...
  double scpCor    = 1.0;
  fPWACorrection->GetPWACorrectionFactors(geant::Log(ekin), beta2, matindx, pMCtoScrA, pMCtoQ1, pMCtoG2PerG1);
  // screening parameter:
  ...
  g1 = 2.0 * scra * ((1.0 + scra) * geant::Log(1.0 / scra + 1.0) - 1.0);
...
double GSMSCModelSimplified::GetTransportMeanFreePathOnly(const MaterialCuts *matcut, double ekin)
{
  ...
  double scpCor      = 1.0;
  fPWACorrection->GetPWACorrectionFactors(geant::Log(ekin), beta2, matindx, mctoScrA, mctoQ1, mctoG2PerG1);
