    ...
    for (unsigned long j = 0; j < userInact.size(); ++j) {
      if (userInact[j] > numRegions) {
        ...
        std::cerr << " *** WARNING: EMModelManager::Initialise() \n"
                  ...
                  << "  in region index = " << userInact[j] << " is ignored since max region index = " << numRegions - 1
                  << " !" << std::endl;
        continue;
      }
...
EMModel *EMModelManager::SelectModel(double ekin, int regionindx)
{
  ...
  if (numModels > 0) {
    ...
    if (numModels > 1) {
      ...
      do {
        --modelIndx;
      } while (modelIndx && ekin <= models[modelIndx]->GetLowEnergyUsageLimit());
    ...
    std::cerr << "  **** ERROR EMModelManager::SelectModel() \n"
              << "     No model in region so discrete interaction should not happen!" << std::endl;
    exit(-1);
  }
