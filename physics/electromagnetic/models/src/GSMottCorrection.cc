...
const std::string GSMottCorrection::gElemSymbols[] = {
                                               ...
                                               int &ekindx, int &deltindx, geant::TaskData *td)
{
  ...
  if (angIndx < gNumAngle - 2) { // normal case: linear interpolation
    val = remRfaction * (perDelta->fRejFuntion[angIndx + 1] - perDelta->fRejFuntion[angIndx]) +
          perDelta->fRejFuntion[angIndx];
  } else { // last bin
...
void GSMottCorrection::InitMCDataMaterial(const Material *mat)
{
  ...
  for (int iek = 0; iek < gNumEkin; ++iek) {
    ...
    for (int ielem = 0; ielem < numElems; ++ielem) {
      ...
      perMatPerEkin->fMCScreening += nZZPlus1 * Math::Log(Z23 * mcScrCF);
      ...
      perMatPerEkin->fMCFirstMoment +=
          nZZPlus1 * (Math::Log(1. + 1. / mcScrCF) - 1. / (1. + mcScrCF)) * perElemPerEkin->fMCFirstMoment;
      ...
      if (ielem == numElems - 1) {
        ...
        double dumScr               = Math::Exp(perMatPerEkin->fMCScreening / zs);
        ...
        for (int idel = 0; idel < gNumDelta; ++idel) {
          ...
          // II. llop over angles
          for (int iang = 0; iang < gNumAngle; ++iang) {
            if (perMatPerDelta->fRejFuntion[iang] > maxVal) maxVal = perMatPerDelta->fRejFuntion[iang];
          }
          for (int iang = 0; iang < gNumAngle; ++iang) {
