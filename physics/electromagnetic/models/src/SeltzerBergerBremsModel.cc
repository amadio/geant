...
//    kresid = (kappa-fLoadDCSReducedPhotonEnergyGrid[ik])/(fLoadDCSReducedPhotonEnergyGrid[ik+1]-fLoadDCSReducedPhotonEnergyGrid[ik]);
...
double SeltzerBergerBremsModel::SamplePhotonEnergy(const MaterialCuts *matcut, double eekin, double r1, double r2, double r3) {
  ...
  egamma = fAliasSampler->SampleLinear(&(als->fXdata[0]), &(als->fYdata[0]), &(als->fAliasW[0]),
...
//    const double xiMax      = std::log(eekin*eekin+densityCor); // xi(kappa=1)
    ...
    for (int ikappa=0; ikappa<fLoadDCSNumReducedPhotonEnergies; ++ikappa) {
      if (fLoadDCSReducedPhotonEnergyGrid[ikappa]>kappac && std::abs(1.-fLoadDCSReducedPhotonEnergyGrid[ikappa]/kappac)>1.e-8) {
