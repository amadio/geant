...
EMElementSelector::EMElementSelector(EMModel *emmodel, double emin, double emax, int binsperdecade, int numelement)
    : fNumEnergyBins(0), fMinEnergy(emin), fMaxEnergy(emax), fLogMinEnergy(-1.), fEnergyILDelta(-1.),
      fEnergyGrid(nullptr), fEMModel(emmodel)
{
...
void EMElementSelector::Build(const MaterialCuts *matcut, const Particle *part)
{
  ...
  const double *theAtomicNumDensityVector =
      matcut->GetMaterial()->GetMaterialProperties()->GetNumOfAtomsPerVolumeVect();
