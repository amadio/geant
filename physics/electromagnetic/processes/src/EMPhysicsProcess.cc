...
EMPhysicsProcess::EMPhysicsProcess(const std::string &name)
    : PhysicsProcess(name), fModelManager(nullptr), fFinalRange(-1.), fDRoverRange(-1.), fLowestKineticEnergy(-1.),
      fLinearEnergyLossLimit(1.)
{
  // set process type
...
void EMPhysicsProcess::Initialize()
{
  ...
  if (fModelManager == 0) {
    std::cerr << " **** ERROR: EMPhysicsProcess::Initialize() \n"
              << "      No any models for Process = " << GetName() << std::endl;
    exit(-1);
  }
  ...
  if (ProcessType::kEnergyLoss == GetType()) {
    for (unsigned long i = 0; i < GetListParticlesAssignedTo().size(); ++i) {
      ELossTableRegister::Instance().RegisterEnergyLossProcessForParticle(
          GetListParticlesAssignedTo()[i]->GetInternalCode(), this);
    }
  ...
  if (GetType() == ProcessType::kEnergyLoss) {
    ...
    if (numModels == 1) {
      ...
      if (i > 0) {
        ...
        // std::cout<< "====== " <<kinenergy<<" "<<lowEnergyLimit << dedx1/(geant::units::MeV/geant::units::mm) <<"
        // "<<dedx2/(geant::units::MeV/geant::units::mm) <<std::endl;
        if (dedx2 > 0.0) {
          delta = (dedx1 / dedx2 - 1.0) * lowEnergyLimit;
        }
        //          std::cout<< "====== " <<kinenergy<<" "<<delta<<" "<<lowEnergyLimit <<
        //          dedx1/(geant::units::MeV/geant::units::mm) <<"  "<<dedx2/(geant::units::MeV/geant::units::mm)
        //          <<std::endl;
      }
...
double EMPhysicsProcess::AlongStepLimitationLength(geant::Track *gtrack, geant::TaskData * /*td*/) const
{
...
int EMPhysicsProcess::AlongStepDoIt(LightTrack &track, geant::TaskData * /*td*/)
{
...
//
//
//   // set number of interaction left to -1.0 to indicate that new sampling is needed in the next step discrete step
//   limit
...
int EMPhysicsProcess::PostStepDoIt(LightTrack &track, geant::TaskData *td)
{
  int numSecondaries = 0;
...
int EMPhysicsProcess::AddModel(EMModel *model)
{
  ...
  if (!model) {
    ...
    std::cerr << " *** WARNING: EMPhysicsProcess::AddModel() \n"
              << "  Attempt to add EMModel = nullptr to process with Name = " << GetName()
