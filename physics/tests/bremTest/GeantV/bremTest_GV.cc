...
static struct option options[] = {
    {"particle-name     (possible particle names: e-, e+)                        - default: e-", required_argument, 0,
     'p'},
    {"material-name     (with a NIST_MAT_ prefix; see more in material doc.)     - default: NIST_MAT_Pb",
     required_argument, 0, 'm'},
    {"primary-energy    (in internal energy units i.e. [GeV])                    - default: 0.1", required_argument, 0,
     'E'},
    ...
    {"number-of-bins    (number of bins in the histogram)                        - default: 100", required_argument, 0,
     'n'},
    {"model-name        (bremSB, bremRel)                                        - default: bremSB", required_argument,
     0, 'b'},
    {"cut-vale          (secondary production threshold value for all particles) - default: 0.1", required_argument, 0,
     'c'},
...
double sampleDistribution(double numSamples, double primaryEnergy, const MaterialCuts *matCut, Particle *primParticle,
                          EMModel *model, Hist *histo1, Hist *histo2, Hist *histo3, Hist *histo4);
//***********************************************************************************************//
...
int main(int argc, char *argv[])
{
  ...
  if (kineticEnergy <= gammaCutEnergy) {
    std::cout << " *** Primary energy = " << kineticEnergy / geant::units::MeV
              ...
              << " [MeV] so there is no secondary gamma production at this energy!" << std::endl;
    return 0;
  }
  ...
  std::cout << std::setw(14) << std::scientific << std::right
            << unRestrictedDEDX / (geant::units::MeV / geant::units::cm) << std::setw(14) << std::left << "   [MeV/cm]";
  std::cout << std::endl;
  //===========================================================================================//
  ...
  delete histo;
  //
  sprintf(fileName, "brem_%s_GV_%s_energy_%s", bremModelName.c_str(), pname.c_str(),
          (matCut->GetMaterial()->GetName()).c_str());
  f     = fopen(fileName, "w");
  ...
  delete histo;
  //
  sprintf(fileName, "brem_%s_GV_%s_angular_%s", bremModelName.c_str(), pname.c_str(),
          (matCut->GetMaterial()->GetName()).c_str());
  f     = fopen(fileName, "w");
