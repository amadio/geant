#ifndef GEANTV_SELTZERBERGERTESTCOMMON_H
#define GEANTV_SELTZERBERGERTESTCOMMON_H
...
const double maxEn   = 10.0 * geant::units::GeV;

SeltzerBergerBremsModel *PrepareSBModel(bool useAlias, bool iselecton)
{
  SeltzerBergerBremsModel *model = new SeltzerBergerBremsModel(iselecton);
...
VecSeltzerBergerBremsModel *PrepareVecSBModel(bool useAlias, bool iselectron)
{
  VecSeltzerBergerBremsModel *model = new VecSeltzerBergerBremsModel(iselectron);
  model->SetLowEnergyUsageLimit(minEn);
  model->SetHighEnergyUsageLimit(maxEn);
  model->SetUseSamplingTables(useAlias);
  model->GetListActiveRegions().resize(1);
