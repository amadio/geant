...
static std::string photoElectricModelName(
    "SauterGavrilaPhotoElectric");      // name of the sauterGavrilaPhotoElectric model to test
...
static struct option options[] = {
    ...
    {"particle-name     (possible particle names: gamma)                         - default: gamma", required_argument,
     0, 'p'},
    {"material-name     (with a NIST_MAT_ prefix; see more in material doc.)     - default: NIST_MAT_Pb",
     required_argument, 0, 'm'},
    {"primary-energy    (in internal energy units i.e. [GeV])                    - default: 0.01", required_argument, 0,
     'E'},
    {"number-of-samples (number of required final state samples)                 - default: 1.e+7", required_argument,
     0, 'f'},
    {"number-of-bins    (number of bins in the histogram)                        - default: 100", required_argument, 0,
     'n'},
    {"model-name        (sauterGavrilaPhotoElectric)                             - default: sauterGavrilaPhotoElectric",
     required_argument, 0, 'b'},
    {"isAlias           (is the Alias sampling active ?)                         - default: false", required_argument,
     0, 's'},
    {"cut-value         (secondary production threshold value for all particles) - default: 0.1", required_argument, 0,
     'c'},
    {"cut-in-energy     (is the production cut value given in energy ? )         - default: false", no_argument, 0,
     'e'},
...
double sampleDistribution(double numSamples, double primaryEnergy, const MaterialCuts *matCut, Particle *primParticle,
                          EMModel *model, Hist *histo1, Hist *histo2, Hist *histo3);
...
int main(int argc, char *argv[])
{
  ...
  EMModel *emModel =
      new SauterGavrilaPhotoElectricModel(photoElectricModelName, true); // true to use Alias Sampling method
  EMModel *emModel_rej =
      new SauterGavrilaPhotoElectricModel(photoElectricModelName, false); // true to use Alias Sampling method
  ...
  if (kineticEnergy < emModel->GetLowEnergyUsageLimit() || kineticEnergy > emModel->GetHighEnergyUsageLimit()) {
    std::cout << " *** Primary energy = " << kineticEnergy / geant::units::GeV
              ...
              << "   - low energy usage limit  = " << emModel->GetLowEnergyUsageLimit() / geant::units::GeV
              << " [GeV]\n"
              << "   - high energy usage limit = " << emModel->GetHighEnergyUsageLimit() / geant::units::GeV
              << " [GeV]\n"
              << "  there is no secondary gamma production otherwise!" << std::endl;
    return 0;
  }
  ...
  double timeInSec = sampleDistribution(numSamples, kineticEnergy, matCut, particle, emModel,
                                        histo_photoelectron_energy, histo_photoelectron_angular, histo_angle);
  double timeInSec_rej =
      sampleDistribution(numSamples, kineticEnergy, matCut, particle, emModel_rej, histo_photoelectron_energy_rej,
                         histo_photoelectron_angular_rej, histo_angle_rej);
  std::cout << "   -------------------------------------------------------------------------------- " << std::endl;
  ...
  sprintf(fileName, "photoElectric_%s_GV_photoelectron_energy_%s_%sMeV.ascii", photoElectricModelName.c_str(),
          (matCut->GetMaterial()->GetName()).c_str(), str.c_str());
  ...
  //
  sprintf(fileName, "photoElectric_%s_GV_photoelectron_angular_%s_%sMeV.ascii", photoElectricModelName.c_str(),
          (matCut->GetMaterial()->GetName()).c_str(), str.c_str());
  f     = fopen(fileName, "w");
  ...
  sprintf(fileName, "photoElectric_%s_GV_photoelectron_angular_rejection_%s_%sMeV.ascii",
          photoElectricModelName.c_str(), (matCut->GetMaterial()->GetName()).c_str(), str.c_str());
  f     = fopen(fileName, "w");
  ...
  // sprintf(fileName,"GV_%s_cosTheta_%s_%sMeV.ascii",photoElectricModelName.c_str(),(matCut->GetMaterial()->GetName()).c_str(),
  // str.c_str());
  ...
  for (int i = 0; i < histo->GetNumBins(); ++i) {
    ...
    // xsec[i]/sum,(histo->GetY()[i]*norm)/(xsec[i]/sum) );
    fprintf(f, "%d\t%.8g\t%.8g\t%.8g\t%.8g\n", i, histo->GetX()[i] + 0.5 * histo->GetDelta(), histo->GetY()[i],
            histo_angle_rej->GetY()[i], ((histo->GetY()[i]) / (histo_angle_rej->GetY()[i])));
  }
  fclose(f);
  delete histo;
...
void help()
{
  ...
  std::cout << "  Model-level GeantV test for testing GeantV e-/e+ models for photoElectric e- emission." << std::endl;
