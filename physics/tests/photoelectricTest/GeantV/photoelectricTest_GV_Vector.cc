...
double sampleDistribution_Vector(int numSamples, double primaryEnergy, const MaterialCuts *matCut,
                                 Particle *primParticle, SauterGavrilaPhotoElectricModel *model, Hist *histo1, Hist *histo2);
...
int main(int argc, char *argv[]) {
    ...
    SauterGavrilaPhotoElectricModel *sauta=new SauterGavrilaPhotoElectricModel(photoElectricModelName, true);
    sauta->SetLowEnergyUsageLimit ( 0.01*geant::keV);
    sauta->SetHighEnergyUsageLimit(100.0*geant::GeV);
    //
    //*******************************************************************************************//
    ...
    //=========== Set the active regions of the model and one physics-parameter object ==========//
    // - Set the model to be active in region index 0
    (sauta->GetListActiveRegions()).resize(1); // one region
    (sauta->GetListActiveRegions())[0] = true; // make it active there
    // - Create one PhysicsParameters object (with defult values)
    PhysicsParameters *physPars_sauta = new PhysicsParameters();
    // - Set it to be active in region index 0
    (sauta->GetListActiveRegions()).resize(1);
    (sauta->GetListActiveRegions())[0] = true;
    // - Initialisation of the model
    sauta->Initialize();
    ...
    std::cout<< "   -------------------------------------------------------------------------------- "<<std::endl;
    std::cout<< "   Model name     =  " << sauta->GetName() << std::endl;
    std::cout<< "   -------------------------------------------------------------------------------- "<<std::endl;
...
//
    ...
    double timeInSec= sampleDistribution_Vector(numSamples, kineticEnergy, matCut, particle, sauta, histo_photoelectron_energy, histo_photoelectron_angular);
    //double timeInSec= sampleDistribution(numSamples, kineticEnergy, matCut, particle, sauta, histo_photoelectron_energy, histo_photoelectron_angular);
    ...
    sprintf(fileName,"e_energy_%s_%s_%sMeV.ascii",photoElectricModelName.c_str(),(matCut->GetMaterial()->GetName()).c_str(), str.c_str());
    ...
    //
    sprintf(fileName,"e_angle_%s_%s_%sMeV.ascii",photoElectricModelName.c_str(),(matCut->GetMaterial()->GetName()).c_str(), str.c_str());
    f     = fopen(fileName,"w");
...
double sampleDistribution_Vector(int numTracks, double primaryEnergy, const MaterialCuts *matCut, Particle *primParticle,
                                 SauterGavrilaPhotoElectricModel *sauta, Hist *histo1, Hist *histo2)
{
    ...
    //int numTracks=32;
    std::vector<LightTrack> primariesLT;
    //primaryLT=new LightTrack[numTracks];
    
    // init time
    
    for (long int i=0; i<numTracks; ++i) {
        //primaryLT[i]=new LightTrack;
        // we will use members:
        ...
        //std::cout<<"Set: "<<ekin<<", retrieve: "<<primaryLT.GetKinE()<<std::endl;
        ...
        primariesLT.push_back(primaryLT);
    }
    ...
    //int numSecs = sauta->SampleSecondariesVectorized<int>(primaryLT, td);
    clock_t  start_time = clock();
    sauta->SampleSecondariesVectorized(primariesLT, td);
    ...
    {
        double ePhotoElectron = secondaryLT[i].GetKinE();
        if (ePhotoElectron>0.0) {
                ePhotoElectron = log10((ekin - ePhotoElectron)*1000000);
                histo1->Fill(ePhotoElectron,1.0);
            
            }
        double costPhotoElectron = secondaryLT[i].GetDirZ();
        if(costPhotoElectron==0) std::cout<<"costPhotoElectron zero!\n";
