...
static struct option options[] = {
    ...
    {"cut-vale          (secondary production threshold value for all particles) - default: 0.1", required_argument, 0,
     'c'},
    {"use-rejection     (should rejection based sampling used ? )                - default: false", no_argument, 0,
     'r'},
...
int main(int argc, char *argv[])
{
  ...
  if (kineticEnergy <= minE) {
    std::cout << " *** Primary energy = " << kineticEnergy / geant::units::MeV
              << " [MeV] is <= minimum energy = " << minE / geant::units::MeV
              << " [MeV] so there is no secondary e- production at this energy!" << std::endl;
    return 0;
  }
...
void help()
{
  ...
  std::cout << "  Model-level GeantV test for testing GeantV e-/e+ model for ionization." << std::endl;
