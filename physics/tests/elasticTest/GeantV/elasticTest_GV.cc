...
double sampleDistribution(double numSamples, double primaryEnergy, Isotope *isotope, Particle *primParticle,
                          HadronicFinalStateModel *elModel, Hist *h1);
//***********************************************************************************************//
...
int main(int argc, char *argv[])
{
  ...
  //  worldl.SetTrackingMediumPtr(new vecgeom::Medium((matDetector->GetName()).c_str(),new vecgeom::Material(),new
  //  double()));
...
void help()
{
  ...
  std::cout << "  Model-level GeantV test for testing GeantV Bethe-Heitler model for e-/e+"
            << "  pair production by photons." << std::endl;
