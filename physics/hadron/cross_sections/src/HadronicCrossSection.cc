...
HadronicCrossSection::HadronicCrossSection()
    : fName(""), fMinEnergy(0.0), fMaxEnergy(100000.0), fMinTargetZ(1), fMaxTargetZ(120), fMinTargetN(1),
      fMaxTargetN(300)
{
}

HadronicCrossSection::HadronicCrossSection(const std::string name, const std::vector<int> &projectilecodevec,
                                           const double minenergy, const double maxenergy, const double mintargetz,
                                           const double maxtargetz, const double mintargetn, const double maxtargetn)
    : fName(name), fMinEnergy(minenergy), fMaxEnergy(maxenergy), fMinTargetZ(mintargetz), fMaxTargetZ(maxtargetz),
...
HadronicCrossSection::HadronicCrossSection(const HadronicCrossSection &other)
    : fName(other.fName), fMinEnergy(other.fMinEnergy), fMaxEnergy(other.fMaxEnergy), fMinTargetZ(other.fMinTargetZ),
