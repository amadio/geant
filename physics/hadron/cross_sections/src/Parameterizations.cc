...
double GetHadronNucleonXscPDG(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  double dumy0 = Math::Log(sMand / s0);
  double term1 = B * dumy0 * dumy0;
  double term2 = Math::Pow(sMand, -eta1);
  double term3 = Math::Pow(sMand, -eta2);
  ...
  if (particlePDG == 2112) { // proton-neutron fit
    if (proton) {
      NucleonTotalXsc = Zt * (35.80 + term1 + 40.15 * term2 - 30. * term3);
    } // on p
    if (neutron) {
      NucleonTotalXsc = Nt * (35.45 + term1 + 42.53 * term2 - 33.34 * term3);
    } // on n pp for nn
  } else if (particlePDG == 2212) {
    if (proton) {
      NucleonTotalXsc = Zt * (35.45 + term1 + 42.53 * term2 - 33.34 * term3);
    }
    if (neutron) {
      NucleonTotalXsc = Nt * (35.80 + term1 + 40.15 * term2 - 30. * term3);
    }
    ...
    if (proton) {
      NucleonTotalXsc = Zt * (17.91 + term1 + 7.14 * term2 - 13.45 * term3);
    }
    if (neutron) {
      NucleonTotalXsc = Nt * (17.87 + term1 + 5.17 * term2 - 7.23 * term3);
    }
  } else if (particlePDG == -321) {
    if (proton) {
      NucleonTotalXsc = Zt * (17.91 + term1 + 7.14 * term2 + 13.45 * term3);
...
double GetHadronNucleonTotalXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  if (particlePDG == 2112 && pORn) {
    if (pLab >= 373.) {
      ...
      NucleonTotalXsc = A0 + B0 * Math::Log(pE) - 11 + 103 * Math::Pow(sMand, -0.165); // mb
    } else if (pLab >= 10.) {
      B0 = 7.5;
      A0 = 100. - B0 * Math::Log(3.0e7);
    ...
    } else // pLab < 10 GeV/c
    {
      ...
      {
        if (pLab < 0.4) {
          NucleonTotalXsc = 23 + 50 * (Math::Pow(Math::Log(0.73 / pLab), 3.5));
        } else if (pLab < 0.73) {
          NucleonTotalXsc = 23 + 50 * (Math::Pow(Math::Log(0.73 / pLab), 3.5));
        } else if (pLab < 1.05) {
          double lp73     = Math::Log(pLab / 0.73);
          NucleonTotalXsc = 23 + 40 * lp73 * lp73;
        } else // 1.05 - 10 GeV/c
        {
          NucleonTotalXsc = 39.0 + 75 * (pLab - 1.2) / (Math::Pow(pLab, 3.0) + 0.15);
        }
      }
      if (proton) // pn to be np
      {
        if (pLab < 0.02) {
          NucleonTotalXsc = 4100 + 30 * Math::Pow(Math::Log(1.3 / pLab), 3.6); // was as pLab < 0.8
        } else if (pLab < 0.8) {
          NucleonTotalXsc = 33 + 30 * Math::Pow(Math::Log(pLab / 1.3), 4.0);
        } else if (pLab < 1.05) {
          NucleonTotalXsc = 33 + 30 * Math::Pow(Math::Log(pLab / 0.95), 2.0);
        } else if (pLab < 1.4) {
          NucleonTotalXsc = 33 + 30 * Math::Pow(Math::Log(pLab / 0.95), 2.0);
        } else // 1.4 < pLab < 10.  )
        {
          NucleonTotalXsc = 33.3 + 20.8 * (Math::Pow(pLab, 2.0) - 1.35) / (Math::Pow(pLab, 2.50) + 0.95);
        }
      }
    }
  ...
  {
    if (proton) {
      NucleonTotalXsc = 35.45 + B * Math::Pow(Math::Log(sMand / s0), 2.) + 42.53 * Math::Pow(sMand, -eta1) +
                        33.34 * Math::Pow(sMand, -eta2);
    }
    if (neutron) // ???
    {
      NucleonTotalXsc = 35.80 + B * Math::Pow(Math::Log(sMand / s0), 2.) + 40.15 * Math::Pow(sMand, -eta1) +
  ...
  } else if (particlePDG == 211 && pORn) // pi+ /////////////////////////////////////////////
  {
    if (proton) // pi+ p
    {
      if (pLab < 0.28) {
        ...
        double lp77     = Math::Log(pLab / 0.77);
      ...
      {
        NucleonTotalXsc = 10.6 + 2. * Math::Log(pE) + 25 * Math::Pow(pE, -0.43); // ns original
      } else                                                                     //  pLab > 100 // my
      {
        NucleonTotalXsc = GetHadronNucleonXscPDG(particlePDG, mass, energyKin, targetPDG) / geant::units::millibarn;
      }
    ...
    if (neutron) // pi+ n = pi- p??
    {
      ...
      } else if (pLab < 0.395676) // first peak
      {
        ...
        NucleonTotalXsc = 26 + 110 * (Math::Log(pLab / 0.48)) * (Math::Log(pLab / 0.48));
      } else if (pLab < 0.65) {
        NucleonTotalXsc = 26 + 110 * (Math::Log(pLab / 0.48)) * (Math::Log(pLab / 0.48));
      } else if (pLab < 0.72) {
        NucleonTotalXsc = 36.1 + 10 * Math::Exp(-(pLab - 0.72) * (pLab - 0.72) / 0.06 / 0.06) +
      ...
      {
        NucleonTotalXsc = 36.1 + 0.079 - 4.313 * Math::Log(pLab) +
                          3 * Math::Exp(-(pLab - 2.1) * (pLab - 2.1) / 0.4 / 0.4) +
  ...
  } else if (particlePDG == -211 && pORn) /// pi- ////////////////////////////////////////////
  {
    ...
    if (proton) // pi- p
    {
      ...
      } else if (pLab < 0.395676) // first peak
      {
        ...
        double lp48     = Math::Log(pLab / 0.48);
    ...
    NucleonTotalXsc = 35.20 + B * Math::Pow(Math::Log(sMand / s0), 2.) - 199. * Math::Pow(sMand, -eta1) +
                      264. * Math::Pow(sMand, -eta2);
  } else if (particlePDG == 22 && pORn) // modify later on
  {
    NucleonTotalXsc = 0.0 + B * Math::Pow(Math::Log(sMand / s0), 2.) +
...
double GetHadronNucleonInelasticXscNS(int particlePDG, double mass, double energyKin, int targetPDG)
{
  ...
  if (particlePDG == 2112 && pORn) {
    if (pLab >= 373.) {
      ...
      NucleonElasticXsc = 6.5 + 0.308 * Math::Pow(Math::Log(sMand / 400.), 1.65) + 9.19 * Math::Pow(sMand, -0.458);
    } else if (pLab >= 100.) {
      B0 = 7.5;
      A0 = 100. - B0 * Math::Log(3.0e7);

      NucleonTotalXsc = A0 + B0 * Math::Log(pE) - 11
                        // + 103*Math::Pow(2*0.93827*pE + pM*pM+0.93827*0.93827,-0.165);        //  mb
                        + 103 * Math::Pow(sMand, -0.165); //  mb

      NucleonElasticXsc = 5.53 + 0.308 * Math::Pow(Math::Log(sMand / 28.9), 1.1) + 9.19 * Math::Pow(sMand, -0.458);
    } else if (pLab >= 10.) {
      B0 = 7.5;
      A0 = 100. - B0 * Math::Log(3.0e7);
    ...
    } else // pLab < 10 GeV/c
    {
      ...
      if (proton) // pn to be np
      {
        if (pLab < 0.02) {
          ...
          double l5p        = Math::Log(0.511 / pLab);
          NucleonTotalXsc   = 33 + 30 * Math::Pow(Math::Log(pLab / 0.95), 2.0);
  ...
  } else if (particlePDG == 211 && pORn) // pi+ /////////////////////////////////////////////
  {
    if (proton) // pi+ p
    {
      if (pLab < 0.28) {
        ...
        NucleonTotalXsc   = 88 * lp77 * lp77 + 14.9;
        NucleonElasticXsc = NucleonTotalXsc * Math::Exp(-3. * (pLab - 0.68));
      } else if (pLab < 1.15) {
        double lp77 = Math::Log(pLab / 0.77);
    ...
    if (proton) // pi- p
    {
      ...
      } else if (pLab < 2.6) // < 3.0) // ns original
      {
        NucleonTotalXsc = 36.1 + 0.079 - 4.313 * Math::Log(pLab) +
                          3 * Math::Exp(-(pLab - 2.1) * (pLab - 2.1) / 0.4 / 0.4) +
                          1.5 * Math::Exp(-(pLab - 1.4) * (pLab - 1.4) / 0.12 / 0.12);
        NucleonElasticXsc = 3. + 13. / pLab; // *Math::Log(pLab*6.79);
      } else                                 // mb
      {
        NucleonTotalXsc   = GetHadronNucleonXscPDG(particlePDG, mass, energyKin, targetPDG) / geant::units::millibarn;
        NucleonElasticXsc = 3. + 13. / pLab;
  ...
  } else if ((particlePDG == -321 || particlePDG == 310) && proton) // Kmp/K0p //////
  {
    if (pLab < pMin) {
      ...
      NucleonElasticXsc = 5.2 / psp + (1.1 * cofLogE * ld2 + 2.23) / (1. - .7 / sp + .075 / p4) + .004 / md +
                          0.005 / hd1 + 0.01 / hd2 + .15 / hd; // small peaks were added

      NucleonTotalXsc = 14. / psp + (1.1 * cofLogT * ld2 + 19.5) / (1. - .21 / sp + .52 / p4) + .006 / md + 0.01 / hd1 +
                        0.02 / hd2 + .20 / hd;
    }
  } else if ((particlePDG == -321 || particlePDG == 310) && neutron) // Kmn/K0n ///////
  {
