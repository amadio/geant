...
HadronicCrossSectionStore::HadronicCrossSectionStore(const HadronicCrossSectionStore &other) : fName(other.fName)
{
...
void HadronicCrossSectionStore::RegisterHadronicCrossSection(HadronicCrossSection *ptrhadxsec)
{
  if (ptrhadxsec) {
...
int HadronicCrossSectionStore::GetIndexFirstApplicableXsec(const int projectilecode,
                                                           ...
                                                           const Material * /*targetmaterial*/)
{
  ...
  for (int i = fHadXsecVec.size() - 1; i >= 0; i--) {
    if (fHadXsecVec[i] &&
        fHadXsecVec[i]->IsApplicable(projectilecode, projectilekineticenergy, targetelement->GetZ(),
                                     targetelement->GetA() / (geant::units::g / geant::units::mole))) {
      index = i;
      break;
    }
  }
...
double HadronicCrossSectionStore::GetIsotopeCrossSection(const int projectilecode, const double projectilekineticenergy,
                                                         const double projectilemass, const Isotope *targetisotope,
                                                         const Element *targetelement, const Material *targetmaterial)
{
  ...
  if (index >= 0) {
    xsec = fHadXsecVec[index]->GetIsotopeCrossSection(projectilecode, projectilekineticenergy, projectilemass,
                                                      targetisotope->GetZ(), targetisotope->GetN());
  }
  return xsec;
}

double HadronicCrossSectionStore::GetElementCrossSection(const int projectilecode, const double projectilekineticenergy,
                                                         ...
                                                         const Material *targetmaterial)
{
  ...
  if (index >= 0) {
    ...
    for (size_t i = 0; i < isotopeVector.size(); i++) {
      ...
      double isotopeXsec = fHadXsecVec[index]->GetIsotopeCrossSection(
          projectilecode, projectilekineticenergy, projectilemass, isotopeVector[i]->GetZ(), isotopeVector[i]->GetN());
      if (isotopeXsec < 0.0) {
        xsec = -1.0;
        break;
      }
...
double HadronicCrossSectionStore::GetMacroscopicCrossSection(const int projectilecode,
                                                             const double projectilekineticenergy,
...
std::pair<int, int> HadronicCrossSectionStore::SampleTarget(const int projectilecode,
                                                            const double projectilekineticenergy,
