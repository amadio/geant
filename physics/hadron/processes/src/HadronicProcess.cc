...
HadronicProcess::HadronicProcess()
    : PhysicsProcess(""), fType(HadronicProcessType::kNotDefined), fXsecStore(nullptr), fModelStore(nullptr)
{
}

HadronicProcess::HadronicProcess(const std::string &name) : PhysicsProcess(name)
{
...
HadronicProcess::HadronicProcess(const std::string &name, const std::vector<int> &particlecodevec,
    ...
    : PhysicsProcess(true, false, isatrest, ForcedCondition::kNotForced, ProcessType::kHadronic, name), fType(type),
      fXsecStore(xsecstore), fModelStore(modelstore)
{
                                                   ...
                                                   const Particle *particle, double mass) const
{
  ...
  for (int iel = 0; iel < numElems; ++iel) {
    xsec += theAtomicNumDensityVector[iel] *
            GetAtomicCrossSection(particle->GetInternalCode(), kinenergy, mass, theElements[iel], mat);
  }

  return xsec;
}

double HadronicProcess::GetAtomicCrossSection(const int particlecode, const double particlekineticenergy,
                                              const double particlemass, const Element *targetelement,
                                              const Material *targetmaterial) const
{
  double xsec = -1.0;
  if (fXsecStore) {
    xsec = fXsecStore->GetElementCrossSection(particlecode, particlekineticenergy, particlemass, targetelement,
                                              targetmaterial);
  }
  return xsec;
}
...
int HadronicProcess::AtRestDoIt(LightTrack & /*track*/, geant::TaskData * /*td*/)
{
  return 0;
}
...
void HadronicProcess::AddModel(HadronicFinalStateModel *model)
{
  if (!model) {
    ...
    std::cerr << " *** WARNING: HadronicProcess::AddModel() \n"
              << "  Attempt to add HadronicFinalStateModel = nullptr to process with Name = " << GetName()
...
void HadronicProcess::AddCrossSection(HadronicCrossSection *xsection)
{
  if (!xsection) {
    ...
    std::cerr << " *** WARNING: HadronicProcess::AddCrossSection() \n"
              << "  Attempt to add HadronicCrossSection = nullptr to process with Name = " << GetName()
              << " that is assigned to particles:" << partNames << " is ignored." << std::endl;
  } else {
