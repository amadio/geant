...
int DiffuseElasticModel::SampleFinalState(LightTrack &track, Isotope *targetisotope, geant::TaskData *td)
{
  ...
  */
  ...
  if (cost > 1.0 || cost < -1.0) {
    std::cout << "G4HadronElastic WARNING (1 - cost)= " << 1 - cost << " after scattering of " << track.GetGVcode()
  ...
  if (eFinal <= GetLowEnergyUsageLimit()) {
    if (eFinal < 0.0) {
      std::cout << "G4HadronElastic WARNING Efinal= " << eFinal << " after scattering of " << track.GetGVcode()
                << " p(GeV/c)= " << plab / geant::units::GeV << " on an ion Z= " << targetisotope->GetZ()
                << " N= " << targetisotope->GetN() << std::endl;
    }
...
double DiffuseElasticModel::SampleInvariantT(double mass, double plab, Isotope *targetisotope, geant::TaskData *td)
{
  ...
  return -GeV2 * Math::Log(1.0 - td->fRndm->uniform() * q1) / bb;
}
