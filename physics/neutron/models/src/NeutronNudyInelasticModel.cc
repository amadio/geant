...
int NeutronNudyInelasticModel::SampleFinalState(LightTrack &track, Isotope *targetisotope, geant::TaskData *td)
{
  ...
  for (int i = 0; i != mtsize; ++i) {
    if (recopoint.fMtValues[idMt][i] != 2 && recopoint.fMtValues[idMt][i] != 18 
      && recopoint.fMtValues[idMt][i] != 19 && recopoint.fMtValues[idMt][i] != 20 
      && recopoint.fMtValues[idMt][i] != 21 && recopoint.fMtValues[idMt][i] != 38 
      && recopoint.fMtValues[idMt][i] != 102 && recopoint.fMtValues[idMt][i] != 5) {
  ...
  if (MT > 50 && MT < 92) {
    ...
    if (cosLab > 1.0 || cosLab < -1.0) {
      ...
      << track.GetGVcode()
      << " E(GeV)= " << eLab << " on an ion Z= " << targetisotope->GetZ()
  ...
  for (int i = 0; i != numSecondaries; ++i) {
    ...
    // problem in sampling 
    if (cosLab > 1.0) {
      std::cout << "GVNeutronNudyInelastic WARNING cost= " << cosLab << " after reaction of " 
      << track.GetGVcode()
      << " E(GeV)= " << eLab << " on an ion Z= " << targetisotope->GetZ()
      << " N= " << targetisotope->GetN() << " for MT = "<< MT << std::endl;
      cosLab = 1.0;
