...
int TabulatedDataManager::SampleInteraction(  const G4int imat, 
                                                const G4Track &atrack,
                                                int &reactionid) {    
  G4int     partIndex;   // GV particle index 
  G4double  kinEnergy;   // kinetic energy of the particle in GeV
...
void TabulatedDataManager::SampleFinalState(const int elementindex, 
                       const int reactionid, const G4Track &atrack, 
  ...
  int  partindex    = TPartIndex::I()->PartIndex(
  ...
  // (normaly a check would be good but not necessary: if(ebinindx<0 -> ...) 
     ...
     double corFactor = std::sqrt( kinEnergy*(kinEnergy+2.0*primMass) /
...
void TabulatedDataManager::SampleFinalStateAtRest(const int imat, 
...
void TabulatedDataManager::SampleDecayInFlight(const int partindex, 
                                               const G4Track &atrack, 
...
void TabulatedDataManager::RotateNewTrack(double oldxdir, double oldydir, 
                                       double oldzdir, G4ThreeVector &newdir){
     ...
     //     almost normalized!
     double delta = one5-half*(newdir.x()*newdir.x() + newdir.y()*newdir.y() +
                                  + newdir.z()*newdir.z() );  
...
void TabulatedDataManager::RotateTrack(G4ThreeVector &newdir, double theta,
                                       double phi){
     const double one  = 1.0;  
     const double zero = 0.0; 
     const double amin = 1.0e-10; 
     ...
     double sinTheta0 = std::sqrt(newdir.x()*newdir.x() + 
                                      newdir.y()*newdir.y());
     double cosPhi0;
     double sinPhi0;
