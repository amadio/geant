#ifndef GUCLHEPRandom_H
#define GUCLHEPRandom_H
...
inline FQUALIFIER double shoot()
{
  ...
  do {
    ...
    v1 = 2.0 * GUCLHEPRandFlat::flat() - 1.0;
    v2 = 2.0 * GUCLHEPRandFlat::flat() - 1.0;
    r = v1 * v1 + v2 * v2;
  } while (r > 1.0);
...
inline FQUALIFIER double gammln(double xx)
{
...
inline FQUALIFIER static double normal()
{
  double r;
  double v1, v2, fac;
  do {
...
inline FQUALIFIER long shoot(double xm)
{
  ...
  if (xm < 12.0) {
    ...
    do {
      do {
        ...
        y = tan(3.14159265358979323846 * GUCLHEPRandFlat::flat());
        em = sq * y + xm;
      } while (em < 0.0);
      em = floor(em);
      t = 0.9 * (1.0 + y * y) * exp(em * alxm - gammln(em + 1.0) - g1);
    } while (GUCLHEPRandFlat::flat() > t);
...
inline FQUALIFIER double genGamma(double a, double lambda)
{
  static double aa = -1.0, aaa = -1.0, b, c, d, e, r, s, si, ss, q0, q1 = 0.0416666664, q2 = 0.0208333723,
                q3 = 0.0079849875, q4 = 0.0015746717, q5 = -0.0003349403, q6 = 0.0003340332, q7 = 0.0006053049,
                q8 = -0.0004701849, q9 = 0.0001710320, a1 = 0.333333333, a2 = -0.249999949, a3 = 0.199999867,
                a4 = -0.166677482, a5 = 0.142873973, a6 = -0.124385581, a7 = 0.110368310, a8 = -0.112750886,
                a9 = 0.104089866, e1 = 1.000000000, e2 = 0.499999994, e3 = 0.166666848, e4 = 0.041664508,
                e5 = 0.008345522, e6 = 0.001353826, e7 = 0.000247453;
  ...
  if (a < 1.0) {                 // CASE A: Acceptance rejection algorithm gs
    ...
    for (;;) {
      p = b * GUCLHEPRandFlat::flat();
      if (p <= 1.0) { // Step 2. Case gds <= 1
        gds = exp(log(p) / a);
        if (log(GUCLHEPRandFlat::flat()) <= -gds)
          return (gds / lambda);
      }
      else { // Step 3. Case gds > 1
        gds = -log((b - p) / a);
        if (log(GUCLHEPRandFlat::flat()) <= ((a - 1.0) * log(gds))) {
          return (gds / lambda);
        }
      }
    }
    ...
    u = GUCLHEPRandFlat::flat(); // Step 3. Uniform random number
    if (d * u <= t * t * t)
    ...
    if (x > 0.0) // Step 5. Calculation of q
    {
      ...
      if (fabs(v) > 0.25) {
        ...
        q = q0 +
            0.5 * t * t * ((((((((a9 * v + a8) * v + a7) * v + a6) * v + a5) * v + a4) * v + a3) * v + a2) * v + a1) *
                v;
    ...
    for (;;) { // Step 8. Double exponential deviate t
      do {
        e = -log(GUCLHEPRandFlat::flat());
        u = GUCLHEPRandFlat::flat();
        u = u + u - 1.0;
        sign_u = (u > 0) ? 1.0 : -1.0;
        t = b + (e * si) * sign_u;
...
inline FQUALIFIER double shoot(double k, double lambda) { return genGamma(k, lambda); }
};
