                                                                              ...
                                                                              double &gammaEnergy, double &sinTheta)
{
  ...
  if (isElectron && x0 < 0.97 && ((kineticEnergy > epeaklimit) || (kineticEnergy < elowlimit))) {
    G4double ylim = math::Min(fDataSB[Z].Value(0.97, 4 * math::Log(10.)), 1.1 * fDataSB[Z].Value(0.97, y));
    if (ylim > vmax) {
      vmax = ylim;
    }
  }
...
VECCORE_CUDA_HOST double BremSeltzerBerger::GetG4CrossSection(int Z, double kineticEnergy)
{
  ...
  // constant
  G4double bremFactor = fine_structure_const * classic_electr_radius * classic_electr_radius * (16. / 3.);
