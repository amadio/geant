...
VECCORE_CUDA_HOST double UrbanWentzelVI::CrossSectionPerAtom(int AtomicNumber, double KineticEnergy)
{
  ...
  static const double epsfactor = 2.*electron_mass_c2*
       electron_mass_c2*Bohr_radius*Bohr_radius/hbarc*hbarc;

  static const double beta2lim = Tlim*(Tlim+2.*electron_mass_c2)/
       ((Tlim+electron_mass_c2)*(Tlim+electron_mass_c2));
  static const double bg2lim   = Tlim*(Tlim+2.*electron_mass_c2)/
       (electron_mass_c2*electron_mass_c2);

  static const double sig0[15] = {0.267  ,  0.5922,  2.653, 6.235,
                                  11.69  , 13.24  , 16.12, 23.00,
                                  35.13  , 39.95  , 50.85, 67.19,
                                  91.15  , 104.4  , 113.1}; //in [barn]

  static const double Tdat[22] = { 0.0001,  0.0002,  0.0004,  0.0007,
  ...
  static const double Zdat[15] = { 4.,  6., 13., 20., 26., 29., 32., 38.,47.,
                                     50., 56., 64., 74., 79., 82. };

  // corr. factors for e-/e+ lambda for T <= Tlim
  static const double celectron[15][22] =
  ...
  double beta2 = eKineticEnergy*(eTotalEnergy+electron_mass_c2)
                                 /(eTotalEnergy*eTotalEnergy);
  double bg2   = eKineticEnergy*(eTotalEnergy+electron_mass_c2)
                                 /(electron_mass_c2*electron_mass_c2);

  double eps = epsfactor*bg2/Z23;

  if     (eps<epsmin)  sigma = 2.*eps*eps;
  else if(eps<epsmax)  sigma = math::Log(1.+2.*eps)-2.*eps/(1.+2.*eps);
  else                 sigma = math::Log(2.*eps)-1.+1./eps;
  ...
  double ratZ = (AtomicNumber-ZZ1)*(AtomicNumber+ZZ1)/
                  ((ZZ2-ZZ1)*(ZZ2+ZZ1));
