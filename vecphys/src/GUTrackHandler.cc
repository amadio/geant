...
GUTrackHandler::GUTrackHandler() : fNumberOfTracks(0), fTrack_aos(0), fBuffer(0) {}

GUTrackHandler::GUTrackHandler(size_t nTracks) { Allocate(nTracks); }

GUTrackHandler::~GUTrackHandler() { Deallocate(); }
...
void GUTrackHandler::Allocate(size_t nTracks)
{
  ...
  if (fNumberOfTracks > 0) {
    ...
    // allocation for soa
    char *soaBuffer = (char *)_mm_malloc(memSizeAlloc, // sizeof(int)+fNumberOfTracks*sizeof(GUTrack),
                                         blockSize);
