...
int main(int argc, char *argv[])
{
  ...
  if (argc >= 6) {
    ...
    if (emProcess < -1 || emProcess >= kNumberPhysicsProcess) {
      std::cout << "  Illegal vector physics process " << emProcess << "! Should be [-1:" << kNumberPhysicsProcess - 1
                << "]" << std::endl;
      exit(0);
    }
  }
  ...
  if (argc >= 8) {
    ...
    if (materialMode < 0 || materialMode > 1) {
      std::cout << "  Illegal material mode " << materialMode << "! Should be [0:1] (see " << std::endl;
      exit(0);
    }
  }
  ...
  if (status == 1)
    std::cout << "RunProcessBenchmark Failed" << std::endl;
  return status;
}
