...
ProcessBenchmarker::ProcessBenchmarker()
    : fNtracks(4992),
      ...
      fMinP(defaultMinP),  
      fMaxP(defaultMaxP),
      fEmProcess(GUPhysicsProcessIndex::kNullProcess),
      fMaterialMode(0),   
      fRunMode(-1)
{
...
int ProcessBenchmarker::RunBenchmark()
{
  printf(" Run ProcessBenchmarker with arguments: %d %d %f %f %d %d %d\n",
	 fNtracks,fRepetitions,fMinP,fMaxP,fEmProcess,fRunMode,fMaterialMode);
  printf(" Ntracks       = %d\n",fNtracks);
...
void ProcessBenchmarker::RunScalar() 
{
  ...
  for(int k = 0 ; k < kNumberPhysicsProcess ; ++k) elapsedTotal[k] = 0.; 
