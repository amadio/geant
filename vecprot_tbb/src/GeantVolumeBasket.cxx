...
void GeantVolumeBasket::ComputeTransportLength(int ntracks, int *trackin) {
  ...
  for (itr = 0; itr < ntracks; itr++) {
    ...
    if (track->charge) {
      ...
      nav->FindNextBoundaryAndStep(pstep, false);
      track->safety = 0.;
      ...
      if (nav->IsOnBoundary() && track->snext < 2. * gTolerance) {
        ...
        nav->FindNextBoundaryAndStep(1.E30, false);
        track->nextpath->InitFromNavigator(nav);
        track->snext += nav->GetStep();
      }
...
void GeantVolumeBasket::PropagateTracks(int ntracks, int *trackin, int &nout, int *trackout, int &ntodo, int *tracktodo,
  ...
  for (int itr = 0; itr < ntracks; itr++) {
    ...
    if (step < safety) {
      track->izero = 0;
      track->PropagateInField(step, false, trackin[itr]);
    ...
    if (0.25 * c * snext < 1E-6 && snext < 1E-3 && snext < step - 1E-6) {
      ...
      if (track->izero > 10)
        snext = 1.E-3;
      basket = track->PropagateInField(snext + 10 * gTolerance, true, trackin[itr]);
      if (snext < 1.E-6)
        track->izero++;
      else
        track->izero = 0;
    ...
    if (safety < gTolerance) {
      ...
      if (track->izero > 10)
        safety = 0.5 * snext;
      basket = track->PropagateInField(safety, true, trackin[itr]);
    } else {
      if (track->izero > 10) {
      ...
      basket = track->PropagateInField(safety, false, trackin[itr]);
    }
