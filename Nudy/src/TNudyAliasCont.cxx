...
//_______________________________________________________________________________
void TNudyAliasCont::BuildIntermediate(TNudyAliasCont *dists,const Int_t len) {
  ...
  for(i=0;i<len-1;i++) {
    ...
    for(j=1;j<dists[i].fLen;j++){
      integral->SetAt(0.5*(dists[i].fX->At(j) - dists[i].fX->At(j-1))*(dists[i].fP->At(j)+dists[i].fP->At(j-1)),j);
      integral->SetAt(integral->At(j) + integral->At(j-1),j);
    }
    //Store bin areas of 
    for(j=1;j<dists[i+1].fLen;j++){
      integralp1->SetAt(0.5*(dists[i+1].fX->At(j) - dists[i+1].fX->At(j-1))*(dists[i+1].fP->At(j)+dists[i+1].fP->At(j-1)),j);
 ...
 */     //}
    ...
    for(j=1;j<dists[i].fLen;j++) {
      ...
      if(j>0){
        h += 0.5 * (dists[i].fInterX->At(j) - dists[i].fInterX->At(j-1)) * (dists[i].fInterP->At(j) + dists[i].fInterP->At(j-1));
      }
    }
    for(j=0; j < dists[i].fLen; j++)
