...
TNudySampling::TNudySampling(Particle* particle, TNudyEndfRecoPoint *recoPoint){
  ...
  //std::cout <<"sampling sigmaTotal "<< recoPoint->GetSigmaTotal(0,kineticE) << std::endl;
  ...
  for(unsigned int crsp = 0; crsp < recoPoint->MtValues[elemId].size(); crsp++){
    ...
    //std::cout<<recoPoint->MtValues[elemId][crsp]<<"  "<< crs[crsp] <<"  "<< recoPoint->GetSigmaPartial(elemId,crsp,kineticE) <<"  "<< recoPoint->GetSigmaTotal(elemId,kineticE) << std::endl;
  }  
  do
  {
    double sum1 = 0;
...
void TNudySampling::GetSecParameter(Particle* particle, TNudyEndfRecoPoint *recoPoint){
  ...
  switch (MF) {
    case 4:
    {
      ...
      //secEnergyCM = recoPoint->GetEnergy5(elemId, MT, kineticE);
      secEnergyLab = fe3;
      //std::cout<<"secEnergyLab = "<< secEnergyLab << std::endl;
      //secEnergyLab = TNudyCore::Instance()->cmToLabInelasticE(secEnergyCM, kineticE, cosCM, particle[elemId].mass);
      if(LCT == 2){
	cosLab = TNudyCore::Instance()->cmToLabInelasticCosT(secEnergyLab, secEnergyCM, kineticE, cosCM, particle[elemId].mass);
      }else if(LCT == 1){
	...
	case 2:
	{
	  cosCM = recoPoint->GetCos6(elemId, MT, law, kineticE) ;
	  double fm1 = 1 ;
	  double fm2 = particle[elemId].mass ;
	  double fm3 = particle[elemId].mass - residueA ;
	  double fm4 = residueA ;
	...
	case 6:
	case 7:
	  cosLab = recoPoint->GetCos6(elemId, MT, law, kineticE);
	  std::cout<< cosLab << std::endl;
	  secEnergyLab = recoPoint->GetEnergy6(elemId, MT, law, kineticE);
