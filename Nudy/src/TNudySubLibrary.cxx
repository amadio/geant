...
void TNudySubLibrary::ListModels(){
	...
	while((model = (TVNudyModel*)modelIter.Next())){
		printf(" %10s | A=%-3d Z=%-3d ISO=%-1d Temperature=%ldK  Reaction=%s | %s\n ",model->GetMaterialName(),model->GetA(),model->GetZ(),model->GetISO(),model->GetTemp(),TNudyCore::Instance()->ExpandReaction(model->GetReaction()),model->GetTitle());
	}
}
  ...
  while((obj = (TNamed*)rIter.Next())){
    ...
    if(!(gDirectory->FindKey(TNudyCore::Instance()->GetKey(mat,reac,temp)))){
    	if(mat->ENDFCode() == 120240)
    		Info("ReadMat","Reading materials %s MAT - %d %s into Sublibrary Tape %s",TNudyCore::Instance()->GetKey(mat,reac,temp),material->GetMAT(),material->GetName(),GetName());
...
//      fIndex->Add(newModel);
      ...
      Warning("ReadFile","Model %s - %s MAT - %d %s already present in file %s ",TNudyCore::Instance()->GetKey(mat,reac,temp),mat->GetName(),material->GetMAT(),material->GetName(),gFile->GetName());
