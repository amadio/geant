...
TNudyEndfMat::TNudyEndfMat(Int_t mat, Int_t za, Double_t awr, 
			   Int_t lrp, Bool_t lfi, Int_t nlib, Int_t nmod) :
  fMAT(mat),
  fZA(za),
  fAWR(awr),
  fLRP(lrp),
...
void TNudyEndfMat::SetDesc(const TString desc, Int_t i) 
{
  if(i<0 || i>=fNWDm5) {
    Error("GetDesc","index %d out of bounds [%d,%d]",0,fNWDm5);
  } else { 
...
void TNudyEndfMat::Print(Option_t *op) const
{
  ...
  if(sop.Contains("h") || sop.Contains("a")) {
    ...
    if(sop.Contains("F")) {
      ...
      std::cout << std::setprecision(4) << std::scientific << std::setfill(' ') 
	   << std::setw(11) << fAWI          << std::setw(11) << fEMAX
	   << std::setw(11) << fLREL         << std::setw(11) << 0
	   << std::setw(11) << fNSUB         << std::setw(11) << fNVER
	   << std::endl;

      std::cout << std::setprecision(4) << std::scientific << std::setfill(' ') 
	   << std::setw(11) << fTEMP         << std::setw(11) << 0.0
	   << std::setw(11) << fLDRV         << std::setw(11) << 0
	   << std::setw(11) << fNWDm5+5      << std::setw(11) << fNXC
	   << std::endl;

      std::cout << fZSYMAM << fALAB << fEDATE << " " << fAUTH << std::endl;
      
      std::cout << " " << fREF << fDDATE  << " " << fRDATE
	   << std::setw(20)    << fENDATE << std::endl;
