            ...
            for (int lseries = 0; lseries < NLS; lseries++) {
              ...
              if (LRF == 1 || LRF == 2) {
                for (int ii = 0; ii < NRS[lseries]; ii++) {
                  ...
                  //       std::cout<<Er[nrsl0 + ii]<<"  "<<PhiEr[nrsl0 + ii]<< " "<< ShiftEr[cueMat]<<"  "<<cueMat<<"
                  //       "<<nrsl0<<std::endl;
                  //	std::cout<<Gamma_r[ii]<<" Gamma_n  "<<Gamma_n[ii]<<" Gamma_g "<<Gamma_g[ii]<<" Gamma_f
                  //"<<Gamma_f[ii]<<std::endl;
                ...
                for (int ii = 0; ii < NRS[lseries]; ii++) {
                  ...
                  ShiftEr.push_back(calcShift(std::fabs(list->GetLIST(ii * 6 + 0)), lseries));
                  Gamma_n[nrsl0 + ii] = Gamma_n[nrsl0 + ii] / PhiEr[nrsl0 + ii];
                  //       std::cout<<lseries<<"  "<<Er[nrsl0 + ii]<<"  "<<PhiEr[nrsl0 + ii]<< " "<< ShiftEr[cueMat]<<"
                  //       "<<cueMat<<"  "<<nrsl0<<std::endl;
              ...
              for (int lseries = 0; lseries < NLS2; lseries++) {
                ...
                for (int jseries = 0; jseries < NJS; jseries++) {
                  ...
                  for (int ii = 0; ii < NE; ii++) {                         // line 6 onwards data
                    ...
                    //	  std::cout<<list3->GetLIST(0)<<"  "<<list3->GetL2()<<"  "<<list3->GetLIST(6+ii)<<"
                    //"<<list->GetLIST(ii)<<std::endl;
                  }
                } // complete NJS reading
              }
              eLo2 = eLo;
...
//------------------------------------------------------------------------------------------------------
double TNudyEndfRecoPoint::recursionLinearFile3(double x1, double x2, double sig1, double sig2, std::vector<double> ene,
                                                std::vector<double> sig)
{
  ...
  // std::cout << x1 <<"  "<< x2 <<"  "<< mid <<"  "<< sig1 <<"  "<< sig2 <<"  "<< siga<<"  "<<sigmid1<<"
  // "<<std::fabs((siga - sigmid1)/siga)/*<<" NR "<< NR <<" NBT "<< NBT[0] <<" INT1 "<< INT1[0]*/<< std::endl;
  if (siga == 0.0) return 0;
  // std::cout<<x1<<"  "<<x2 <<"  "<< sig1 <<"  "<< sig2 <<"  "<<std::setprecision(12)<<
  // std::min(sig2,sig1)/std::max(sig2, sig1) << std::endl;
  if (x1 == x2 || std::fabs((siga - sigmid1) / siga) <= sigDiff) {
    //  if(x1==x2 || (std::fabs((siga - sigmid1)/siga)<=sigDiff && (std::min(sig2,sig1) > 0 &&
    //  std::min(sig2,sig1)/std::max(sig2, sig1) > 0.9) )){
    return 0;
  } else {
    eLinearFile3.push_back(mid);
    xLinearFile3.push_back(siga);
  }
  //  std::cout<<" linear " << mid <<"  "<<x1<< "  "<<x2 << "  "<< siga <<"  "<< sigmid1 <<"  "<< (siga -
  //  sigmid1)/siga<< std::endl;
...
//------------------------------------------------------------------------------------------------------
double TNudyEndfRecoPoint::addFile3Resonance(double &x1, double &x2, std::vector<double> &x3, std::vector<double> &x4)
{
  int linsize = x3.size();
  ...
  for (int cr1 = 0; cr1 < linsize; cr1++) {
    if (x3[cr1] >= x1 && x3[cr1] <= x2) {
      ...
      x4[cr1] += crs;
      // std::cout<<x3[cr1]<<" add  "<<crs<<"  "<<x4[cr1]<<std::endl;
    }
  }
  return 0;
}
...
//------------------------------------------------------------------------------------------------------
double TNudyEndfRecoPoint::insertFile3(std::vector<double> &x1, std::vector<double> &x2)
{
  int size1 = eLinearFile3.size();
...
//------------------------------------------------------------------------------------------------------
double TNudyEndfRecoPoint::insertFile3High(std::vector<double> &x1, std::vector<double> &x2)
{
  if (flagResolve != 0) {
  ...
  for (int lcnt = 0; lcnt < NLS; lcnt++) {
    ...
    do {
      ...
      for (int r = nrs; r < nrend; r++) {
        //       std::cout<<" jVal "<<jVal<<"  "<<  J[r] <<"  "<< MissingJ[l[lcnt]][jVal] <<"  "<< nrs << "
        //       "<<NJValue[l[lcnt]]<<std::endl;
      ...
      for (int lcnt = 0; lcnt < NLS; lcnt++) {
        ...
        do {
          ...
          for (int r = nrs; r < nrend; r++) {
            ...
            //	  std::cout<< r <<"  "<< GJ[r] << "  "<< MissingJ[l[lcnt]][jVal]<< "  "<< NJValue[l[lcnt]]<< "  "<< x<<"
            //"<< Er[r] << std::endl;
          }
        ...
        //   std::cout<<phil<<"  "<<std::setprecision(8)<< x << "  "<<std::setprecision(10)<<sumCrsElastic << "
        //   "<<std::setprecision(10)<<sumCrsCapture<< "  "<<std::setprecision(10)<<sumCrsFission << std::endl;
      }
  ...
  case 2: {
    ...
    case 1: {
      for (int lcnt = 0; lcnt < NLS; lcnt++) {
        ...
        //    std::cout<<"phi "<< phil <<" shift "<<calcShift(x, lVal)<<" p "<< calcPene(x, lVal) <<" E "<< x <<
        //    std::endl;
        //     for (int jVal = 0; jVal < Jnum; jVal++) {
        //    for (int jVal = 0; jVal < NJValue[l[lcnt]]; jVal++) {
        jVal      = 0;
        ...
        do {
          ...
          for (int r = nrs; r < nrend; r++) {
            ...
            //	 std::cout<<"facCapture "<<facCapture<<"  "<< GJ[r] * fachNumCapture <<"  "<< x <<" Er "<< Er[r] <<" "
            //<< GJ[r]<< std::endl;
          }
          sumElastic += facElastic;
          sumCapture += facCapture;
          ...
          //	 std::cout<<"sumCapture "<< sumCapture <<" facCapture "<< facCapture <<" E "<< x <<" jVal "<< jVal<<" lVal
          //"<< lVal << std::endl;
          jVal += 1;
        } while (jVal < NJValue[l[lcnt]]);
        nrs = nrstemp;
        ...
        sumCrsCapture += pibyk2 * (sumCapture);
        sumCrsFission += pibyk2 * (sumFission);
        // std::cout<< x << "  "<<pibyk2 * (fac1 + sumElastic +sumElasticM) <<"  "<< pibyk2 * (sumCapture)
        //      <<"  "<< pibyk2 * (sumFission) << std::endl;
      }
      siga = sumCrsElastic;
      sigb = sumCrsCapture;
      sigc = sumCrsFission;
      // std::cout<<"enegy "<< x << "  "<<siga <<"  "<< sigb <<"  "<< sigc << std::endl;
    } break;
    case 2: {
      int nrsj = 0;
      nrs      = 0;
...
//------------------------------------------------------------------------------------------------------
double TNudyEndfRecoPoint::recursionLinear(double x1, double x2, double sig1, double sig2, double sig3, double sig4,
                                           double sig5, double sig6)
{
  ...
  if (elRatio <= sigDiff && capRatio <= sigDiff && fisRatio <= sigDiff) {
    ...
    // std::cout<<elRatio <<"  "<<capRatio<<"  "<< fisRatio <<"  "<<std::setprecision(6)<< mid <<"  "<< x1 <<"  "<< x2
    // << std::endl;
    // if(elRatio > sigDiff){
    eLinElastic.push_back(mid);
    xLinElastic.push_back(siga);
    //  std::cout<<" el1 " << mid <<"  "<< x1 <<"  "<< x2 <<"  "<< siga <<"  "<< sigb <<"  "<< sigc <<"  "<< elRatio <<"
    //  "<< capRatio<<"  "<<fisRatio << std::endl;
    //}
    // if(capRatio > sigDiff){
    eLinCapture.push_back(mid);
    xLinCapture.push_back(sigb);
    //  std::cout<<" cap " << mid <<"  "<< x1 <<"  "<< x2 <<"  "<< siga <<"  "<< sigb <<"  "<< sigc <<"  "<< elRatio <<"
    //  "<< capRatio<<"  "<<fisRatio << std::endl;
    //}
    // if(fisRatio > sigDiff){
    eLinFission.push_back(mid);
    xLinFission.push_back(sigc);
    //  std::cout<<" fis " << mid <<"  "<< x1 <<"  "<< x2 <<"  "<< siga <<"  "<< sigb <<"  "<< sigc <<"  "<< elRatio <<"
    //  "<< capRatio<<"  "<<fisRatio << std::endl;
    //}
  }
  ...
  if (LRU == 1) {
    for (int j = 0; j < NLS; j++) {
      ...
      for (int i = nrs; i < nrs + NRS[l[j]]; i++) {
        ...
        //	      std::cout<<"L = "<< j <<" r " << i <<" energy = "<< Er[i] <<" nrs "<< nrs << " nrsl "<< nrsl <<" Er
        // size "<< Er.size() << std::endl;
    ...
    */
    ...
    //	std::cout<<"before "<< eLinElastic[intLinLru1] <<"  "<<eLinElastic[eLinElastic.size()-1]<<"  "<<intLinLru1<<"
    //"<<eLinElastic.size()<<std::endl;
    // recursionLinear(eLinElastic[intLinLru1], eLinElastic[eLinElastic.size()-1], xLinElastic[intLinLru1],
    // xLinElastic[eLinElastic.size()-1]);
    //  for (int i =0;i<eLinCapture.size(); i++)
        ...
        /*
          ...
          while ((sec = (TNudyEndfSec *)secIter.Next())) {
            ...
            if(MT != 3 && MT != 4 && MT != 27 && MT != 19 && MT != 20 && MT != 21 && MT != 38 && MT != 101 && MT < 250){
              ...
              if(MT==1)header->SetCont(header->GetC1(),header->GetC2(),header->GetL1(),header->GetL2(),NR,(int)eneUni[0].size());
              TNudyEndfTab1 *tab1 = (TNudyEndfTab1 *)(sec->GetRecords()->At(0));
              if(MT==1){
                for(int cr=0; cr < NR ; cr ++){
            tab1->SetNBT((int)eneUni[0].size(),0);
            tab1->SetINT(2,0);
                }
                for(int crs=0; crs < (int)eneUni[0].size() ; crs ++){
            tab1->SetX(eneUni[0][crs],crs);
            tab1->SetY(sigUniT[0][crs],crs);
                }
              continue;
              }
              if(MT==MtValues[0][mtcount]){
                header->SetCont(header->GetC1(),header->GetC2(),header->GetL1(),header->GetL2(),NR,(int)sigUniOfMt[0][mtcount].size());
                tab1->SetNBT((int)sigUniOfMt[0][mtcount].size(),0);
                tab1->SetINT(2,0);
            for(int crs=0; crs < (int)sigUniOfMt[0][mtcount].size() ; crs ++){
              tab1->SetX(eneUni[0][energyLocMtId[ielemId][mtcount] + crs],crs);
              tab1->SetY(sigUniOfMt[0][mtcount][crs],crs);
              std::cout << eneUni[0][energyLocMtId[ielemId][mtcount] + crs] <<" "<< sigUniOfMt[0][mtcount][crs] <<
          std::endl;
            }
              }
                mtcount ++;
...
//------------------------------------------------------------------------------------------------------
void TNudyEndfRecoPoint::broadSigma(std::vector<double> &x1, std::vector<double> &x2, std::vector<double> &x3)
{
...
void TNudyEndfRecoPoint::fixupTotal(std::vector<double> &x1, std::vector<double> &x2)
{
  ...
  for (unsigned long i = 0; i < sigmaUniOfMts.size(); i++) {
    ...
    for (int j = 0; j < size; j++) {
      ...
      // std::cout <<"MT= "<< MtValues[elemId][i] <<"  "<<std::setprecision(12)<< x1[energyLocationMts[i]+j] << "  "<<
      // sigmaUniOfMts[i][j]<< std::endl;
    }
  }
  // */
