...
public:
  ...
  void GetMTF(int mtf[4]) const { FGetMTF(PASSCHARD(fLine), mtf PASSCHARL(fLine)); }

  void GetCONT(double c[2], int nl[4], int mtf[4]) const { FGetCONT(PASSCHARD(fLine), c, nl, mtf PASSCHARL(fLine)); }

  void GetFloat(double c[6]) const { FGetFloat(PASSCHARD(fLine), c PASSCHARL(fLine)); }

  void GetInt(int n[6]) const { FGetInt(PASSCHARD(fLine), n PASSCHARL(fLine)); }
