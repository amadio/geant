...
public:
  TNudyEndfMat();
  TNudyEndfMat(int mat, int za, double awr, char lrp, bool lfi, int nlib, int nmod);
  virtual ~TNudyEndfMat();
  void SetName   (char *name)  {strncpy(fName,name,11); fName[11]='\0';}
  void SetMAT    (int mat)     {fMAT = mat;}
  void SetZA     (int za)      {fZA = za;}
  void SetAWR    (double awr)  {fAWR = awr;}
  void SetLRP    (char lrp)    {fLRP = lrp;}
  ...
  void SetLIS    (unsigned char lis)   {fLIS = lis;}
  void SetLISO   (unsigned char liso)  {fLISO = liso;}
  ...
  void SetNXC    (int nxc);
  void SetZSYMAM (const char *zsymam)  {strncpy(fZSYMAM,zsymam,11);fZSYMAM[11]='\0';}
  void SetALAB   (const char *alab)    {strncpy(fALAB,alab,11);fALAB[11]='\0';}
  void SetEDATE  (const char *edate)   {strncpy(fEDATE,edate,10);fEDATE[10] = '\0';}
  void SetAUTH   (const char *auth)    {strncpy(fAUTH,auth,33);fAUTH[33] = '\0';}
  void SetREF    (const char *ref)     {strncpy(fREF,ref,21);fREF[21] = '\0';}
  void SetDDATE  (const char *ddate)   {strncpy(fDDATE,ddate,10);fDDATE[10] = '\0';}
  void SetRDATE  (const char *rdate)   {strncpy(fRDATE,rdate,10);fRDATE[10] = '\0';}
  ...
  unsigned char       GetLIS()    const {return fLIS;}
  unsigned char       GetLISO()   const {return fLISO;}
...
private:
  ...
  char   fLRP;        // True if resonance parameters given in File 2
  ...
  unsigned char  fLIS;        // State number of the target nucleus; 0=ground state
  unsigned char  fLISO;       // Isomeric state number; 0=ground state
