...
class TNudyLibrary : public TNamed {
 public:
  ...
  TNudySubLibrary* GetSubLib(TParticlePDG *particle) {return (TNudySubLibrary*)fSubLib->FindObject(particle->GetName());}  //Get sublibrary by particle
  TNudySubLibrary* SetSubLib(TParticlePDG *particle){
    return fCurSubLib = GetSubLib(particle);
  }
