...
G4bool LHCbApp::fgIsScoreActive = FALSE;
...
G4bool LHCbApp::Initialize() {
  ...
  for (unsigned int ivol = 0; ivol < lvs->size(); ++ivol) {
    ...
    fLogicVolToIndex[vol] = ivol;

    G4String svol          = vol->GetName();

    // ECAL cells
    if (svol.compare("/dd/Geometry/DownstreamRegion/Ecal/Modules/InnCell")==0 ||
	svol.compare("/dd/Geometry/DownstreamRegion/Ecal/Modules/MidCell")==0 ||
	svol.compare("/dd/Geometry/DownstreamRegion/Ecal/Modules/OutCell")==0) 
      {
    ...
    // HCAL cells  
    if (svol.compare("/dd/Geometry/DownstreamRegion/Hcal/Cells/lvHcalInnCellUpScTile")==0 ||
	svol.compare("/dd/Geometry/DownstreamRegion/Hcal/Cells/lvHcalInnCellLowScTile")==0 || 
	svol.compare("/dd/Geometry/DownstreamRegion/Hcal/Cells/lvHcalOutCellScTile")==0) {
      std::cout<< "---  indx = "<< ivol <<"  Name = "<<svol<<std::endl;
  ...
  G4cout<< G4endl<<
  "=== LHCbApp::Initialize: necal = "<< necal << "  nhcal = " << nhcal << G4endl << G4endl;
  fInitialized = TRUE;
  return TRUE;
}


void LHCbApp::SteppingAction(const G4Step *step) {
  if (fgIsScoreActive == FALSE)
    return;
  ...
  ivol = fLogicVolToIndex[vol];
  idtype = 0;
  if(fSensFlags[ivol]) { 

    if (vol->GetName()[30] == 'E') idtype = 1;
    else idtype = 2;
    switch (idtype) {
      case 1:
