...
void RunAction::BeginOfRunAction(const G4Run *)
{
  ...
  G4cout << "\n " << partName << " (" << G4BestUnit(energy, "Energy") << ") in " << material->GetName()
  ...
  GetCuts();
  if (charge != 0.) {
    G4cout << "\n  Range cuts : \t gamma " << std::setw(8) << G4BestUnit(fRangeCut[0], "Length") << "\t e- "
           << std::setw(8) << G4BestUnit(fRangeCut[1], "Length");
    G4cout << "\n Energy cuts : \t gamma " << std::setw(8) << G4BestUnit(fEnergyCut[0], "Energy") << "\t e- "
           << std::setw(8) << G4BestUnit(fEnergyCut[1], "Energy") << G4endl;
  }
  ...
  if (charge != 0.) {
    ...
    G4cout << "\n  Max_energy _transferable  : " << G4BestUnit(Tmax, "Energy") << " (" << G4BestUnit(range, "Length")
           << ")" << G4endl;
  }
  ...
  for (size_t j = 0; j < emName.size(); j++)
    G4cout << "\t" << std::setw(16) << std::right << emName[j] << std::setw(10) << " ";
  G4cout << "\t" << std::setw(16) << std::right << "total" << std::setw(10) << " ";
  ...
  if (material->GetNumberOfElements() == 1) {
    ...
    for (size_t j = 0; j < sigma0.size(); j++) {
      G4cout << "\t" << std::setw(14) << std::scientific << std::right << sigma0[j] / barn << std::setw(14) << std::left
             << "     [barn]";
    }
    // G4cout << G4endl;
  }
  ...
  for (size_t j = 0; j < sigma0.size(); j++) {
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << sigma0[j] * cm << std::setw(14) << std::left
           << "     [1/cm]";
  }
  G4cout << "\n  cross section per volume : ";
  for (size_t j = 0; j < sigma1.size(); j++) {
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << sigma1[j] * cm << std::setw(14) << std::left
           << "     [1/cm]";
  }

  G4cout << "\n  cross section per mass   : ";
  for (size_t j = 0; j < sigma2.size(); j++) {
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << sigma2[j] / (cm2 / g) << std::setw(14)
           << std::left << "    [cm2/g]";
  }
  ...
  for (size_t j = 0; j < sigma1.size(); j++) {
    ...
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << lambda / cm << std::setw(14) << std::left
           << "       [cm]";
  }
  ...
  for (size_t j = 0; j < sigma2.size(); j++) {
    ...
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << lambda / (g / cm2) << std::setw(14) << std::left
           << "    [g/cm2]";
  }
  G4cout << G4endl;
  ...
  G4cout << "\n \n  restricted dE/dx         : ";
  for (size_t j = 0; j <= nproc; j++) {
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << dedx1[j] / (MeV / cm) << std::setw(14)
           << std::left << "   [MeV/cm]";
  }

  G4cout << "\n      (MeVcm2/g)          : ";
  for (size_t j = 0; j <= nproc; j++) {
    G4cout << "\t" << std::setw(14) << std::scientific << std::right << dedx2[j] / (MeV * cm2 / g) << std::setw(14)
           << std::left << " [MeVcm2/g]";
  }
  dedxtot = 0.;
  ...
  G4cout << "\n \n  range from restrict dE/dx: "
         << "\t" << std::setw(14) << std::scientific << std::right << range1 / mm << std::setw(14) << std::left
         << "       [mm]"
  ...
  G4cout << "\n \n  transport mean free path : "
         << "\t" << std::setw(8) << G4BestUnit(MSmfp1, "Length") << " (" << std::setw(8)
