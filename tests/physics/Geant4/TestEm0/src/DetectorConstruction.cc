...
DetectorConstruction::DetectorConstruction() : G4VUserDetectorConstruction(), fBox(nullptr), fMaterial(nullptr)
{
  fBoxSize = 1 * mm;
  DefineMaterials();
...
void DetectorConstruction::DefineMaterials()
{
  ...
  G4Material *ams = new G4Material("ams", density = 7.409 * g / cm3, ncomponents = 3);
  ...
  G4Material *butane = new G4Material("Isobutane", density = 2.42 * mg / cm3, ncomponents = 2, kStateGas,
                                      273.15 * kelvin, 1 * atmosphere);
  butane->AddElement(C, natoms = 4);
  butane->AddElement(H, natoms = 10);

  G4Material *ArButane = new G4Material("ArgonButane", density = 1.835 * mg / cm3, ncomponents = 2, kStateGas,
                                        273.15 * kelvin, 1. * atmosphere);
...
{
  G4cout << "\n The Box is " << G4BestUnit(fBoxSize, "Length") << " of " << fMaterial->GetName() << G4endl;
}
...
void DetectorConstruction::SetMaterial(const G4String &materialChoice)
{
  ...
  if (pttoMaterial) {
    ...
    if (fBox) {
      fBox->GetLogicalVolume()->SetMaterial(fMaterial);
    }
    G4RunManager::GetRunManager()->PhysicsHasBeenModified();
  } else {
    G4cout << "\n--> warning from DetectorConstruction::SetMaterial : " << materialChoice << " not found" << G4endl;
  }
}
